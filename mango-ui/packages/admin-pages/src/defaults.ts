...
export function registerDefaultAdminPages(options: RegisterDefaultAdminPagesOptions = {}) {
  ...
  const registries: MangoPageRegistry[] = [
    ...
    {
      moduleCode: 'mango-auth',
      pages: {
        'auth/provider-config/index': () => import('@mango/auth').then((m) => m.ProviderConfigView),
      },
    },
    {
      ...
      pages: {
        ...
        'system/event/index': () => import('@mango/system').then((m) => m.SystemEventView),
      },
    },
  ];
  ...
  registries
    .filter((registry) =>
      isMangoAdminFeatureEnabled(enabledFeatures, resolveMangoAdminModuleFeature(registry.moduleCode)),
    )
    .filter((registry) => {
