...
提供单体和 Shell 共用的页面注册、默认页面和 feature 注册入口。

主要使用者：前端开发者、业务开发者和 AI Agent。

## 2. 适用场景

新增前端模块页面、菜单 component 映射或开发页注册时使用。
...
包名：`@mango/admin-pages`。本包只提供前端运行时、页面、组件、API 封装或页面注册能力，不改变后端接口契约。

## 5. 接入方式

在业务前端或 Mango 前端包中引入 `@mango/admin-pages`；如包导出 `./admin-pages`，需要同步注册到统一页面注册表。
...
```bash
pnpm -F @mango/admin-pages build
```

## 11. 业务接入最小闭环

业务前端模块导出 `admin-pages`，把页面注册进统一 registry，验证菜单 component 能解析到页面。
