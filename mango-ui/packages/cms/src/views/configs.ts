import type { CmsApi, CmsPageQuery } from '../api/cms';
import type { CmsResourceConfig } from './CmsResourceView.vue';

type CmsPageData = Awaited<ReturnType<CmsApi['pageSites']>>;
...
export function createCmsResourceConfigs(cmsApi: CmsApi) {
  const siteConfig: CmsResourceConfig = {
    key: 'sites',
    title: '站点管理',
    ...
    page: (query, options) => cmsApi.pageSites(query, options) as Promise<CmsPageData>,
  ...
  const contentCategoryConfig: CmsResourceConfig = {
  ...
  const contentTagConfig: CmsResourceConfig = {
    key: 'contentTags',
    title: '内容标签',
  ...
  const siteCategoryConfig: CmsResourceConfig = {
    key: 'siteCategories',
    title: '站点栏目',
    ...
    normalizePayload: (payload, form) => ({
      ...payload,
      categoryCode: String(
        form.categoryCode || payload.categoryCode || createCategoryCode(payload.categoryName, payload.accessPath),
      ),
  ...
  const contentConfig: CmsResourceConfig = {
    key: 'contents',
    title: '内容管理',
    ...
    columns: [
      ...
      {
        prop: 'contentType',
        subProp: 'categoryName',
        label: '类型/分类',
        type: 'meta',
        optionKey: 'contentType',
        minWidth: 160,
      },
    ...
    detail: {
      groups: [
        ...
        {
          title: '正文',
          fields: [{ prop: 'body', label: '正文', type: 'richText', detailSpan: 2 }],
        },
      ],
    },
  };

  const publishConfig: CmsResourceConfig = {
  ...
  const navigationConfig: CmsResourceConfig = {
    key: 'navigations',
    title: '导航管理',
    ...
    normalizePayload: (payload, form) => {
      ...
      const next = {
        ...
        openTarget: jumpType === 'URL' ? String(payload.openTarget || form.openTarget || 'SELF') : 'SELF',
    ...
    editor: {
      ...
      fields: [
        ...
        { prop: 'externalUrl', label: '链接地址', required: true, visibleWhen: { jumpType: 'URL' } },
        {
          ...
          label: '打开方式',
          type: 'select',
          optionKey: 'openTarget',
          visibleWhen: { jumpType: 'URL' },
        },
        { prop: 'sort', label: '排序', type: 'number' },
        { prop: 'status', label: '状态', type: 'select', optionKey: 'status', required: true },
      ],
  ...
  const advertisementConfig: CmsResourceConfig = {
    key: 'advertisements',
    title: '广告位管理',
    ...
    editor: {
      ...
      fields: [
        ...
        {
          prop: 'supportedMaterialTypes',
          label: '支持物料',
          type: 'checkButtonGroup',
          optionKey: 'materialType',
          required: true,
          span: 24,
        },
  ...
  const adDeliveryConfig: CmsResourceConfig = {
    key: 'adDeliveries',
    title: '广告投放管理',
    ...
    normalizePayload: (payload, form) => {
      ...
      return {
        ...payload,
        materialType,
        imageFileId: ['IMAGE', 'SINGLE_IMAGE'].includes(materialType)
    ...
    editor: {
      ...
      fields: [
        ...
        },
        {
          ...
          height: 320,
          span: 24,
          visibleWhen: { materialType: 'RICH_TEXT' },
        },
        {
          prop: 'htmlContent',
          label: 'HTML 内容',
