...
- 用户管理在具体组织内提供“移出当前部门”，仅解除当前组织关系；“移出租户成员”会撤销当前租户角色和全部部门关系，但保留原账号与成员标识。
- 新增成员会先查询登录账号三态；当前租户可恢复账号展示脱敏姓名、联系方式、成员编号和移出时间，可恢复原成员或改用新账号，同名姓名不作为账号冲突依据。
- 排查登录后菜单、按钮权限和组织数据。
...
- 原成员恢复异常时，检查 `/identity/users/account-availability` 和 `/org/member-accounts/restore`；账号查询失败时页面停留在新增表单，不继续提交创建请求。
