...
菜单的 component 字段应与上面的默认页面 key 保持一致。角色管理的「分配权限」弹框直接展示后端可分配菜单树，后端返回的按钮节点会与菜单节点一起展示，供角色按需勾选授权。弹框初始化只把角色已授权的叶子节点交给树组件，祖先目录由树组件显示为半选，不会因此勾选未授权兄弟节点；保存时提交全选节点与半选祖先的去重并集，角色菜单 API 和字符串 ID 语义不变。
...
- Issue #918 修复角色菜单树把已授权父目录直接设为全选而扩展勾选兄弟节点的问题。页面 key、角色菜单 API、后端返回协议和 Element Plus 级联模式保持不变。

- Issue #805 待发布修复在 `postApi` 边界将后端分页字段 `total`、`current`、`size` 的 numeric string 转为 number，并将已确认的 Element Plus Radio value 用法从废弃 `label` 迁移到 `value`。后端大 ID 仍按字符串传递；岗位分页 API、表单值、页面 key、菜单、权限和租户语义不变。
