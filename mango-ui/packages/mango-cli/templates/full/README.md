...
- 本地启动由全局 `@mango/cli` 读取 `mango.dev.json` 执行。
...
| `application.yml` | `mango.seed.enabled` | `${MANGO_SEED_ENABLED:false}` | seed 开关 | 默认不初始化官方种子数据 | `application.yml` |
| `application.yml` | `mango.seed.admin.initial-password` | `${MANGO_SEED_ADMIN_PASSWORD:}` | 初始管理员密码 | 启用 seed 时必须显式提供 | `application.yml` |
...
full preset 默认后端 app 依赖 `io.mango:mango-admin-starter`。seed 能力不是默认业务初始化入口，启用前要确认依赖、空库状态和管理员初始密码。
...
## 8. 数据与初始化
模板自身不直接写数据库；数据库由后端启动时的 Flyway 模块和可选 seed runner 初始化。
...
| seed 初始化 | `mango.seed.enabled` | 默认租户、管理员、套餐、角色等官方种子数据 | seed runner 幂等逻辑 | 显式设置 `MANGO_SEED_ENABLED=true` 后启动 | 查 seed 日志、用户、租户、角色和菜单授权 |
...
生产环境不得使用模板默认 JWT secret、空数据库密码或弱 seed 密码。
...
|-------------|---------------|--------|----------|-----------------|--------------|
| Mango 平台页面 | 各 `@mango/*` 包登记 | 各平台模块定义 | 平台模块 migration 或 resource manifest | 平台模块或 seed 定义 | 各平台模块 Controller / Service |
...
4. 首次需要平台默认数据时，明确启用 seed 并提供强密码；不要在已有业务库上盲目启用。
...
| 问题 | 原因 | 处理方式 |
|------|------|----------|
| `global mango CLI not found` | 机器没有安装全局 CLI | 安装 `@mango/cli` 后重试 |
...
| seed 没有生成管理员 | `MANGO_SEED_ENABLED` 未启用或密码为空 | 显式设置 seed 相关环境变量并查看启动日志 |
