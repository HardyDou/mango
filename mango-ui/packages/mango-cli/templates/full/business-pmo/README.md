...
事实明确后，交付保障 Skill 调用中文批量选择器；用户在同一界面选择文档版本、勾选 M01-M16，再用一次回车确认：
...
PR 模板分别记录需求影响和解决方案风险，最终等级取二者最大值，并记录文档版本、工作区决策、不可降级事实、已勾选措施、证据与剩余风险。`直接做` 不创建交付文档；`一页纸/标准版/详细版` 各使用一份文档；`四文档` 分别维护业务需求、系统需求、技术设计和实施与验证计划。

M09-M16 只按真实观察面推荐，用户勾选即采用；未选项不逐项填写否定说明。例如只移动按钮位置且行为不变时可选择 M09 静态验证与 M13 页面验证；后端租户或事务结果由真实集成或接口入口证明，没有浏览器入口时不添加 M13。
