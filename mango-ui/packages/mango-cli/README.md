...
| 初始化和启动本地开发工作区 | `mango workspace init`、`mango validate`、`mango dev doctor`、`mango dev plan`、`mango dev start` | `.mango/workspace.json`、`.mango/dev-workspace.env`、`.mango/run` |
...
| `.mango/dev-workspace.env` | `MANGO_DB_AUTO_CREATE` | `true` | 数据库自动创建开关 | `mango dev start` 启动 Spring Boot app 前创建 `mango_dev_*` 工作区数据库 | `defaultDevWorkspaceEnv`、`ensureWorkspaceDatabase` |
...
| `mango dev plan` | 展开启动计划 | group 或 app | 不改文件 |
| `mango dev start` | 启动本地开发应用 | group 或 app | `.mango/run` |
| `mango dev backend` | 启动后端分组 | 无 | `.mango/run` |
| `mango dev frontend` | 启动前端分组 | 无 | `.mango/run` |
...
| 本地数据库连接 | `.mango/dev-workspace.env` | DB host、port、name、username、password | 无 | `mango dev start` 注入后端启动参数 | `mango dev plan` 查看命令，`mango dev logs <backend>` 查 datasource |
...
4. 执行 `mango validate`、`mango dev doctor`、`mango dev plan`，确认 manifest、工具链和端口。
5. 执行 `mango dev start`，通过 `mango dev status`、`mango dev logs <app>` 查看状态。
