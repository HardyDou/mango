...
| 编排本地优先发布批次           | `mango release plan`、`prepare`、`publish`、`status`、`repair` 与 `mango release registry doctor`                 | `.changeset/release-plan.json`、`.runtime/mango-release/<planDigest>`         |
...
- `mango release` 只允许在 Mango 主仓内编排 Mango 平台组件发布，不用于 Mango 普通 PR、业务 PR、业务应用打包、部署或发布；业务项目安装 CLI 后也不会获得项目级 `mango-release` Skill。
...
npm view @mango/cli@1.0.109 version --registry "$MANGO_NPM_REGISTRY"
npm install -g @mango/cli@1.0.109 --registry "$MANGO_NPM_REGISTRY"
```
...
生成项目中的 `scripts/dev-workspace.sh` 只保留为历史兼容 shim，会把旧命令转发到 Mango CLI。历史项目升级时，先用全局 CLI 执行 `mango pmo upgrade --project-dir . --to 1.3.8 --sync-shell`；该升级会锁定升级前已有、且当前合同明确支持的生命周期文档版本。已经锁定到该 bundle 的项目只需用 `mango pmo sync --project-dir . --sync-shell` 修复当前锁。随后进入 `frontend` 安装项目内依赖，并在每个 active worktree 执行 `pnpm exec mango workspace init` 生成 `.mango/workspace.json` 并补齐 `.mango/dev-workspace.env`。
...
| `mango.dev.json`                  | `apps.<name>.mangoLifecycle`     | 按源码入口自动识别                            | 显式启用 Mango 生命周期     | `true` 时本地启动先准备 bootstrap 回执，再启动 runtime                                    | `resolveDevCommand`、`prepareManagedMangoLifecycle`    |
...
| `.mango/dev-workspace.env`        | （已移除）                       |                                               | 后端自动 install 开关       | Spring Boot 开发始终从当前 Maven reactor 编译运行，不再安装 app 制品                      | `dev-maven-reactor.mjs`                                |
...
mango release prepare --consume-registry https://nexus.example/repository/npm-group/
pnpm -C mango-ui release:local-check -- --base=origin/main --head=HEAD
...
| `mango pmo upgrade`              | 原子升级到 CLI 精确依赖的 `@mango/pmo` bundle，并锁定升级前已有的受支持历史生命周期文档版本                                   | `--project-dir`、`--to`、`--dry-run`、`--write-agents`、`--sync-shell`、`--adopt-governance` | PMO bundle、项目 Skill、GitHub/Gitea workflow、历史文档版本基线、兼容资产和 `.mango` 备份目录          |
| `mango pmo rollback`             | 原子恢复已校验的本地 PMO 备份                                                                                                 | `--project-dir`、`--to`、`--dry-run`                                                         | PMO bundle、项目 Skill、PR 模板、GitHub/Gitea workflow 和 `.mango` 备份目录；不覆盖项目预算            |
...
| 业务 API 模板        | `frontend/packages/<module>-api/src/api.ts`                                                             | `createXxxApi(HttpClient)` 的类型化 CRUD 端口                                        | host 注入的 client instance | 注册业务页面前创建                      | 禁止 Vue、Axios、环境变量和绝对 endpoint                                                |
| 业务聚合列表页模板   | `frontend/packages/<module>/src/views/<module>/<aggregate>/index.vue`                                   | 基于 `@mango/common` 列表四件套和 `MangoDialog` 的 CRUD 起点，默认启用常用搜索项折叠 | component key               | `mango module add` 后写入前端页面包     | 检查列表四件套、`MangoDialog`、语义锚点和前端页面增量门禁；特殊页面支持按类型或整页例外 |
...
Windows 后台启动会由 CLI 随包提供的 Node runner 托管 `.cmd`，让应用 stdout/stderr 持续写入 `.mango` 下的 `run/logs/<app>.log`；`mango dev restart` 和 `mango dev stop` 按 runner PID 停止完整子进程树。命令参数中的空格和 `&` 等 shell 元字符会作为单个参数传递，业务项目不需要增加引号、重定向或额外 wrapper。若 Windows `taskkill` 明确无法非强制终止包含 Java、Maven 或 Node 子进程的树，CLI 会立即升级为强制停止，不消耗应用配置的完整优雅等待时间。

已有业务项目同步：

1. 在项目根目录确认有 `mango.config.json` 和 `mango.dev.json`。
2. 首次迁移或升版先执行 `mango pmo upgrade --project-dir . --to 1.3.8 --dry-run` 查看计划；计划会列出将写入的历史生命周期文档版本基线。
...
6. 需要恢复时先执行 `mango pmo rollback --project-dir . --dry-run`；rollback 恢复 workflow 和 PR 模板，但不回滚项目预算。只有明确要同步兼容启动脚本时才加 `--sync-shell`。
7. 如果 `AGENTS.md` 仍引用外部 `mango-pmo`，先人工确认，再用 `--write-agents` 迁移。
...
| Windows 下应用已健康但 `mango dev logs <app>` 没有 runtime 输出               | 使用的 CLI 版本没有通过随包 Node runner 托管后台 `.cmd`，或应用不是由当前 workspace 启动           | 先用 `pnpm exec mango --version` 确认项目锁定的 CLI，升级到包含 Windows 后台 runner 的版本后执行 `pnpm exec mango dev restart <app>`；不要在 manifest 中手工追加 `>>` 重定向                         |
...
| `release registry doctor` 的 Maven baseline probe 失败                        | 当前计划或 CLI 矩阵无法解析已发布 Maven 源版本，或同一 BOM POM 没有同时出现在 publish/consume 仓库 | 先核对 `.changeset/release-plan.json` 的 `maven.sourceVersion` 和 `release-versions.json`；再按输出中的精确 `io.mango:mango-bom:<version>` 路径检查两个仓库，禁止用仓库根目录 404 代替制品结论       |
...
`@mango/cli@1.0.107` 将前端矩阵更新为 `@mango/admin-shell@1.0.60`、`@mango/admin@1.0.66` 与不变的 `@mango/common@1.0.26`，修复旧矩阵在 Admin 生产构建时导入 Common 不存在导出的错误。Mango Maven 保持 `1.0.36`，`@mango/pmo` 升级到 `1.3.15`，其它 npm 坐标不变。
...
`@mango/cli@1.0.94` 锁定 `@mango/pmo@1.3.8`。升级时，CLI 只为升级前已有、合同显式允许且内容 SHA-256 未变化的历史生命周期文档写入版本基线；它不改写文档正文、审批证据或上游摘要。新建或迁移后的文档仍必须使用 `1.3.8`。
...
`@mango/cli@1.0.91` 把生成和升级项目锁定到 Mango Maven `1.0.27`、`@mango/pmo@1.3.6` 和本批完整前端版本矩阵。该批次新增 File HTTP multipart 上传能力，完善 Notice 多邮箱配置、路由与密钥管理及带附件邮件发送，并统一 `Require.nonNull` 与 SpotBugs 空值检查约定；业务项目应按本批精确版本整体升级，现有公共入口保持兼容。

### 1.0.90 发布影响

`@mango/cli@1.0.90` 把生成和升级项目锁定到 Mango Maven `1.0.26`、`@mango/pmo@1.3.5` 和本批完整前端版本矩阵。该批次包含模块运行诊断、受控存量模块纳管、资源同步、Workflow 管理员数据范围、消息路由与富文本资源能力；继承 `mango-parent` 的项目更新统一 `mango.version`，保留自有 Parent 的项目导入 `io.mango:mango-bom:1.0.26`。CLI 命令和现有项目运行时配置保持兼容。
...
`@mango/cli@1.0.83` 首次生成 `createXxxApi(HttpClient)` 业务 API，host 通过 `@mango/http-client@1.0.0` 创建实例并注入业务页面注册器；页面不再依赖全局 request 或 Axios。该版本仅形成历史本地候选，未发布。
