...
| 能力 | 常用入口 |
|------|----------|
| 新建 Mango 业务项目：mango init &lt;project&gt; --preset full 或 mango init &lt;project&gt; --preset custom | 前端注册 / 组件 / API 封装 |
| custom 项目追加 Mango 可选能力：mango add file workflow --project-dir &lt;dir&gt; | 前端注册 / 组件 / API 封装 |
| 生成业务模块骨架：mango module add order --aggregate order-item --project-dir &lt;dir&gt; | 前端注册 / 组件 / API 封装 |
| 同步业务 PMO baseline：mango pmo sync --project-dir &lt;dir&gt; | 前端注册 / 组件 / API 封装 |
| 初始化、校验、诊断和启动本地开发工作区：mango init-dev、mango validate、mango doctor、mango plan、mango start | 前端注册 / 组件 / API 封装 |
| 查看 CLI 发布说明：mango changelog | 前端注册 / 组件 / API 封装 |

## 3. 适用场景
- 新建 Mango 业务项目：`mango init <project> --preset full` 或 `mango init <project> --preset custom`。
- custom 项目追加 Mango 可选能力：`mango add file workflow --project-dir <dir>`。
- 生成业务模块骨架：`mango module add order --aggregate order-item --project-dir <dir>`。
- 同步业务 PMO baseline：`mango pmo sync --project-dir <dir>`。
- 初始化、校验、诊断和启动本地开发工作区：`mango init-dev`、`mango validate`、`mango doctor`、`mango plan`、`mango start`。
- 查看 CLI 发布说明：`mango changelog`。
...
| `templates/full/mango.dev.json` | 生成项目开发工作区 manifest | 业务项目可用 `.mango/dev-workspace.local.json` 本机覆盖 |
