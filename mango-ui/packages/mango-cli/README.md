...
| 业务聚合列表页模板   | `frontend/packages/<module>/src/views/<module>/<aggregate>/index.vue`                                   | 基于 `@mango/common` 列表四件套和 `MangoDialog` 的 CRUD 起点，默认启用常用搜索项折叠 | component key               | `mango module add` 后写入前端页面包     | 检查列表四件套、`MangoDialog`、语义锚点和前端页面增量门禁                           |
