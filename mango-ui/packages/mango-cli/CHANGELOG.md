...
- [PR #884](https://github.com/HardyDou/mango/pull/884) Added Resource cold-baseline and incremental-release behavior. Packages: Mango Maven `1.0.43`, docs bundle and `@mango/cli@1.2.3`. Business Adaptation: generated consumers can use BSQL reset release and Resource hash-based incremental release while preserving admin-modified `SYSTEM_CONFIG` values.
- [PR #885](https://github.com/HardyDou/mango/pull/885), [PR #886](https://github.com/HardyDou/mango/pull/886) and [PR #887](https://github.com/HardyDou/mango/pull/887) Added tenant-scoped Workflow identity, participation assignment and designer Provider contracts. Packages: Mango Maven `1.0.43`, `@mango/workflow@1.0.45`, its generated dependency closure and `@mango/cli@1.2.3`. Business Adaptation: generated consumers must apply Flyway normally, use stable user IDs and load designer candidates through Workflow.

### Fixed

- Carry portable Resource BSQL materialization, changed-only synchronization and `SYSTEM_CONFIG` runtime-edit preservation into generated consumers.

### Added

- Carry Workflow participation assignment and tenant-scoped designer options into the generated compatibility tuple.

### Changed

- Lock generated projects to Mango Maven `1.0.43` and the generated npm topology ending at `@mango/cli@1.2.3`; PMO remains `1.4.2`.

### Versions

- `@mango/cli`: `1.2.2` to `1.2.3`; Mango Maven: `1.0.42` to `1.0.43`; `@mango/workflow`: `1.0.44` to `1.0.45`; PMO remains `1.4.2`.

### Published Packages

- Publish the complete non-application Maven/docs batch at `1.0.43`, followed by the generated npm topology `@mango/workflow@1.0.45` -> `@mango/admin-shell@1.0.66` -> `@mango/workflow-business-example@1.0.44` -> `@mango/admin@1.1.2` -> `@mango/cli@1.2.3`.

### Business Impact

- Generated consumers gain reset/incremental Resource release support and the current Workflow identity/assignment/designer contracts. Resource baseline generation is limited to one datasource group, and generic runtime-edit preservation remains Handler-specific outside `SYSTEM_CONFIG`.

### Upgrade Estimate

- Audience: generated Mango consumers, Resource Bootstrap consumers and Workflow API/UI consumers.
- Engineering Effort: 30 to 90 minutes for aggregate consumers; 1 to 3 hours for direct consumers or custom handlers.
- Execution Window: 1 to 3 hours including dependency upgrade, Flyway, clean build and reset/incremental verification.
- Service Downtime: no framework-mandated downtime; applications use their normal restart window.
- Rollback Effort: 15 to 45 minutes to restore CLI `1.2.2`, Maven `1.0.42` and the prior npm tuple.
- Assumptions: Node `22.23.1`, Java 21, clean locks, configured registries and a disposable local MySQL validation database.

### Upgrade Notes

1. Upgrade CLI to `1.2.3` and apply its Maven `1.0.43` plus exact npm compatibility matrix.
2. Run Flyway normally for existing databases; use packaged BSQL only for an empty reset database.
3. Verify unchanged Resource hashes skip execution and admin-modified `SYSTEM_CONFIG` values remain authoritative.
4. Use stable Workflow user IDs and the Workflow designer options endpoint.
...
- Restore CLI `1.2.2`, Maven `1.0.42` and the prior exact npm tuple; never overwrite immutable `1.0.43`, `1.2.3` or package coordinates.
...
- Lock the release operator tuple to `@mango/pmo@1.4.1` and preserve Maven `1.0.39` as the unchanged backend prerequisite.

### Versions

- `@mango/cli`: `1.1.0` to `1.1.1`; PMO: `1.4.0` to `1.4.1`.

### Published Packages

- Publish `@mango/pmo@1.4.1` before `@mango/cli@1.1.1`; no Maven coordinate is republished.

### Business Impact

- Release operators receive the optimized Maven verification defaults; generated runtime projects have no API or database migration.

### Upgrade Estimate

- Audience: Mango maintainers and release operators.
- Engineering Effort: 10 to 20 minutes.
- Execution Window: 15 to 30 minutes including clean consumer verification.
- Service Downtime: none.
- Rollback Effort: 5 to 15 minutes to restore CLI `1.1.0` and PMO `1.4.0`.
- Assumptions: configured npm registries, clean locks and Maven `1.0.39` consume access.

### Upgrade Notes

1. Upgrade CLI and PMO together to the exact tuple.
...
- Restore CLI `1.1.0` and PMO `1.4.0`; never overwrite immutable coordinates.
...
- Keep the local Maven candidate repository outside wildcard mirror routing and verify every sealed coordinate through one generated consumer POM, so `prepare` neither queries unpublished coordinates from the consume registry nor starts 188 Maven processes.
