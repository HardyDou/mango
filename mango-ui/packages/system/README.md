...
`@mango/system` 是 Mango 系统管理前端包，提供字典、系统配置、网站配置、租户、公共路径、地区、业务域、登录日志、操作日志、系统事件页面，并导出系统配置面板、业务域选择和参与人选择等后台业务组件。
...
<template>
  <SystemConfigPanel :domain-codes="['ORDER']" :domain-labels="{ ORDER: '订单中心' }" />
</template>
```
...
| `adminBrandingApi` | `/system/admin-branding`、`/system/admin-branding/public`            | 网站配置读取、保存和未登录公共读取。               |
...
| 网站配置                     | `mango-system` 的 `sys_config` 和文件中心 ID         | `AdminBrandingView`、Admin Shell 登录页和框架展示。 |
...
| ---------------------- | --------------------------------------------------------------------------------------------------- |
| 系统配置               | 可能影响全局业务开关。                                                                              |
| 网站配置               | 会影响登录页、后台展开 Logo、折叠 Logo、浏览器标题、favicon 和页脚展示；图片字段只保存文件中心 ID。 |
| 租户                   | 可能影响租户状态和菜单授权。                                                                        |
...
| 网站图片不显示       | 图片字段为空、文件 ID 无效或文件中心预览失败               | 确认保存值是文件中心 ID，再查文件中心预览接口。             |
