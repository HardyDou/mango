...
包名：`@mango/calendar`。本包只提供前端运行时、页面、组件、API 封装或页面注册能力，不改变后端接口契约。

## 5. 接入方式

在业务前端或 Mango 前端包中引入 `@mango/calendar`；如包导出 `./admin-pages`，需要同步注册到统一页面注册表。
...
```bash
pnpm -F @mango/calendar build
```

## 11. 业务接入最小闭环

业务前端注册 calendar admin-pages，访问菜单并验证列表、详情和 API 请求。
