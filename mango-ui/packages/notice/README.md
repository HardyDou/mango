...
包名：`@mango/notice`。本包只提供前端运行时、页面、组件、API 封装或页面注册能力，不改变后端接口契约。

## 5. 接入方式

在业务前端或 Mango 前端包中引入 `@mango/notice`；如包导出 `./admin-pages`，需要同步注册到统一页面注册表。
...
```bash
pnpm -F @mango/notice build
```

## 11. 业务接入最小闭环

业务应用注册 notice admin-pages 和客户端组件，验证消息列表、已读状态和实时推送。
