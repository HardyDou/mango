...
| 规则片段编辑 | 支持 `TEXT`、`DATE`、`PARAM`、`SEQ`、`EXPR` 片段。DATE 可配置 `yyyy`、`MMdd` 或任意 Java `DateTimeFormatter` 格式。 |
| 编号预览     | 按当前规则和动态参数预览编号。                                                                                      |
...
分离式保函协议号 `2026年分离式字第CH04220001号` 可配置为：`TEXT(2026年分离式字第)`、`TEXT(CH)`、`DATE(MMdd, sequenceScope=1)`、`SEQ(4位, 0)`、`TEXT(号)`。日期片段参与分组后，每个出函日期的流水从 `0001` 开始。

保函编号 `516（2026）年第CH0028号` 可配置为：`TEXT(516（)`、`DATE(yyyy)`、`TEXT(）年第)`、`TEXT(CH)`、`SEQ(4位, 0)`、`TEXT(号)`，并将 `DATE(yyyy)` 的 `sequenceScope` 设为 `1`，流水片段本身不参与分组。这样每年从 `0001` 重新开始。
...
- `@mango/numgen@1.0.21` 将精确依赖升级到 `@mango/admin-pages@1.0.20` 和 `@mango/system@1.0.19`；编号 API、
  页面 key、权限、租户和运行时行为相对 `1.0.20` 不变。
