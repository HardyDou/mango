...
- 本次 PR 新增 `@mango/grid-widgets` 网址导航系统小组件，并在工作台默认布局首行展示；不改变 Shell 菜单 `component` key、菜单树接口、页面注册方式、角色授权、按钮权限、租户绑定、登录态装配和运行时页面加载协议。网址导航小组件默认消费 link 模块我的收藏网址，并提供百度、谷歌外部搜索入口。
