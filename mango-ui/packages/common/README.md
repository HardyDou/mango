...
提供请求、消息、菜单树、标签页、验证码、编辑器、选择器等公共能力。

主要使用者：前端开发者、业务开发者和 AI Agent。

## 2. 适用场景

业务前端开发通用 UI、请求和工具能力时使用。
...
包名：`@mango/common`。本包只提供前端运行时、页面、组件、API 封装或页面注册能力，不改变后端接口契约。

## 5. 接入方式

在业务前端或 Mango 前端包中引入 `@mango/common`；如包导出 `./admin-pages`，需要同步注册到统一页面注册表。
...
业务页面按需引入组件或工具，运行单测或构建验证样式、请求和交互。
