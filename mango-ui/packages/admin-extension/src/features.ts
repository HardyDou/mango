export type MangoAdminFeatureCode =
  'authorization' | 'system' | 'workflow' | 'file' | 'template' | 'notice' | 'numgen' | 'calendar' | 'job';

export type MangoAdminFeaturePreset = 'core' | 'full';

export type MangoAdminFeatures =
  MangoAdminFeaturePreset | MangoAdminFeatureCode[] | Partial<Record<MangoAdminFeatureCode, boolean>>;

export const MANGO_ADMIN_CORE_FEATURES: MangoAdminFeatureCode[] = ['authorization', 'system'];
...
export function resolveMangoAdminFeatures(features: MangoAdminFeatures = 'core') {
  ...
  if (Array.isArray(features)) {
    return new Set([...MANGO_ADMIN_CORE_FEATURES, ...features]);
  }
  return new Set(
    MANGO_ADMIN_FULL_FEATURES.filter(
      (feature) => MANGO_ADMIN_CORE_FEATURES.includes(feature) || features[feature] === true,
    ),
  );
}
