...
`@mango/link-page@1.0.5` 只发布已合并的首页视觉样式优化。组件的 props、事件、导出类型、Open API 路径、卡片数据字段和 `@mango/link-page/style.css` 引入方式均保持不变；业务项目升级依赖版本后无需调整路由、权限、菜单或后端数据。
...
| 卡片字段   | 展示 logo、名称、地址、一句话介绍、推荐标识和标签。                        |
...
| `recommended`  | 为 `true` 时展示推荐标识。               |
| `tags`         | 展示为卡片标签，同时参与后端关键词匹配。 |
| `categoryName` | 分组归属。                               |
| `sortNo`       | 同组排序兜底。                           |
