...
| `api-client`         | workflow 后端接口封装和 TypeScript 类型；业务申请日期-only 范围会补齐整日边界。 | 需要直接调用 workflow 接口的前端代码。   |
...
| 流程定义 `startEntryVisible`                                           | `true` / `false`                                                       | 是否展示在审批管理发起流程入口；默认 `true`，业务内嵌流程可设为 `false`。 |
...
检查后端流程定义是否已发布，当前租户是否有可用定义，当前账号是否有流程定义或发起流程权限。声明为“仅业务内嵌”的流程不会显示在审批管理发起流程列表中，需要从业务页面按业务上下文发起。
...
业务页面不要直接读取 workflow 表。查询当前进度使用 `businessApplyLatestProgress()` 或 `businessApplyLatestProgressBatch()`；审批、保存、认领和取消认领后需要立即刷新时，使用带 `WithResult` 的任务动作 API，读取返回值里的 `progress.currentTask`、`claimStatus`、`candidateUsers` 和 `candidateGroups`。
