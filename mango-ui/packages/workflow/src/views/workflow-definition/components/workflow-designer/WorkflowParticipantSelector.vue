<template>
  <div class="workflow-participant-selector">
    <button type="button" class="participant-trigger" @click="open">
      <span class="participant-trigger-text" :class="{ empty: !summaryText }">{{ summaryText || placeholder }}</span>
      <el-icon><ArrowDown /></el-icon>
    </button>

    <div v-if="selectedGroups.length" class="participant-selected-groups">
...
const emit = defineEmits<{
  'update:modelValue': [value: WorkflowParticipantValue];
  'ensure-users': [];
  'ensure-roles': [];
  'ensure-posts': [];
  'ensure-orgs': [];
...
const selectedSummary = computed(() => [
  { type: 'USER' as const, label: '用户', count: props.modelValue.userIds?.length || 0 },
  { type: 'ORG' as const, label: '部门', count: props.modelValue.orgIds?.length || 0 },
  { type: 'ROLE' as const, label: '角色', count: props.modelValue.roleIds?.length || 0 },
  { type: 'POST' as const, label: '岗位', count: props.modelValue.postIds?.length || 0 },
...
const summaryText = computed(() => {
  const total = selectedSummary.value.reduce((sum, item) => sum + item.count, 0);
  if (!total) return '';
  return `已选择 ${total} 项`;
});
...
function pickedOf(type: ParticipantType, values: string[], options: ApprovalTargetOption[]) {
...
function flattenOrgOptions(items: ApprovalOrgTreeOption[]): ApprovalTargetOption[] {
...
function filterOrgTree(items: ApprovalOrgTreeOption[], normalized: string): ApprovalOrgTreeOption[] {
  return (items || []).reduce<ApprovalOrgTreeOption[]>((result, item) => {
...
function handleOrgCheck(_node: ApprovalOrgTreeOption, checked: { checkedKeys: Array<string | number> }) {
  tempValue.value = {
    ...tempValue.value,
...
function normalizeValue(value: WorkflowParticipantValue): WorkflowParticipantValue {
  return {
...
function emptyValue(): WorkflowParticipantValue {
...
function valueKey(type: ParticipantType): keyof WorkflowParticipantValue {
...
.participant-trigger:hover {
  border-color: var(--el-color-primary-light-3);
}

.participant-trigger:focus-visible {
  outline: 0;
  border-color: var(--el-color-primary);
  box-shadow: 0 0 0 2px var(--el-color-primary-light-8);
}

.participant-trigger-text {
  overflow: hidden;
...
.participant-trigger-text.empty {
  color: var(--el-text-color-placeholder);
}

.participant-selected-groups {
  display: grid;
  gap: 6px;
  padding: 8px;
