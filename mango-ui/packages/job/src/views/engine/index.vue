  ...
  <div class="job-page">
    <section class="job-toolbar">
      <div class="job-toolbar-head">
        <div>
          <h2>运行状态</h2>
          <p>汇总 Mango Job 任务定义同步、调度实例和运行时健康状态。</p>
        </div>
    ...
    <section class="job-panel">
      ...
      <el-table v-loading="loading" :data="rows" stripe empty-text="暂无运行状态">
        <el-table-column prop="engineType" label="运行时" min-width="140" fixed="left" />
...
async function loadRows() {
  ...
  try {
    ...
    errorMessage.value = requestErrorMessage(error, '运行状态加载失败');
  } finally {
    loading.value = false;
  }
}
