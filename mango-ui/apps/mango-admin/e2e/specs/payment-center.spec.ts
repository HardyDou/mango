...
test.describe('支付中心 E2E', () => {
  ...
  test('Web 收银台网银和线下转账物料真实返回', async ({ page }) => {
    ...
    await cashierPage.getByRole('button', { name: /中国工商银行/ }).click();
    await cashierPage.getByPlaceholder('请输入账号或卡号').fill('622202060900000001');
    await cashierPage.getByPlaceholder('请输入付款户名').fill('芒果科技有限公司');
    await cashierPage.getByText('已完成企业网银授权准备').click();
