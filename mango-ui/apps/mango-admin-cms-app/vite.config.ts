...
export default defineConfig((mode: ConfigEnv) => {
  ...
  const proxyTarget = process.env.VITE_ADMIN_PROXY_PATH || env.VITE_ADMIN_PROXY_PATH || 'http://127.0.0.1:18081';
  const allowedHosts = ['localhost', '127.0.0.1', 'a.mango.io', 'b.mango.io', 'c.mango.io', 'd.mango.io', 'e.mango.io'];
  const allowedOrigins = createAllowedOrigins(process.env.VITE_MANGO_ALLOWED_ORIGINS || env.VITE_MANGO_ALLOWED_ORIGINS);
  ...
  return {
    ...
    server: {
      ...
      port: Number(env.VITE_PORT || 5184),
      allowedHosts,
      cors: {
    ...
    preview: {
      host: env.VITE_HOST || '0.0.0.0',
      port: Number(env.VITE_PREVIEW_PORT || 4184),
      allowedHosts,
