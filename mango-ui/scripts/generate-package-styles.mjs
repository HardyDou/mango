...
function findPackageJson(startPath, expectedName, rootDir) {
  ...
  fail(`Could not locate package.json for ${expectedName} from ${relativePath(rootDir, startPath)}`);
}
...
function validateStyleItems(items, consumerPackageJson, consumerPackagePath, rootDir) {
  ...
  return items.map((item) => {
    ...
    return {
      ...
      packageName: name,
      style: item.style,
      packageJsonPath: relativePath(rootDir, resolution.packageJsonPath),
      resolvedStylePath: relativePath(rootDir, resolution.resolvedStylePath),
    };
  });
}
...
function renderStyles(packages, options) {
  ...
  return [
    ...
    `/* Edit ${relativePath(options.root, options.manifest)}, then run the package style generation command. */`,
    imports,
    '',
  ].join('\n');
}
...
function renderBuildDepsScript(defaultPackages, fullPackages, consumerPackageJson, options) {
  ...
  return [
    ...
    '}',
    '',
    `// Generated from ${relativePath(options.root, options.manifest)}.`,
    '',
  ].join('\n');
}
...
function checkOrWrite(path, nextContent, options) {
  ...
  if (options.check) {
    if (currentContent !== nextContent) {
      fail(`${relativePath(options.root, path)} is out of date. Run the package style generation command.`);
    }
    return;
  }
...
export function main(argv = process.argv.slice(2)) {
  ...
  if (options.check) {
    console.log(`[package-styles] ${relativePath(options.root, options.out)} is up to date.`);
    console.log(`[package-styles] checked ${checkedCount} package style exports.`);
    return;
  }

  console.log(`[package-styles] wrote ${relativePath(options.root, options.out)}`);
  console.log(`[package-styles] checked ${checkedCount} package style exports.`);
}
