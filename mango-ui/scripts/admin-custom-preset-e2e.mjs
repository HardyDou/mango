...
const tempRoot = args.keepTemp
  ? mkdirAndReturn(resolve(tmpdir(), `mango-admin-custom-preset-e2e-${Date.now()}`))
  : mkdtempSync(join(tmpdir(), 'mango-admin-custom-preset-e2e-'));
const appRoot = join(tempRoot, 'admin-custom-preset-app');
const stagedPackageRoot = join(tempRoot, 'staged-packages');
const evidenceRoot = resolve(args.evidenceDir || join(workspaceRoot, 'mango-docs/evidence/2026-05-30-sprint-3/custom-preset'));
...
try {
  ...
  createCustomPresetApp(stagedPackages);
  ...
  if (!args.useExternalWebServer) {
    devServerProcess = startCustomPresetDevServer();
    waitForHttp(baseUrl);
  } else {
    waitForHttp(baseUrl);
  }
  ...
  runBrowserCheck();
  writeSummary();
  log('Admin custom preset E2E passed.');
} finally {
...
function createCustomPresetApp(stagedPackages) {
  ...
  writeFileSync(join(appRoot, 'src/main.ts'), [
    "import { createMangoAdmin, mangoDefaultCapabilities, resolveMangoAdminPreset } from '@mango/admin';",
    "import '@mango/admin/style.css';",
    '',
    '(globalThis as any).__MANGO_IMPORT_META_ENV__ = import.meta.env;',
    "const selectedCapabilities = mangoDefaultCapabilities.filter((capability) => ['system', 'template', 'file', 'notice', 'numgen', 'calendar', 'workflow'].includes(capability.capabilityCode));",
    "const presetResolution = resolveMangoAdminPreset({ preset: 'custom', capabilities: selectedCapabilities });",
    'const businessMenus = [',
    '  {',
    "    menuId: 'business:reports',",
    "    appCode: 'internal-admin',",
    "    moduleCode: 'business-reports',",
    '    parentId: 0,',
    '    menuType: 2,',
    "    menuName: '业务报表',",
    ...
    '  },',
    '];',
    "(window as any).__MANGO_ADMIN_CUSTOM_PRESET_E2E__ = 'createMangoAdmin:@mango/admin';",
    "(window as any).__MANGO_ADMIN_CUSTOM_PRESET_RESOLUTION__ = presetResolution.dependencyReport;",
    "(window as any).__MANGO_ADMIN_CUSTOM_BUSINESS_MENUS__ = businessMenus;",
    '',
    'createMangoAdmin({',
    "  preset: 'custom',",
    ...
    "      moduleCode: 'business-reports',",
    '      pages: {',
    "        'business/reports/index': () => import('./BusinessReports.vue'),",
    '      },',
    '    },',
    '  ],',
    "  mountTarget: '#app',",
    "  apiBaseUrl: '/api',",
    "  title: 'Mango Admin Custom Preset',",
    "  runtimeConfigUrl: '/mango-runtime-config.json',",
    '  menu: {',
    '    businessMenus,',
    "    onMergeReport: (report) => { (window as any).__MANGO_ADMIN_CUSTOM_MENU_MERGE_REPORT__ = report; },",
    '  },',
    '}).mount();',
    '',
  ].join('\n'), 'utf8');
  writeFileSync(join(appRoot, 'src/BusinessReports.vue'), [
    '<template>',
    '  <section class="business-reports-page">',
    '    <el-card shadow="never">',
    '      <template #header>',
    '        <div class="business-reports-page__header">',
    '          <span>业务报表</span>',
    ...
    'const rows = [',
    "  { name: '经营日报', owner: '运营部', status: '启用' },",
    "  { name: '风险周报', owner: '风控部', status: '启用' },",
    '];',
    '</script>',
    '',
    '<style scoped>',
    '.business-reports-page { padding: 16px; }',
    '.business-reports-page__header { display: flex; align-items: center; justify-content: space-between; }',
    '</style>',
    '',
  ].join('\n'), 'utf8');
  writeFileSync(join(appRoot, 'vite.config.mjs'), [
    ...
    '    proxy: {',
    "      '/api': {",
    '        target: process.env.MANGO_CUSTOM_PRESET_BACKEND_URL,',
    '        changeOrigin: true,',
  ...
  writeFileSync(join(appRoot, 'package.json'), `${JSON.stringify({
    name: 'admin-custom-preset-e2e-app',
    private: true,
    type: 'module',
    scripts: {
      build: 'vite build',
...
function startCustomPresetDevServer() {
  const child = spawn('pnpm', [
    ...
    env: {
      ...process.env,
      MANGO_CUSTOM_PRESET_BACKEND_URL: args.backendUrl,
      VITE_MANGO_E2E: 'true',
    },
  });
...
function runBrowserCheck() {
  const browserScript = join(evidenceRoot, 'admin-custom-preset-browser-check.mjs');
  writeFileSync(browserScript, createBrowserScript(), 'utf8');
  run(process.execPath, [browserScript], {
    env: {
      ...process.env,
      MANGO_ADMIN_CUSTOM_PRESET_URL: baseUrl,
      MANGO_ADMIN_CUSTOM_PRESET_EVIDENCE: evidenceRoot,
...
const baseUrl = process.env.MANGO_ADMIN_CUSTOM_PRESET_URL;
const evidenceRoot = process.env.MANGO_ADMIN_CUSTOM_PRESET_EVIDENCE;
...
function expectedContentTextForMenu(menu) {
  ...
  const rules = [
    { pattern: 'business/reports', text: '报表名称' },
...
async function sampleMenus(menuTree, extraMenus = []) {
  const samples = [
    ...collectMenuSamples(menuTree),
    ...extraMenus.map((menu) => ({ topMenu: menu, children: [menu] })),
  ];
  const records = [];
  ...
  const report = {
    rule: 'Every visible top-level menu samples 1-3 child pages; custom preset also samples appended business menus.',
...
async function collectReport() {
  ...
  const resolutionReport = await page.evaluate(() => window.__MANGO_ADMIN_CUSTOM_PRESET_RESOLUTION__ || null);
  const mergeReport = await page.evaluate(() => window.__MANGO_ADMIN_CUSTOM_MENU_MERGE_REPORT__ || null);
  const businessMenus = await page.evaluate(() => window.__MANGO_ADMIN_CUSTOM_BUSINESS_MENUS__ || []);
  writeFileSync(join(evidenceRoot, 'resolution-report.json'), \`\${JSON.stringify(resolutionReport, null, 2)}\\n\`, 'utf8');
  ...
  const expectedResolvedCodes = ['auth', 'rbac', 'system', 'template', 'file', 'notice', 'numgen', 'calendar', 'workflow'];
  const checks = {
    usesCustomPresetPackage: await page.evaluate(() => window.__MANGO_ADMIN_CUSTOM_PRESET_E2E__ === 'createMangoAdmin:@mango/admin'),
    hasResolutionReport: Boolean(resolutionReport),
    autoInstalledFoundations: Array.isArray(resolutionReport?.autoInstalledCodes)
      && resolutionReport.autoInstalledCodes.includes('auth')
      && resolutionReport.autoInstalledCodes.includes('rbac'),
    hasExpectedResolvedCapabilities: expectedResolvedCodes.every((code) => resolutionReport?.resolvedCodes?.includes(code)),
    ...
    backendMenusRemainAuthoritative: Array.isArray(mergeReport?.items)
      && mergeReport.items.some((item) => item.source === 'backend' && item.action === 'added'),
    businessMenuAppended: Array.isArray(mergeReport?.items)
      && mergeReport.items.some((item) => item.source === 'business' && item.menuCode === 'business:reports' && item.action === 'added'),
...
const businessMenusForSampling = await page.evaluate(() => window.__MANGO_ADMIN_CUSTOM_BUSINESS_MENUS__ || []);
const menuSamplingReport = await sampleMenus(menuTreeForSampling, businessMenusForSampling);
...
if (failedChecks.length > 0) {
  throw new Error(\`Admin custom preset checks failed: \${failedChecks.join(', ')}. Report: \${JSON.stringify(report)}\`);
}
...
function writeSummary() {
  const summary = [
    '# Mango Admin Custom Preset E2E',
    '',
    `- Base URL: ${baseUrl}`,
    `- Backend URL: ${args.backendUrl}`,
    '- Entry: temporary app importing `createMangoAdmin`, `mangoDefaultCapabilities`, and `resolveMangoAdminPreset` from staged `@mango/admin` file package',
    '- Checks: custom preset dependency report, auto-installed auth/rbac, business menu append, menu merge report, login, backend menu tree, shell layout, top bar, user area, settings entry, tags, theme color, CSS links, screenshots, menu/page sampling, function sampling',
    '- Evidence: home-1440x960.png, user-dropdown-1440x960.png, settings-drawer-1440x960.png, resolution-report.json, menu-merge-report.json, menu-sampling-report.json, function-sampling-report.json, layout-report.json',
    '',
  ].join('\n');
...
function log(message) {
  console.log(`[admin-custom-preset-e2e] ${message}`);
}
