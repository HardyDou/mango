...
Mango 主仓正式发布统一使用 Catalog 驱动的批次入口：

```bash
pnpm -C mango-ui release:plan
pnpm -C mango-ui release:prepare -- --consume-registry=<url>
pnpm -C mango-ui release:status -- --publish-registry=<url> --consume-registry=<url>
pnpm -C mango-ui release:publish -- --authorize --publish-registry=<url> --consume-registry=<url>
