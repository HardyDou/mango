...
const seed = process.env.MANGO_MENU_SAMPLE_SEED || 'sprint-3-menu-sampling';
...
async function main() {
  ...
  for (const topMenu of samplingPlan) {
    ...
    for (const sample of topMenu.samples) {
      ...
      const fileName = `s3-sample-${sanitizeFileName(topMenu.topLevelMenu)}-${sanitizeFileName(sample.menuName)}.png`;
