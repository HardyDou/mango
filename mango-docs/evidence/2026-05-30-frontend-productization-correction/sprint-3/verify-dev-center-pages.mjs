...
async function main() {
  ...
  try {
    ...
    for (const targetPage of targetPages) {
      ...
      const shot = await screenshot(page, `s3-dev-center-${sanitizeFileName(targetPage.name)}.png`);
      state.pages.push({
        screenshot: shot,
  ...
  if (state.pageErrors.length || state.consoleErrors.length || state.failedResponses.length || pageFailures.length) {
