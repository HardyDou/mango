...
> 历史阶段证据：本文只记录 PR-0E 组件登记批次，不代表当前总状态。当前完成定义以采用边界决定和最终候选证据为准；业务灰度、故障注入和生产回滚不属于规范毕业门禁。
...
该批次当时只能判定组件登记 `READY`，不能代表完整规范状态。后续规范状态以采用边界决定和最终候选证据为准；真实 Nexus 属于发布合同，灰度部署和生产回滚属于业务部署/运维合同，均单独管理。
