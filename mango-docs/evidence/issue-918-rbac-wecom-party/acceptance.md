...
| TASK-003 | TC-003  | MySQL / Flyway                    | 历史主体修复与幂等           | Identity V5、Authorization V2，含重复与空 appCode 样本                 | 错误主体 0、重复组 0、最小 ID 保留；重复执行 no-op                                   | 非 UI 项：只读 SQL 回查                                   | 非网络项：MySQL 命令退出码 0                  | `.runtime/issue-918/migration-verification.txt`                                                                                        | PASS |
