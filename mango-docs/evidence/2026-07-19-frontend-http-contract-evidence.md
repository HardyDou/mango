...
- 本阶段仅完成传输契约，不包含 Axios adapter、鉴权/刷新、运行时注入、真实服务联调、Nexus 发布、灰度或回退演练，因此整体仍为 `NOT READY`。
...
上述结果仅表示本次变更没有增加基线债务。生产毕业要求相关基线归零或按最终批准的零债务口径完成迁移，不能依靠抬高基线、跳过检查或 `continue-on-error`。
...
4. 为候选版本生成不可变制品和部署清单，完成 Nexus candidate 回读。
5. 执行可观测灰度与上一稳定版本回退演练，保留指标、触发条件、命令和复验报告。
