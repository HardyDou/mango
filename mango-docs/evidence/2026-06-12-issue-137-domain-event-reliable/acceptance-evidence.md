...
| EVT-003 | 后端 Outbox / Redis Stream 配置 | 可靠投递、失败转终态、补偿回放、Redis Stream transport bean | `DomainEventOutboxAutoConfigurationTest`、`OutboxAutoConfigurationTest`、`RedisStreamDomainEventTransportIntegrationTest` | 16 个相关回归测试全部通过；失败消费进入 `FAILED`，`reconsume` 回到 `PENDING`；不存在 messageId 重新投递返回 `false`；Redis Stream transport 可装配；真实 Redis Stream 重启恢复和多实例消费通过 | 不涉及 UI | Maven 输出 `BUILD SUCCESS` | 命令输出记录于本轮执行结果 | PASS |
...
| EVT-005 | Redis Stream transport | 真实 Redis Stream 端到端投递、服务组广播、同组竞争、异常重启恢复 | 本机真实 Redis `127.0.0.1:6379`；测试 stream `mango:test:domain-event:*`；真实 `RedisKvStore` + `KvOutboxStore` + `OutboxDomainEventPublisher` + `TransportDomainEventDispatcher` | `redis-cli -h 127.0.0.1 -p 6379 ping` 返回 `PONG`；Outbox 发布后 dispatcher 写入真实 Redis Stream 并 ack 本地 Outbox；`payment-service` 与 `notice-service` 两个不同 group 都收到 `workflow.process.completed`；同一 `payment-service` group 两个 consumer 只消费一次；微服务下多实例场景中 payment/notice 各两个实例时每个服务各收到一次且服务内不重复；消费者 ACK 前异常后 pending 留存，关闭旧 Redisson client 模拟实例停止，再用新 Redisson client 模拟重启实例 auto-claim 并 ACK 清空 pending | 不涉及 UI | Redisson 日志显示连接 `localhost/127.0.0.1:6379`；无 mock、无 Mockito；失败消费者日志为 pending/重启恢复用例的预期异常 | Maven 输出 `Tests run: 6, Failures: 0, Errors: 0` | PASS |
| EVT-006 | Flyway migration / 菜单权限默认数据 | 系统事件菜单与权限 SQL 固化 | `V47__system_event_menu.sql` | 已通过 Flyway migration 固化“系统管理 / 系统维护 / 系统事件”菜单、`system:event:list`、`system:event:detail`、`system:event:reconsume` 权限、套餐绑定和 `role_id=1` 管理员角色绑定；组件路径 `@/views/system/event/index.vue` 可被前端运行时规范化为 `system/event/index` 并匹配默认页面注册 | 不涉及 UI | 静态核对 migration 与 `admin-pages` 注册路径一致；没有新增重复 migration | 命令输出记录于本轮执行结果 | PASS |

## 未验证项和风险

- Redis Stream 已连接本机真实 Redis 单实例完成 Outbox 端到端、不同服务 group 广播、微服务下多实例、同 group 多实例竞争、pending auto-claim 和异常重启后继续投递验证；未做 Redis 集群、网络分区和长时间压测。
