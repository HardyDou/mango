...
- 修改密码已进入个人中心子页，原 `/password` 路由继续兼容；顶部主题入口进入个人中心主题设置子页，主题切换即时生效并在验收后恢复默认。
...
| ACC-643-011 | TC-643011 | 顶部用户区、`/#/profile?tab=password`、`/#/profile?tab=theme`、`/#/password`               | 用户摘要、修改密码和页内主题设置                             | admin；当前成员无主组织；主题布局切换后恢复默认                   | 顶栏常驻区只有头像、用户名和箭头；下拉显示“部门未设置｜芒果集团”；个人中心、修改密码、退出三个动作可用；修改密码子页和旧路由均可访问；主题切换即时生效               | 下拉层级紧凑；密码和主题页面位于个人中心右侧；Vue 插槽组件保持 raw，不产生响应式组件警告   | 最终 Errors: 0，Warnings: 0；主题设置期间路由保持在个人中心，恢复后布局为 defaults                                                              | `profile-header-user-dropdown.png`、`profile-password.png`、`profile-theme.png`；`@mango/auth` 配置单测                   | PASS |
...
| admin shell   | 顶部用户区     | 头像、用户名和下拉箭头                 | 部门｜公司摘要与三项功能菜单                       | 常驻区域克制，下拉层级紧凑且组织缺省文案明确            | `profile-header-user-dropdown.png`                                                                                                                                                   | PASS |
