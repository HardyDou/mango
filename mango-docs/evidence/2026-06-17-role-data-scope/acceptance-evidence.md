...
|---|---|---|---|---|---|---|
| `@mango/rbac` | 角色管理 | 使用登录级运行时应用接口加载角色表单上下文 | 数据权限弹窗行内新增、编辑、保存和删除 | 弹窗表格、新增按钮、行内控件、按钮和提示均可见 | `历史验收图片已清理（可从 Git 历史恢复）` | PASS |
