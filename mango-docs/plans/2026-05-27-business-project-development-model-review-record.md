...
| `mango-business-template` 是否作为第一落地资产 | 条件通过 | 在发布物检查和 ecosystem baseline 完成后启动模板仓库 |
...
2. `mango-business-template` 是否单独建仓，还是先放在 Mango 仓库内孵化。
...
| Sprint 3 | 业务模板仓库 | `mango-business-template`、示例业务域、本地启动脚本、业务 PMO 初始结构 | Sprint 2 完成 |
| Sprint 4 | `mango init` | `npm create mango-business@latest`、变量替换、能力选择、环境检查 | Sprint 3 完成 |
