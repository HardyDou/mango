...
| NOTICE-S01-011A | 用户追加要求 | 增加发送任务菜单并完善发送功能 | 发送任务页面主体展示发送列表，记录每次发送后的内容和状态；右上角提供“新增任务”按钮；点击后弹窗按“发送给、消息模板、自定义字段”顺序交互；弹窗按钮为暂存、发送、取消；后端根据消息模板启用渠道和用户联系方式生成发送明细，无对应联系方式的渠道不发送，最低生成系统消息；去掉业务单号、优先级、幂等键、手工联系方式和手工渠道选择 | 发送任务列表页、发送任务弹窗、用户搜索接口关键字查询、统一发送接口按用户资料补全收件人、系统消息兜底、实际发送记录计数回归测试 | `pnpm -F mango-admin build`；`pnpm -F mango-admin test:e2e e2e/specs/notice-site-message.spec.ts --project=chromium`；`mvn -pl mango-platform/mango-notice/mango-notice-core,mango-platform/mango-identity/mango-identity-core -am test -DskipITs` | DONE | `mango-ui/packages/notice/src/views/send-message/index.vue`；`mango-ui/packages/notice/src/api/notice.ts`；`mango-ui/apps/mango-admin/e2e/specs/notice-site-message.spec.ts`；`mango/mango-platform/mango-notice/mango-notice-core/src/main/java/io/mango/notice/core/service/impl/NoticeService.java`；`mango/mango-platform/mango-notice/mango-notice-core/src/test/java/io/mango/notice/core/service/impl/NoticeServiceTest.java` |
| NOTICE-S01-011B | 用户追加要求 | 发送任务选择人时支持部门、岗位、角色、用户 | 发送任务弹窗的“发送给”区域按用户、部门、岗位、角色分组选择；前端提交结构化接收目标；发送登记列表展示任务维度，不展示发送明细，接收对象和参数按 JSON 快照展示；后端通过身份用户 API 将目标解析为当前租户内启用用户并补全联系方式，同一发送任务内按用户去重，避免同一人重复接收；保留 `userId/userIds` 兼容旧调用 | 接收目标 Command、身份用户目标解析 API、任务接收目标 JSON 快照、发送任务弹窗、E2E 请求断言、后端解析与去重测试 | `pnpm -F mango-admin build`；`pnpm -F mango-admin test:e2e e2e/specs/notice-site-message.spec.ts --project=chromium`；`mvn -pl mango-platform/mango-notice/mango-notice-core,mango-platform/mango-identity/mango-identity-core -am test -DskipITs`；`git diff --check` | DONE | `mango-ui/packages/notice/src/views/send-message/index.vue`；`mango-ui/packages/notice/src/api/notice.ts`；`mango-ui/packages/notice/src/types/notice.ts`；`mango/mango-platform/mango-notice/mango-notice-api/src/main/java/io/mango/notice/api/command/NoticeRecipientTargetCommand.java`；`mango/mango-platform/mango-notice/mango-notice-api/src/main/java/io/mango/notice/api/vo/NoticeTaskVO.java`；`mango/mango-platform/mango-notice/mango-notice-core/src/main/resources/db/migration/notice/V7__notice_task_recipient_targets_snapshot.sql`；`mango/mango-platform/mango-notice/mango-notice-core/src/main/java/io/mango/notice/core/service/impl/NoticeService.java`；`mango/mango-platform/mango-identity/mango-identity-api/src/main/java/io/mango/identity/api/query/IdentityUserTargetQuery.java` |
