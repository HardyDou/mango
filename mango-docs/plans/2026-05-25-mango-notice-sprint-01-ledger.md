...
| NOTICE-S01-012 | 用户新版方案 | 通知渠道支持站内信、短信、邮件、微信公众号、企业微信、钉钉、飞书、Webhook | 渠道配置按类型和供应商展示不同表单，底层仍保存 JSON；页面使用传统后台布局并参考 ASCII 线框 | 通知渠道 API、channel forms、传统列表页 | Maven 测试 + 前端构建 | TODO | 待实施 |
...
| NOTICE-S01-017 | 用户新版方案 | 邮件渠道结构化配置 | 供应商不是 SMTP；支持 CUSTOM_SMTP、ALIYUN_DM、TENCENT_SES、SENDCLOUD、MAILGUN、OTHER；SMTP 主机、端口、安全协议、账号、密码、发件人为 CUSTOM_SMTP 配置项 | 邮件配置表单、sender 适配 | sender 测试 + 前端构建 | TODO | 待实施 |
...
| NOTICE-S01-030 | 用户新版方案 | 右上角小铃铛保留用户站内消息入口 | 站内消息是用户入口，不作为后台配置一级菜单；小铃铛保持无背景同风格 | NoticeBell、用户站内消息 API | 组件测试 + E2E | TODO | 待实施 |
