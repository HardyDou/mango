...
| T19 | 文件能力基础模块 | 待处理 | 增加文件上传、下载、预览入口、存储适配、文件记录、逻辑目录、运行时配置、权限边界；详细规划见 [文件能力基础模块计划](./file-capability-module-plan.md) | 后端文件 API 集成测试；文件管理页面 E2E；统一 Upload 组件真实接口联调；权限访问 E2E |
