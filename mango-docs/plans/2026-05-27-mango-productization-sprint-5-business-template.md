...
提供 `mango-business-starter` 孵化模板，覆盖业务项目单仓结构、后端模块分层、前端业务包、菜单权限资源清单、单体和微服务拓扑、业务交付契约模板和模板完整性检查。该资产作为 Mango business starter 的首个落地点，并明确 monolith 与 microservice 两种生成拓扑。
...
- `mango-business-starter`：新增业务项目孵化模板。
...
通过 `node mango-business-starter/scripts/check-template.mjs` 校验模板必需文件、后端分层、`R<T>` 契约、资源清单、前端 page registry、`@mango/common` 请求封装、无 `workspace:*` 依赖语义。
...
- Initializr CLI 和 `npm create mango-app` 由后续 Sprint 交付。
