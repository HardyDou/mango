...
当前阶段只交付“可依赖、可组合、可启动、可验证”的基础链路：

- 默认完整复用 Mango Admin。
- custom 按能力依赖组合。
- local、micro、mixed 只验证基础运行协议和真实页面可访问。
- 初始化项目必须能安装、类型检查、构建、启动、登录、访问真实菜单和真实页面。
- 每阶段必须保留 E2E 截图、布局报告、菜单/样式/颜色/数据识别结果。

当前阶段不做灰度发布、生产级远程配置中心、动态下发 registry、缓存治理、远程模块版本回滚、发布平台、监控告警和性能专项。涉及微前端的 `entry`、`version`、`healthCheckUrl` 只用于基础接入和失败诊断。
...
- 微前端能力当前只要求基础入口、版本展示、健康检查和错误诊断可验证；远程入口缓存、版本兼容治理和回滚策略不进入当前基础稳定主线。
