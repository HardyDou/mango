...
- [ ] 为 `mango-pmo/rules/backend` 每条规则分配规则 ID
- [ ] 记录规则来源文件
- [ ] 记录规则原文
- [ ] 记录规则目标
...
- [ ] 标记自动 / 半自动 / 人工
- [ ] 标记 PMD / Checkstyle / `mango-maven-plugin` / PR checklist
- [ ] 标记误报风险
- [ ] 标记是否进入第一批实现
...
- [ ] 设计 PMD 规则文件结构
- [ ] 设计 `mango-maven-plugin` 检查项
- [ ] 保留 `duplicate` / `method-length` / `class-length` 既有能力
- [ ] 明确严格模式和迁移模式
