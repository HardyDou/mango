...
- 最终风险：L2
- 工作区决策：CREATE 后 REUSE - `D:\Project\mango-data-table`，分支 `codex/mango-data-table`；自动合并前已合入 `origin/main@5eaef3908` 并在原任务 worktree 完成冲突修复。
...
- 处理范围：`@mango/common` 组件、主题变量、测试、包导出、组件合同、README、能力地图、Catalog 投影、Changeset 和本交付记录。
...
| TD-04 | SR-06        | 根入口和组件子路径同时导出，更新 `mangoArchitecture.sourceExports`、组件合同、README、能力地图、Catalog 投影和 Changeset；Changeset 同时升级 Common 与精确依赖 Common 的 Admin | package metadata、Catalog 与文档                   | 回退对应导出、合同、Catalog 和文档变更   |
...
| IM-05 | TD-04    |    5 | `index.ts`、`package.json`、`component-contracts.json`、类型生成脚本、README、能力地图、Catalog、Changeset | 发布面元数据、Catalog、子路径产物与源码事实一致                |
...
| SR-01 至 SR-06                    | M09 受影响范围和样式治理   | `pnpm check:pr -- --base=origin/main --head=HEAD`、Catalog check、`test-quality-check.mjs --base origin/main`、`git diff --check`                                 | PASS    | 最新 `main` 基线上 41 个 workspace 构建、全局 typecheck ratchet、19 个 workspace 测试、Catalog、样式、导出和质量脚本全部通过                                                               |
...
- 用户已明确授权本任务分支提交、Push 及满足门禁后的自动合并；不包含 npm 发布或业务部署。
- Windows 下 Node `spawnSync` 无法直接解析 `corepack.cmd` 与 `mvn.cmd`；本地完整门禁临时调用同一 Corepack JS 入口并使用 Maven 生成的 Effective POM，所有断言均执行通过，临时适配已恢复且不会进入提交。
