...
| R2  | 管理员编辑人工渠道        | 某 Secret 已配置                | 表单和 JSON 两种模式均显示 `****`；JSON 文本保留 Secret 键并提供逐字段小眼睛；未修改直接保存时保留原 Secret | 禁止把 `****` 当作 Secret 提交或覆盖                         | 列表不含明文，JSON 可识别已配置字段，保存后原 Secret 仍可使用 |
...
| D5  | R1-R3    | 前端根据 `configuredSecretKeys`/缺失状态在表单和 JSON 两种模式构造掩码；JSON 文本补齐已配置 Secret 键，独立字段复用逐字段查看组件；不把掩码写入提交，明文只保留在组件内存并定时清理 | `mango-ui/packages/notice`                                | 隐藏查看入口，恢复只写 Secret 交互                       |
...
| I3  | D5       | 3    | `mango-ui/packages/notice/src/api`、`types`、渠道页面              | 表单与 JSON 完整回显、默认掩码、小眼睛查看/隐藏、生命周期清理 |
...
| R1-R6   | 前端生命周期与并发回归测试                                | `pnpm -C mango-ui --filter @mango/notice test -- --run`                                                                                                                                         | PASS               | 12 个测试文件、51 项测试通过；覆盖默认掩码、单字段查看/再次隐藏、JSON 掩码入口、替换时不提交掩码、失败保持掩码、关闭/切换/60 秒超时清理、丢弃切换渠道后的迟到响应                                                                                                                              |
| R1-R3   | 本地真实浏览器走查                                        | 登录 `http://127.0.0.1:7777/#/notice/channel`，编辑企微人工渠道并切换 JSON，逐字段查看后直接保存并再次编辑                                                                                      | PASS               | JSON 文本回显 `secret`、`callbackToken`、`encodingAesKey` 的 `****`；三个独立字段默认掩码；查看只请求当前字段；保存请求不含 `***`/`****` 或重复 Secret 值；再次编辑仍全部掩码，Console 0 error                                                                                                 |
...
- 已使用独立数据库 `mango_issue_825_runtime` 启动真实后端与管理端，后端 Actuator 为 `UP`。已在 Chromium 完成 JSON 掩码、小眼睛单字段查看、保存不提交占位符及保存后再次回显走查；浏览器 Console 0 error。`no-store` 仍由 `NoticeChannelSecretNoStoreFilterTest` 的 HTTP 响应头契约覆盖，本次浏览器网络列表确认查看请求为单字段接口且返回 HTTP 200，但命令行网络视图不展示响应头。
