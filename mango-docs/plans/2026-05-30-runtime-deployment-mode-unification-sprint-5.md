...
- 不改造生产微前端发布系统。
- 不新增后端接口。
- 不做灰度发布、生产级远程配置中心、动态下发 registry、缓存治理或远程模块版本回滚。
- `version`、`entry`、`healthCheckUrl` 只作为基础接入和 E2E 诊断字段，不扩展为发布治理能力。
...
- 本 Sprint 不覆盖生产灰度、缓存失效、远程配置中心、发布平台和远程模块版本回滚策略；这些不属于当前基础稳定主线。
