...
所有带租户支付实体使用 canonical `TenantEntity`、String tenantId 与 Long orgId。支付尚未发布且只使用新数据库，历史 V3-V102 已重整为单一纯 DDL V1；正式必需数据由 payment starter 的 `resources/` 登记，演示数据由 `demo/` 登记且默认关闭，运行态数据不初始化。
...
| TC-006 | AC-005 | 旧链与 V1 结构一致、零 DML、资源分层、Mapper 与双租户 | P0 | 组件 | AUTO | 隔离 `mango_dev_*`、两个 `IT_PAY_` 租户 | schema dump、DDL 扫描、资源计数、索引约束和查询结果 | migration/Resource/Mapper integration tests | 同上 | AUTOMATED |
...
| TASK-004 | 用户批准；SAC-005 | String tenant、V1 与资源初始化边界正确 | TenantEntity + 纯 DDL V1 + 正式/demo Resource + 真实 MySQL | `mango/mango-platform/mango-payment/mango-payment-core`、`mango-payment-starter` | `mango/mango-platform/mango-payment/README.md` | `mango-docs/designs/payment-architecture-debt/technical-design.md` | `mango-ui/apps/mango-admin/e2e/specs/payment-center.spec.ts` | `mango-docs/evidence/baselines/payment-architecture/latest/report.md` | TC-006 | DONE | 同上 |
...
|---|---|---|---|---|---|---|---|---|---|
| TASK-001, TASK-007 | TC-001 | payment 四子模块 Maven suite | before/after 同入口回归 | 用例独立 `TEST_/IT_` 前缀 | 同一 suite 从 268/268 到 281/281，失败、错误、跳过均为 0 | 不涉及页面交互；API 与组件行为由自动化断言验证 | Maven 执行无失败，报告记录各模块计数 | `mango-docs/evidence/baselines/payment-architecture/latest/report.md` | DONE |
| TASK-002, TASK-007 | TC-002 | payment core 金额、状态、幂等与回调 | 核心业务不变量 | 固定金额边界、状态与重复事件 | Money、订单/退款状态、幂等键及 PaymentCode 断言全部通过 | 不涉及页面交互；核心逻辑由单元测试验证 | 定向用例并入 281 条 suite，无未解释错误 | `mango-docs/evidence/baselines/payment-architecture/latest/report.md` | DONE |
| TASK-002, TASK-007 | TC-003 | payment core 通知、对账、差错与结算 | 扩展业务链路不变量 | 唯一批次、明细、凭证与通知记录 | 状态、汇总金额、差异、重试与审计断言全部通过 | 不涉及页面交互；组件行为由自动化测试验证 | 定向用例并入 281 条 suite，无未解释错误 | `mango-docs/evidence/baselines/payment-architecture/latest/report.md` | DONE |
...
| TASK-004, TASK-007 | TC-006 | payment V1、Resource 与 Mapper | 最终结构、String tenant、资源边界和隔离 | 独立库 44 表；正式 65；demo 73 | schema dump 差异 0、V1 DML 0、运行态/密钥 0、默认/demo 双启动通过 | 不涉及页面交互；schema、资源和接口由 SQL/HTTP 证明 | Flyway、资源同步、双租户和接口断言通过 | `mango-docs/evidence/baselines/payment-architecture/latest/report.md` | DONE |
| TASK-005, TASK-007 | TC-007 | payment 定向架构扫描与正式预算 | 历史债务归零且不改写其它模块条目 | payment/common/architecture verification 定向 reactor | payment 1,869→0，dependency/ArchUnit/PMD/blocking 均为 0 | 不涉及页面交互；规则正反例与定向扫描验证 | 按用户要求不执行全仓检查；CI 负责治理文件触发的完整范围 | `mango-docs/evidence/baselines/payment-architecture/latest/report.md` | DONE |
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| BASELINE-001 | TASK-001 至 TASK-007 | TC-001, TC-002, TC-003, TC-004, TC-005, TC-006, TC-007, TC-008 | `mango-ui/apps/mango-admin/e2e/specs/payment-center.spec.ts` | implementation-plan VAL-001 至 VAL-009 | worktree slot 178；版本由报告记录 | `.mango/workspace.json` 独立库；用例独立数据 | `admin` / tenant `1`；凭据不写入报告 | before 268/268、after 281/281，架构 1,869→0；V1/default/demo/API 通过 | TC-008 浏览器执行 EXCEPTION；按用户要求本地不执行全仓检查，由 CI 执行治理变更的完整范围 | `mango-docs/evidence/baselines/payment-architecture/latest/` | 业务不变量 0 非批准变化；接口差异符合批准映射；不声明页面结果通过 |
