...
- 真实双进程单体多实例下两个内嵌 Worker 自动注册和每分钟调度窗口去重。
...
|---|---|---|---|---|---|
| JOB-PRD-001 | 台账 | 交付台账完整 | `JOB-ACC-001` 到 `JOB-ACC-025` 全部 `DONE` | 已通过 | `mango-docs/plans/2026-06-07-mango-native-job-engine-acceptance-ledger.md` |
| JOB-PRD-002 | 台账 | 台账 verify 通过 | `Rows=25`、`DONE=25`、`EXCEPTION=0`、`Incomplete=0` | 已通过 | 2026-06-07 本轮 `delivery-contract-check` 输出：`Rows=25`、`DONE=25`、`EXCEPTION=0`、`Incomplete=0` |
...
| BLOCK-002 | 预发长时间调度稳定性未验证 | 不能开放生产定时任务 | 本地 10 分钟真实调度观察已通过，期间 10 个成功实例、重复窗口 0、失败 0；本地双进程最近 12 个窗口重复窗口 0；预发仍需连续运行 2-4 小时并记录实例数、重复窗口、失败数、积压数和调度延迟分布 |
...
| BLOCK-006 | 旧本地开发库存在历史脏数据或 Flyway checksum mismatch | 不影响当前干净 worktree 主库和独立 Job 库运行，但说明旧环境升级到独立库时需要历史表清理/迁移口径 | 预发使用干净主库验证；如从旧版本升级，追加 DBA 清理脚本或历史数据迁移方案，禁止自动删除业务未知数据 |
...
- 2026-06-07 补做真实双进程单体多实例验证：`18657` 和 `18658` 两个 monolith 同时连接 `mango_dev_a1ce46` 和 `mango_dev_a1ce46_job`，健康检查均为 `UP`；Worker 表存在两个 `IN_MEMORY/EMBEDDED_AUTO/ONLINE` 节点 `embedded-29094`、`embedded-35634`；每分钟示例任务最近 12 个窗口均为 1 条 `SUCCESS` 实例，重复窗口数 0。
