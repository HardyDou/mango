...
- 交付模式：STANDARD
- 需求影响：L2 - 新增公共组件 prop，并改变共享图片预览的默认交互行为
- 方案风险：L2 - 容器布局使用局部 CSS modifier；图片查看器将 Element Plus 模态组件约束为组件内常驻 Viewer，需要验证焦点、键盘、层级和容器缩放
- 最终风险：L2
- 工作区决策：CREATE - `codex/file-preview-fit-container`

## 2. 目标与范围

- 目标：为 `@mango/file` 的 `FilePreviewPanel` 增加类型安全、向后兼容的容器填充模式，并让图片默认直接使用无蒙层的内嵌 Element Plus Image Viewer。
- 成功条件：`fit-container` 模式下根节点、预览区域和各类内容随父容器尺寸变化；默认模式保持自然高度；消费者无需手工组合高度 CSS 变量；图片无需二次点击即可缩放、旋转、切换适应模式和拖拽，且 Viewer 不覆盖预览区之外的内容。
- 处理范围：组件 prop、图片渲染与布局样式、文件管理页真实消费者、组件测试、README、业务集成指南和能力索引。
...
| SR-007 | 图片预览消费者     | 存在可用图片预览 URL        | 直接在 stage 内展示 Element Plus Viewer 和原生工具栏，无遮罩、无二次弹层 | 点击后再覆盖页面，或 Viewer 超出 stage | 默认常驻 Viewer；缩放、旋转、适应模式、原始尺寸和拖拽可用；弹框操作不被阻断 |
...
| TD-005 | SR-004, SR-006, SR-007 | 图片改用非 Teleport 的 `ElImageViewer`，禁用关闭语义，并以 scoped 样式将 wrapper 约束到 stage、隐藏 mask 和 close；不新增图片模式 prop | `FilePreviewPanel.vue`、README | 恢复 `ElImage` 和 `preview-src-list` |
...
| TASK-005 | TD-005                 | 5    | `FilePreviewPanel.vue`、组件测试与浏览器 harness              | 图片默认常驻 Viewer，工具栏交互和容器约束通过验证 |

## 6. 验收映射与结果

| 要求 ID                  | 验证方式       | 命令或步骤                                                                                               | 结果 | 证据                                                                                                                                                                                                                                           |
| ------------------------ | -------------- | -------------------------------------------------------------------------------------------------------- | ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SR-001 至 SR-005、SR-007 | M10 组件测试   | `pnpm --filter @mango/file test`                                                                         | PASS | 6 个测试文件、37 个测试全部通过；覆盖默认模式、fit 模式、操作栏显隐、PDF/Office、图片常驻 Viewer、音视频、空状态和下载占位                                                                                                                     |
...
| SR-002 至 SR-007         | M11/M13 运行时 | Chromium 最小运行时 harness，组合真实 `MangoDialog`、Element Plus Image Viewer 与源码 `FilePreviewPanel` | PASS | 1280×720 视口下弹框从 768×468 拖拽缩放至 888×548，stage/Viewer 同步增大且正文 `scrollHeight === clientHeight`；mask/close 为 `display: none`；原生放大得到 `scale(1.2)`、旋转得到 90° 矩阵、拖拽产生 30×20px 位移；Viewer 外部按钮鼠标点击正常 |
...
- Element Plus Image Viewer 官方定位仍是模态查看器；本次通过组件局部样式约束为内嵌形态。未来升级 Element Plus 时需要回归其 wrapper、mask、close、actions class 与焦点管理行为。
- Element Plus 2.14.1 没有关闭 Image Viewer 内部 FocusTrap 的公开 API；Viewer 常驻时鼠标可以正常操作同一弹框内的其它按钮，但键盘 `Tab` 可能被保留在当前焦点。没有通过依赖私有 API 或 DOM 脚本强行拆除 FocusTrap，该键盘限制作为剩余风险保留。
