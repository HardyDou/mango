...
- 默认完整复用 Mango Admin：full preset 启动后就是原 Mango 主框架和原 Mango 菜单能力，不允许仿写 Shell。
- 按依赖组合能力包：custom preset 通过 `@mango/*-admin` 组合页面、菜单、权限、样式和必需依赖。
- API SDK 和 Admin UI 分包：`@mango/*-api` 可独立给非管理后台 UI 使用，`@mango/*-admin` 负责后台能力集成。
- 初始化器生成可启动项目：生成项目可安装、类型检查、构建、启动、登录和访问真实菜单/页面。
- 部署形态只覆盖基础 local、micro、mixed 协议验证：证明本地模块和微前端模块能接入同一 Mango Admin，不做生产发布治理。
- 每阶段必须保留新特性测试、回归测试、E2E 截图、布局报告、颜色/样式/菜单/数据识别结果。

当前主线不做以下扩展能力：

- 不做灰度发布、A/B、流量分配或按租户发布策略。
- 不做生产级远程配置中心、动态下发能力开关或远程 registry。
- 不做缓存失效、离线缓存、预加载缓存或 CDN 缓存治理。
- 不做远程模块版本回滚、版本协商、发布平台或升级编排。
- 不做性能优化专项、监控告警专项或运维控制台。

以上扩展能力只有在基础链路完成并经过单独需求、设计、台账和验收后才能进入新 Sprint；不得混入当前基础稳定主线。
...
边界：本 Sprint 只验证基础运行时协议和 E2E 可访问性。`entry`、`version`、`healthCheckUrl` 仅作为本地/微前端接入诊断字段，不代表引入生产级远程配置、灰度、缓存失效、版本回滚或发布治理。
...
阶段边界：Sprint 7 在基础稳定主线验收前不启动。启动时只处理 npm 包版本、兼容出口、changelog、迁移说明和升级 E2E，不引入灰度发布、远程配置中心、缓存治理、发布平台或版本回滚编排。
