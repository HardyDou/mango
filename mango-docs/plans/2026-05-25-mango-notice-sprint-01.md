...
- 发送任务提供管理端人工触发入口，页面主体展示发送列表，右上角“新增任务”弹窗内按发送给、消息模板、自定义字段的顺序提交发送。
...
| 发送任务 | 主体展示发送列表；右上角“新增任务”打开弹窗；弹窗先选接收用户，再选消息模板，最后按参数 schema 填自定义字段；实际渠道由消息配置和用户联系方式决定，最低发送系统消息 |
