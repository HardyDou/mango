...
交付台账：`mango-docs/plans/2026-06-05-mango-job-implementation-ledger.md`
...
- `mango-docs/plans/2026-06-05-mango-job-implementation-ledger.md`
...
node mango-pmo/tools/delivery-contract-check.mjs \
  ...
  --ledger mango-docs/plans/2026-06-05-mango-job-implementation-ledger.md \
  --mode verify
```
...
### Sprint 3：PowerJob Adapter 和 Worker 桥接

目标：把 Mango Job 契约映射到 PowerJob，并让 PowerJob Worker 能回调 Mango 原生处理器。
...
- 增加 `MangoPowerJobProcessor`，PowerJob 只调统一 processor，再由 Mango 派发到业务 `MangoJobHandler`。
- 增加 PowerJob Worker 手工装配，避免直接引入 Spring Boot starter 导致所有 Mango 应用默认启动 Worker。
...
说明：`PowerJobEngineAdapter` 单测通过只代表 SDK 请求映射正确，不代表 PowerJob 已完成真实集成。真实集成必须同时具备 PowerJob Server、Worker 注册、真实 `engineJobId`、真实 `engineInstanceId`、实例状态同步、日志可查和 E2E 证据。
...
- 单体部署采用 Mango 管理的 PowerJob Server 子进程、容器或 sidecar，加 Mango 进程内 Worker；不把 PowerJob Server 源码合入 Mango Spring Boot 3 应用上下文。
- 多节点单体部署允许每个 Mango 节点启动 Worker，但 JobCenter 通过外部化的 PowerJob Server 集群或同一组 Server 节点保证调度一致性。
- 执行后端、前端、集成和 E2E 回归。
- 发布 Maven 模块和前端包。
...
- external PowerJob Server + Mango Worker 已实跑通过。
- embedded-single 和 embedded-cluster 已提供配置样例，均要求 PowerJob Server 以官方镜像、发行包、子进程或 sidecar 运行，不把 PowerJob Server 源码合入 Mango Boot 3 应用上下文。
- embedded-cluster 的防重复调度依赖同一个 PowerJob Server 集群或同一组外部化 Server 节点，禁止每个 Mango 节点各自启动互不感知的调度中心。
- 发布 Maven 模块和前端包不纳入本轮人工验收，需在 PR 合并和版本策略确认后执行。
