...
- 方案风险：L2 - 同时调整预览源 URL、内部回环地址、转换结果读取链路和 vendored 引擎的编码判定；新增 token 约束的内部公开读取接口，不改变业务权限、数据或 token 生命周期。
- 最终风险：L2
...
- 处理范围：`mango-file-preview-core` 的源地址生成和转换结果访问校验，`mango-file-preview-engine` 的编码判定与 PDF.js 加载路径，starter 安全属性兼容和受 token 约束的转换结果接口，模块说明、单元测试与真实浏览器联调。
...
| SR-005 | PDF.js | LibreOffice 已生成本地 PDF | 通过同源 `/file-preview/generated` 携带 source token 读取，不经 `/getCorsFile` 二次回源 | token 无效、文件名与 token 的 fileId 不匹配时拒绝；资源不存在时返回 404 | PDF 响应 200、类型为 `application/pdf`，PDF.js 页和 canvas 均非空；不可通过可预测静态路径绕过 token |
...
| TD-005 | SR-005 | 本地转换 PDF 由 PDF.js 同源调用 `/file-preview/generated`；接口为网关可达的 PUBLIC 路由，但必须校验 source token、签发上下文、fileId 与转换文件名，并以 `no-store` 返回 | core、`FilePreviewSourceController`、`pdf.ftl` | 恢复根路径资源 URL 和 `getCorsFile` 代理逻辑 |
...
| IMPL-005 | TD-005/TD-006 | 5 | core、engine、starter 与 Playwright E2E | 转换 PDF 通过 token 约束的同源接口读取，容器路径可读，浏览器真实渲染成功 |
...
|---|---|---|---|---|
| SR-001 | M10 单元测试 | `mvn -f mango/mango-platform/mango-file-preview/mango-file-preview-core/pom.xml -Dtest=io.mango.file.preview.core.service.impl.FilePreviewServiceImplTest test` | PASS（10/10） | `mango-file-preview-core/target/surefire-reports` |
...
| SR-005 | M10 单元测试 | `FilePreviewServiceImplTest`、`WebConfigTest`、`PdfTemplateTest`、`FilePreviewControllerAccessModeTest`、`FilePreviewEngineResourceRegistrarTest` | PASS | token 仅能读取对应 fileId 的转换名；路径穿越拒绝；文件 URI 无 authority；未注册新的根路径资源规则 |
| SR-001/SR-002/SR-004/SR-005 | M11 真实集成 + M13 E2E | Docker 启动 Java 21 + LibreOffice 24.2.7.2；`MANGO_E2E_PREVIEW_CASE=docx-mixed-name MANGO_E2E_OFFICE_ENABLED=true pnpm exec playwright test e2e/specs/file-preview-types-live.spec.ts --project=chromium --workers=1 --repeat-each=2` | PASS（2/2）；PDF 1 页，返回 200 + `application/pdf`，双重编码请求 0 | `mango-ui/apps/mango-admin/e2e/.tmp/file-preview-types-live-results.json`、`file-preview-issue-563-docx.png` |
| 全部 | M09/M10 回归 | `mvn -f mango/pom.xml -pl :mango-file-preview-core,:mango-file-preview-engine,:mango-file-preview-starter -am test` | PASS（core 10；engine 40；starter 11；合计 61） | Maven 输出与各模块 `target/surefire-reports` |
| 全部 | Java 架构与静态质量门禁 | `mvn -f mango/pom.xml -pl :mango-architecture-verification,mango-platform/mango-file-preview/mango-file-preview-core,mango-platform/mango-file-preview/mango-file-preview-engine,mango-platform/mango-file-preview/mango-file-preview-starter -DskipTests -Dmango.architecture.skip=false -Dmango.architecture.mode=changed -Dmango.architecture.requireFullReactor=false -Dmango.architecture.base=1a81ed9cf0e1c2841194cf2f6a8a16111c9e9ecc -Dmango.check.changedOnly=true -Dmango.check.gate=no-new-violations -Dmango.check.baseRef=1a81ed9cf0e1c2841194cf2f6a8a16111c9e9ecc verify` | PASS（dependency 0；ArchUnit 0；PMD 0；静态新增问题 0） | `mango/target/mango-static-report.json` |
| 全部 | 测试质量检查 | `node mango-pmo/tools/test-quality-check.mjs --base origin/main`；`node mango-pmo/tools/audit-backend-test-mocks.mjs --report-only --changed-only --base origin/main` | PASS（8 个变更测试文件；block/warn 0） | 命令输出 |

## 7. 例外与剩余风险

- 源文件仍由预览引擎通过 source token 发起一次内部 HTTP 下载；配置 `source-base-url` 后不会经过公网或外围网关。转换后的本地 PDF 不再通过 URL 代理二次回源。
- 工作区已初始化为 `mango_011`：后端端口 `18011`、前端端口 `30011`、数据库 `mango_dev_mango_issue_563_011`。
