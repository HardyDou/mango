...
| JOB-ACC-010 | 用户要求；设计 6.5 | 后台菜单为 `平台能力 -> 任务管理 -> 任务定义/执行实例/Worker 节点/运行状态/告警规则` | 通过授权模块 migration 入库菜单，前端 package 注册页面 | `V43__native_job_menu_names.sql`；`admin-pages.ts` | Playwright 通过菜单进入各页面并截图 | DONE | `10-job-instance.png`、`10-job-worker.png`、`10-job-engine.png`、`10-job-alarm.png` |
