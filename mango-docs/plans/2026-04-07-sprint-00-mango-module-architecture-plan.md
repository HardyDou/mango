...
**注意：mango 默认使用“部署分组”概念，不把 BFF 作为基础分层。**

更准确的定义是：

- `app` = 部署单元 / 运行时组合层，负责把多个 `*-starter` 装配到同一个进程。
- `BFF` = 可选的前端体验适配层，只在需要为特定前端渠道单独提供接口聚合、交互编排、会话收敛或体验裁剪时独立存在。
- 因此 `mango-admin-app` 属于 `app`，不是 `BFF`；只有未来确实出现独立前端适配进程时，才应该引入 `*-bff`。
...
- `*-bff` = 可选的前端体验适配进程；只有存在独立的前端聚合层时才使用
