...
| 2026-07-15 | Web HTTP 边界与内部调用安全基线：统一 Feign/Web 多值查询签名，原子 nonce 防重放，路径加载 fail-closed；真实 Tomcat 验证异常、Jackson、内部访问与重放拒绝 | [Web README](../../mango/mango-infra/mango-infra-web/README.md) | [Web 历史债务验收证据](../evidence/baselines/infra-web/latest/acceptance.md)；模块/消费测试、随机端口 E2E、架构与静态门禁 |
