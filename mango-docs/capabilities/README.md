...
| 2026-07-14 | GitHub 到内网 Jenkins Maven 发布桥接：GitHub `main` 手工 Workflow 完成秒级元数据检查，内网 Release Runner 通过出站 HTTPS 领取任务并调用 Jenkins；Jenkins 锁定精确 SHA，默认 dry-run，只执行 Maven 非 app 发布批次，结果返回原 GitHub 运行记录 | [CI 桥接 README](../../scripts/ci/README.md)、[GitHub Workflow](../../.github/workflows/maven-release.yml)、[Jenkinsfile](../../jenkins/mango-maven-release.Jenkinsfile) | [发布桥接设计](../designs/github-jenkins-release-bridge/technical-design.md)、[实施计划](../designs/github-jenkins-release-bridge/implementation-plan.md)；Nexus、Jenkins 和发布凭据保持内网 |
