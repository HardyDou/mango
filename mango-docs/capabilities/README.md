...
| 日期 | 能力 | 入口 | 设计与交付 |
|------|------|------|------------|
| 2026-07-17 | File Preview 历史债务最终态：JSON、页面和二进制流适配器分离，预览源文件改用查询参数临时令牌；公共预览请求仍经过安全过滤链，文件 ID、租户恢复、访问校验、kkFileView 页面和业务上传表单行为不变。共享内部调用标记迁入 Web support，Resource sync 补齐独立制品模块元数据，仅修正依赖与装配边界 | [File Preview README](../../mango/mango-platform/mango-file-preview/README.md)、[File README](../../mango/mango-platform/mango-file/README.md) | [File Preview 集成设计](../designs/mango-file集成file-preview方案.md)、[验收证据](../evidence/baselines/file-preview/latest/README.md)；模块/接口/Flow 测试、Fresh MySQL、单体浏览器与双进程真实文件链路 |
