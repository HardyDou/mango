...
| 日期 | 能力 | 入口 | 设计与交付 |
|------|------|------|------------|
| 2026-07-16 | Resource 历史债务最终态：稳定 HTTP Command/Query/VO 保留在 API，进程内 Provider/Handler/Dispatcher 和声明构造能力统一迁入 support；目标服务端独立为 target-core/target-starter，remote 只保留客户端。19 个消费者仅迁移本地 SPI 依赖，资源类型、声明字段、HTTP 路径、权限码和业务功能不变 | [Resource README](../../mango/mango-platform/mango-resource/README.md) | [Resource 验收证据](../evidence/baselines/resource/latest/acceptance.md)；50 条 Resource 测试、19 个消费者编译、Fresh MySQL、真实 API 与 Chromium E2E |
