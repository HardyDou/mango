...
| 2026-07-14 | GitHub Release 到内网 Jenkins 主动发布：公开 Release 附加不可覆盖的机器授权清单；内网 watcher 每两分钟主动发现并校验 tag、SHA、版本和防倒退规则，再调用 Maven 发布 Job；GitHub 始终是 Mango 事实源，watcher 只读取单个 poller 脚本，轮询不再 fetch/checkout 整仓；不依赖内网 Self-hosted Runner | [主动发布 README](../../scripts/ci/README.md)、[GitHub Workflow](../../.github/workflows/maven-release.yml)、[Watcher Jenkinsfile](../../jenkins/mango-github-release-watcher.Jenkinsfile)、[Maven Jenkinsfile](../../jenkins/mango-maven-release.Jenkinsfile) | GitHub 不持有 Jenkins/Nexus 凭据；`claimed`、`failed`、`success` 不自动重跑；Nexus 全坐标回查成功才算发布完成 |
