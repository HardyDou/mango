...
- Mango `1.0.31` 不改变文件上传、回显、预览、下载、ZIP/PDF API、fileId 持久化、权限或租户边界。使用 `1.0.30` 或其它 `1.0.3x` 版本的业务项目应成组升级 Maven、`@mango/file` 及其固定依赖、CLI 和 PMO，并在独立 Bootstrap 成功后复验上传到下载的完整闭环；已有文件记录、对象内容和业务附件关系不得重建或迁移为 URL 字段。
