...
- Issues #824/#826 增加的企业微信资料刷新入口面向当前登录人及其当前绑定，服务端继续校验登录主体、租户、应用和 CorpID，不新增可分配的 `permissionCode`，也不改变角色按钮授权、登录态权限集合或其它接口鉴权。业务按钮显示异常仍按本指南排查；资料刷新失败应检查企业微信权限、成员可见范围和可信 IP，补角色按钮权限不会改变该身份边界。
