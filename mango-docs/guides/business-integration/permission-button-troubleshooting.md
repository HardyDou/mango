...
- Mango `1.0.31` 不改变既有按钮 `permissionCode`、角色授权、登录态权限集合、后端接口鉴权、租户隔离或 `v-auth` 展示语义。使用 `1.0.30` 或其它 `1.0.3x` 版本的业务项目成组升级后，应先确认 Bootstrap 的 `API_RESOURCE`、`AUTH_MENU` 与租户对账回执成功并重新登录刷新权限，再按本指南核对前后端权限码；不得以手工写授权表或关闭鉴权代替升级验证。
