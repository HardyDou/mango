...
该能力只控制前端 UI 展示，不替代后端接口鉴权。真实操作的安全校验仍由后端接口权限链路兜底。

本文是业务接入说明，不作为长期规范源；文档归档边界参考 [文档资产归档边界](../../../mango-pmo/rules/06-document-assets.md)。
...
| `buttonDisplayRule` | 按钮展示规则 | 受限展示表达式，最长 1000 字符，非必填 |
...
- 按钮展示规则只支持 `row`、`pageState`、`query`、`selectedRows` 的属性读取，以及 `!`、比较、`&&`、`||` 和括号。
- 展示表达式不支持函数调用、赋值、数组下标和全局对象访问；解析失败时按钮隐藏。
