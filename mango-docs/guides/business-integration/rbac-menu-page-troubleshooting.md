...
- Issues #824/#826 只统一本人企业微信绑定的昵称、头像快照与 Notice 接收身份来源；个人中心和用户列表中的资料刷新按钮不是框架菜单，不新增或修改菜单 `component` key、菜单树接口、页面注册、角色菜单授权或租户应用绑定。菜单不可见或页面 404 仍按本指南检查资源同步、角色授权和页面 key，不通过补企业微信绑定修复菜单问题。
