...
- PR #414 收口首页工作台小组件权限展示：工作流首页卡片必须同时具备权限码和目标页面入口，否则在卡片内显示“缺少权限”并禁用交互；不改变菜单 `component` key、菜单树接口、页面注册方式、角色授权关系、租户应用绑定、登录态权限聚合、页面路由和本场景排障步骤。若首页卡片缺少页面入口，应按菜单资源同步、角色授权和前端页面 key 注册链路排查，不再通过点击卡片进入 404 页面定位。
