...
- PR #414 仅调整首页工作台可选能力 404 降级和小组件权限展示；不新增租户字典、组织、用户或系统配置基础数据表，不改变租户字典、组织、用户、系统配置公开查询 API、权限、租户隔离、页面入口、页面路由、启动方式和运行时数据行为。首页 link 或 home 能力不可用时显示内联状态或回退内置工作台，不应按租户字典或系统配置基础数据为空处理。
