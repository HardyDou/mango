...
- Issues #613/#614 补齐工作流异步通知中的 `tenantId`、`appCode`、`realm` 和原申请人，只用于按事件来源应用域解析角色、岗位、组织成员及终态接收人；不新增或修改租户字典、组织、用户、系统配置或 Resource 声明数据，也不改变这些能力的公开查询 API、初始化、权限和租户隔离方式。若流程已推进但角色成员收不到通知，应检查事件上下文和角色绑定的应用域，不应通过修改租户基础数据绕行。
