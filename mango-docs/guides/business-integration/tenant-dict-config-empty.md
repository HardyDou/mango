...
- Issue #722 只调整 Admin 前端 Web Crypto UUID 兼容和开发态 MSW/生产构建边界；不新增或修改租户、字典、组织、用户、系统配置及 Resource 声明数据，也不改变公开查询 API、权限、租户隔离和本指南的空数据排查路径。

- Issue #721 只恢复 Admin Shell 个人中心的消息、公告、通知设置和登录日志入口聚合；不新增或修改租户、字典、组织、用户或系统配置数据，也不改变公开查询 API、租户隔离和本指南的空数据排查路径。
...
## 2026-08-02 Issue 643 身份与第三方登录数据影响

- Issue 643 在 Identity 中增加当前用户实名字段，并按租户、应用保存企业微信或钉钉绑定；在 Auth 中按租户、应用保存 Provider 配置。这些数据不属于租户字典、组织或系统配置初始化，不改变本指南的基础数据形成顺序。第三方入口为空时应检查当前租户与 `appCode` 的 Provider 配置是否完整并启用；个人中心资料为空时检查当前登录用户，而不是修改字典或租户 seed。

## 2026-08-03 Mango 1.0.31 发布影响

- Mango `1.0.31` 不改变租户字典、组织、用户或系统配置的公开查询 API、权限、数据模型与租户隔离语义；它把升级所需初始化统一到可验证的 Bootstrap/Resource 链路。使用 `1.0.30` 或其它 `1.0.3x` 版本的业务项目应成组升级完整 release tuple，执行 `bootstrap apply` 与 `bootstrap verify` 并核对稳定回执，不删除已有业务库、不重放历史 migration，也不以手工 SQL 制造“已初始化”状态。
...
- Issue #805 不发布 Maven、不新增 migration，也不改变租户、字典、组织、用户或系统配置的数据模型、初始化顺序和隔离语义；它只修复前端发布包图和全新消费者的页面装配。业务升级后基础数据仍按 Resource、租户上下文和角色绑定链路排查，无需重建数据库、补写 seed 或手工修改菜单数据。

## 2026-08-19 Issues #824/#826 企业微信身份数据影响

- Issues #824/#826 为 Identity 外部绑定增加企业微信昵称、头像文件快照，并让 Notice 按当前租户与 CorpID 读取同一绑定；不新增或修改租户字典、组织、岗位、角色或系统配置初始化，也不会由单账号资料刷新触发通讯录同步。基础数据为空仍按本指南检查 Resource、租户上下文和初始化回执；企业微信资料为空应检查 Provider 配置与厂商权限，不需要补字典 seed 或调整组织架构。
...
## 2026-08-30 Issue #890 当前租户选项影响

- Home 用户候选由登录态推导当前租户，并只返回已启用用户的最小字段；Workflow 推送机构选项同样由可信上下文限定租户。该变更不新增或修改租户、字典、组织、用户或系统配置初始化数据；候选为空时应检查当前租户成员关系、启用状态和领域 Provider，不需要重建基础数据或补写 seed。
