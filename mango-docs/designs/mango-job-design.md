...
- Mango 禁止管理或修改 PowerJob 内部表；日志详情仅允许 `PowerJobEngineAdapter` 以只读方式读取 PowerJob 归档实例日志。
...
- 首轮实现只支持当前 Mango 应用内 Spring Bean 处理器；远程业务模块注册和远程回调执行属于后续 Sprint。
...
后续扩展候选：`REMOTE_API`、`HTTP`、`SCRIPT`、`ENGINE_NATIVE`。这些类型当前不进入后端枚举、API 契约和前端选项，避免配置可选但运行链路不存在。
...
- Mango 后端不管理或修改 PowerJob 内部表；只通过 `PowerJobEngineAdapter` 访问引擎能力，其中执行日志详情可只读读取 PowerJob 归档实例日志。
...
| `/job/logs/page` | GET | 查询日志索引，供执行实例行内日志入口使用 | `job:log:query` |
| `/job/logs/detail` | GET | 查看执行日志详情，供执行实例行内日志入口使用 | `job:log:view` |
...
- Mango 执行实例行内日志入口能查看本次执行的日志索引、摘要或引擎日志。
...
`datasource: job` 表示 PowerJob 内部表与 Job 模块数据库共置。独立部署时可改为 `powerjob`，但仍不代表 Mango 直接管理 PowerJob 内部表。`native-log.datasource` 只用于执行日志详情读取 PowerJob 归档实例日志。
...
| 执行实例 | 查询状态、查看详情、终止运行、重试入口、行内查看执行日志。 |
| Worker | 查看 Worker 在线状态、地址、最近心跳、所属应用。 |
| 引擎状态 | 展示 PowerJob 连接、Server、数据库和 Adapter 状态。 |

说明：业务处理器是任务定义的执行动作字段和 Mango Job 后端契约，不作为 `平台能力 / 任务管理` 下的独立菜单；执行日志是执行实例的行内详情能力，不作为独立菜单。失败、超时、连续失败和恢复通知接入 `mango-notice`，不在 Job 模块单独提供通信平台配置页面。
...
| 执行实例 | `job:instance:list`、`job:instance:sync` |
| 日志 | `job:log:list`，仅作为执行实例行内详情权限，不作为独立菜单 |
| Worker | `job:worker:list` |
