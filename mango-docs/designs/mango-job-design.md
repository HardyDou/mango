...
| `/job/handlers/page` | GET | 查询可用处理器 | `job:handler:query` |
...
- 手动示例任务触发后，Mango 实例 `engineInstanceId=943323584758022208`，状态为 `SUCCESS`；PowerJob `instance_info.status=5`，执行结果为 `Mango Job runtime probe executed`。
...
| 执行器 | 查看 Worker 在线状态、地址、最近心跳、所属应用。 |
| 告警规则 | 配置失败、超时、连续失败、恢复通知规则。 |
| 引擎状态 | 展示 PowerJob 连接、Server、数据库和 Adapter 状态。 |
| 处理器 | 查看业务模块注册的可执行处理器、参数 schema、所属应用和健康状态。 |
