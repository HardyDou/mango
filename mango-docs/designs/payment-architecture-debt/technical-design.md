...
| DEC-008 | 未发布支付模块累积 V3-V102，且团队每次使用全新数据库 | 保留历史链；Flyway baseline 到 102；把历史文件机械合并；生成一个纯 DDL 最终态 V1 | 以旧链在干净 MySQL 的最终 schema 为参照，生成一个可读、可重复执行于空库的 canonical V1，删除 V3-V102；V1 只允许 DDL，禁止任何 INSERT/UPDATE/DELETE | `rules/backend/04-db.md` 明确允许未发布前重整初始化 SQL；用户确认只使用新数据库，并进一步要求按最新规范让 Flyway 只负责 DDL | 用户 2026-07-13 批准；DB-002；PMO 数据库规则 | 否 | payment Flyway history、初始化数据、迁移测试、README、最终服务验证 | 已存在旧 payment Flyway history 的库不能原地升级；结构生成可能遗漏约束 | 仅支持全新数据库；新旧 schema 指纹不一致、V1 出现 DML、Flyway 启动失败或支付接口/页面回归失败即停止并恢复 V3-V102 |
| DEC-009 | 旧 payment migration 把必需配置、演示业务数据、运行态数据和凭据混在一起 | 配置继续留 V1；全部移为测试 SQL；按 Resource Registry 正式/demo 目录与资源类型分开登记 | payment starter 在自身 `META-INF/mango/resources/` 下按 method-category、method、channel、channel-capability、risk-rule 等资源类型拆分 `payment-common-*` 声明并默认登记；在自身 `META-INF/mango/demo/` 下按 tenant、application、subject、bank-account、cashier、contract、contract-capability、route 等拆分 `payment-demo-*` 声明且默认关闭；每类由 payment 类型化 handler 落库。声明不含商户密钥，运行态数据不登记 | 符合最新数据治理规范和模块资产归属；细粒度类型可独立校验字段、依赖、幂等、禁用和版本，不制造不可审查的大 profile | 用户 2026-07-13 明确要求“Flyway 只负责 DDL；演示数据、必须用的数据都分开登记”，并确认 `META-INF/mango/resources/` 也按模块拆分；mango-resource README | 否 | payment core resource handlers、payment starter 正式/demo 声明、启动行为、E2E 数据准备 | handler 或依赖顺序遗漏会导致配置不完整；demo 开关误开会污染正式环境 | 默认启动必须只有 payment 正式资源；显式 demo-enabled 才出现 payment demo 资源；handler 集成测试、全新库实际启动和页面 E2E 任一失败即停止 |
...
| DB-002 | DM-002, DR-002, FR-004 | 所有 payment 表与初始化资源 | V1 直接声明最终列；tenant_id 为 `VARCHAR(64)`，org_id 为可空 `BIGINT`；V1 零 DML | 主键、业务唯一约束、金额精度、空性和默认值与旧链最终态保持 | 索引名、列顺序与唯一性保持 | Entity 统一继承 `TenantEntity`，查询从 `MangoContextHolder.tenantId()` 取得 String | Wrapper、XML 与 core Criteria 使用 String tenantId | 旧 V3-V102 在一次性临时库的最终 schema；正式/演示数据分别来自 payment starter 的类型化 Resource 声明 | 删除 V3-V102，以 `V1__payment_platform.sql` 只创建最终结构；正式必需数据按 payment resource type 登记，演示业务数据按 payment demo resource type 登记且默认关闭；不登记运行态数据和凭据 | 仅支持丢弃未发布的新库重建；旧库如需保留必须继续使用重整前版本，本任务不提供升级兼容 | MANGO-ARCH-ENTITY-003, rules/backend/04-db.md, rules/backend/07-persistence.md | 新旧 information_schema 指纹、V1 DML 扫描、各 Resource handler 幂等/依赖/默认关闭测试、Flyway 新库启动、双租户真实读写和唯一约束测试 |
...
| TC-006 | SAC-005 | DM-002, DB-001, DB-002, SEC-003, FLOW-006 | 空库 V1、旧链最终 schema 等价、V1 零 DML、正式/demo Resource 隔离、Mapper 与双租户读写 | P0 | 数据库集成 | AUTO | 隔离 `mango_dev_*` MySQL；默认与 demo-enabled 两种启动；两个 `IT_PAY_` 租户和相同业务键 | 两租户完全隔离；demo 默认不可见 | information_schema 列/索引/约束指纹一致；V1 仅一条 Flyway history 且所有表初始为空；正式 profile 默认登记；demo profile 仅显式开关登记；无凭据/运行态数据；唯一约束与查询结果正确 | migration/Resource handler/Mapper integration tests + 完整服务启动 | 同上 | 数据库名不匹配安全前缀、结构差异、DML、demo 默认泄露、敏感数据或任一读写差异即停止 | persistence/backend/resource test rules |
...
| IMP-002 | DEC-006, DEC-008, DEC-009, DB-002 | payment 数据库消费者 | V3-V102 历史初始化链与混合 seed | 单一纯 DDL `V1__payment_platform.sql` + payment starter 分类型 `payment-common-*` 正式资源 + 默认关闭的 `payment-demo-*` 演示资源 | 仅支持全新数据库，不提供旧 Flyway history 兼容；demo 必须显式开启 | 未发布环境丢弃数据库后重建；若必须保留旧库则继续使用重整前版本 | README 明确新库前提、V1、分模块 Resource 登记与 demo 开关 | 后续后端发布批次 | TC-006 | 支付负责人 |
...
|---|---|---|---|
| SC-001, SA-001, SA-002, FR-001, FR-002, FR-003, FR-004, FR-005, FR-006, UC-001, UC-002, PG-001, BT-001, DR-001, DR-002, IR-001, NFR-001, NFR-002, NFR-003, SAC-001, SAC-002, SAC-003, SAC-004, SAC-005 | DEC-001, DEC-002, DEC-003, DEC-004, DEC-005, DEC-006, DEC-007, DEC-008, DEC-009, MOD-001, MOD-002, MOD-003, MOD-004, MOD-005, MOD-006, DM-001, DM-002, DM-003, FLOW-001, FLOW-002, FLOW-003, FLOW-004, FLOW-005, FLOW-006, API-001, API-002, API-003, API-004, API-005, DB-001, DB-002, SEC-001, SEC-002, SEC-003, ERR-001, ERR-002, ERR-003, UI-001, IMP-001, IMP-002, IMP-003 | TC-001, TC-002, TC-003, TC-004, TC-005, TC-006, TC-007, TC-008 | 全部系统需求映射到根因迁移、支付业务不变量、接口与安全、租户数据、前端入口和完整架构预算验证 |
