...
系统不按短信、邮件、微信拆业务模型。短信、邮件、系统消息、微信公众号、企业微信、钉钉、飞书、Webhook 都是同一条业务消息的投递渠道。
...
| 范围 | 消息定义、通知渠道、接收设置、发送记录、失败重试、系统监控、用户侧系统消息入口 |
...
- `发送消息` 是管理端人工触发入口，用于运营、客服或管理员按已发布消息定义补发一次业务消息。
- `通知渠道` 管理系统可用的渠道账号、供应商和发送能力。
- `接收设置` 管理用户是否接收某类消息。
- `发送记录` 记录所有发送历史。
- `失败重试` 独立处理失败消息。
- `系统监控` 监控渠道状态、队列堆积和发送统计。
- 系统消息、短信、邮件、微信不是后台一级菜单；它们是渠道类型。
...
| 供应商 | 阿里云短信、腾讯云短信、自定义 SMTP、阿里云邮件推送、企业微信等 |
...
第一版只开放已明确结构化表单和接入边界的供应商；暂未接入的邮件平台不在下拉选项中展示，后续需要接入时再补设计、字段和发送适配。
...
| 区域 | 例如 `cn-hangzhou` |
| Endpoint | 例如 `dm.aliyuncs.com` |
| 发信地址 | 已在阿里云邮件推送中配置的发信地址 |
| 地址类型 | 随机账号或发信地址 |
| 发信别名 | 展示给收件人的发件人名称 |
| 回信地址 | 可选回信邮箱 |
...
系统消息为系统内置通道，不配置第三方账号；在渠道配置中只维护投递运行参数。
...
├── 邮件-阿里云邮件推送：AccessKey、Secret、区域、Endpoint、发信地址、地址类型、发信别名、回信地址
...
标题区：发送消息
  右侧按钮：发送消息
主体区：发送列表
  列表字段：任务ID、消息类型、渠道、状态、发送标题、发送内容、接收人、请求流水号、失败原因、发送时间
弹窗：发送消息
  发送给：远程搜索并选择接收用户
  消息模板：选择消息模板，展示业务域、消息名称、生效版本、启用渠道
  自定义字段：按消息定义参数 schema 动态生成结构化表单
弹窗操作区：暂存、发送、取消
```

交互规则：

- 页面主体是发送列表，用于查看每次点击发送后产生的发送内容和状态。
- 点击右上角“发送消息”打开弹窗，不在页面主体直接展示发送表单。
- 弹窗字段顺序固定为：发送给、消息模板、自定义字段。
- 消息模板只允许选择已启用的消息定义。
- 选择消息模板后，页面根据该消息的参数 schema 生成自定义字段输入项，必填参数必须校验。
- 接收对象只能选择系统用户，支持按用户名、姓名、手机号或邮箱远程搜索。
- 点击“暂存”只保存当前弹窗草稿，不触发发送。
- 页面不提供业务单号、优先级、幂等键、手工联系方式和手工渠道选择。
- 点击发送后调用 `/notice/send`，管理端只提交 `bizType`、`params` 和 `userIds`。
- 后端根据消息模板启用渠道、用户联系方式和通道路由策略决定实际发送明细；用户没有对应联系方式的渠道不发送。
- 最低必须发送系统消息；当消息模板未显式配置系统消息但业务消息存在时，后端生成系统消息兜底。
- 发送成功只表示任务已提交；实际每个渠道、每个接收人的结果在发送记录中查看。
- 发送消息页面不提供“测试发送”概念，不允许绕过消息定义直接拼模板内容。
...
- 发送记录记录发送明细；一次发送请求命中多个渠道或多个接收人时，会产生多条发送记录。
...
2. 明确通知中心定位为统一消息编排中心。
3. 明确系统消息、短信、邮件、微信等是渠道类型，不是一级菜单。
4. 明确消息定义是核心入口。
