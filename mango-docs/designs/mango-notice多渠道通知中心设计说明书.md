...
标题区：发送消息
表单区：
  基础信息：业务消息、业务单号、优先级、幂等键、生效版本、启用渠道
  接收对象：用户ID、接收人、手机号、邮箱、微信OpenId、企微用户ID、钉钉用户ID
  发送渠道：默认按消息定义启用渠道发送；勾选后只发送指定渠道
  业务参数：按消息定义参数 schema 动态生成结构化表单
操作区：重置、发送
```
...
- 业务消息只允许选择已启用的消息定义。
- 选择业务消息后，页面根据该消息的参数 schema 生成参数输入项，必填参数必须校验。
- 接收对象至少填写一个可投递标识，例如用户ID、手机号、邮箱、微信OpenId、企微用户ID或钉钉用户ID。
- 发送渠道不勾选时表示 AUTO，后端按消息定义已启用渠道和路由策略发送；勾选后只发送指定渠道。
- 点击发送后调用 `/notice/send`，提交 `bizType`、`bizId`、`params`、`recipients`、`channelTypes`、`priority` 和 `idempotentKey`。
