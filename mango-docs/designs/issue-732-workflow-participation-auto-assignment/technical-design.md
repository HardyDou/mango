...
upstreamDocumentHash: 50a2d14bbd3ea564505a0d7b5d6f078be531a15f43c3fb8050e480e7e84da985
---
...
| DEC-004 | 设计器候选数据由谁提供 | 前端直调五个模块；给菜单追加跨域权限；Workflow 自有接口+可替换 Provider | `WorkflowDesignerOptionProvider` 提供数据，Workflow 自有接口聚合输出；默认实现调用平台公共 Java API | 前端只依赖 Workflow 权限，承载应用可替换目录实现，租户只来自可信上下文 | FR-008, IR-003, BAC-008 | 否 | 新增 API/VO、默认 Provider、自动配置和前端聚合请求 | Provider 缺失或上游不可用 | 两类情况分别稳定失败；自定义 Bean 通过 `@ConditionalOnMissingBean` 替换默认实现 |
...
| DEC-007 | AUTO 运行时候选用户解析位置 | 继续放运行时服务；Workflow 目录适配器；复用设计器 Provider | 抽出 WorkflowCandidateDirectory，集中运行时跨表读取并 fail-closed | 运行时需要稳定 userId、成员状态和事务语义，不能依赖面向展示的候选 VO | IR-001, FR-005, WorkflowCandidateGroupProvider | 否 | 运行时服务只编排 | 上游表结构耦合 | 用集成契约测试锁定查询；不把设计器展示 Provider 当授权事实 |

## 2. 模块与依赖边界

| 模块设计ID | 模块或包 | 职责 | 改动类型 | 依赖方向 | 公开能力 | 系统需求ID | 适用规范ruleId | 验证方式 |
|---|---|---|---|---|---|---|---|---|
| MOD-001 | mango-workflow-api | 参与查询/声明契约、设计器候选 Provider/VO、Query、Command 和枚举 | 扩展 | api 不依赖 core/starter | WorkflowParticipationApi、WorkflowDesignerOptionProvider | FR-001, FR-002, FR-003, FR-005, FR-008 | rules/backend/03-api.md, rules/backend/05-module.md | M09、M10 |
...
| MOD-003 | mango-workflow-starter 与 starter-remote | REST Controller、权限声明、Feign client、默认设计器 Provider 和装配 | 扩展 | starter/remote -> workflow-api/core；默认 Provider 只调用平台公共 API | 参与查询、声明和设计器候选远程入口 | FR-001, FR-002, FR-003, FR-008 | rules/backend/03-api.md, rules/backend/06-security.md | M09、M10、M12 |
| MOD-004 | mango-ui/packages/workflow | 节点派单模式、配置控件、设计器候选 API 类型和单请求缓存 | 扩展 | UI -> workflow REST；定义设计器不调用 Identity/Authorization/Org/System REST | CLAIM/AUTO 节点配置、五类候选项 | FR-005, FR-008, PG-001, BT-001 | rules/frontend/01-vue-code.md, rules/frontend/04-test.md, rules/frontend/12-business-api.md | M10、M12、M13 |
...
| API-006 | FR-008 | 流程定义管理页面 | MOD-001, MOD-003, MOD-004 | REST/Feign Query | GET /workflow/definitions/designer-options | 无参数 | R<WorkflowDesignerOptionsVO> | 不接受 tenantId；Provider 返回非空聚合对象 | PERMISSION `workflow:definition:query`；Provider 使用当前上下文 | 同一页面会话共享一次 Promise；输出含 users/roles/posts/organizations/dictTypes | DESIGNER_OPTION_PROVIDER_MISSING、DESIGNER_OPTION_LOAD_FAILED | 纯新增；替代设计器五个跨域请求 | rules/backend/03-api.md, rules/backend/06-security.md | M09、M10、M12 |
...
| SEC-005 | FR-008 | 设计器候选加载 | `workflow:definition:query` | 不追加 `system:*`、`authorization:*`、Identity 或 Org 菜单权限 | Controller 权限门禁+Provider 当前上下文 | 请求无 tenantId；默认公共 API 自行执行租户和数据范围 | 前端仅消费 Workflow 输出 VO | 无权限 403；Provider/上游异常为稳定业务错误 | 不记录候选全集和敏感字段 | rules/backend/06-security.md | M09、M10、M12 |
...
| ERR-005 | FR-008 | 设计器 Provider 缺失 | 没有默认或自定义 Provider Bean | DESIGNER_OPTION_PROVIDER_MISSING | 装配异常 | “流程设计器候选数据 Provider 未配置” | requestId、应用和 Provider 类型 | workflow_designer_option_failed_total{reason=provider_missing} | 配置 Provider 后重试 | 不回退前端跨域接口 |
| ERR-006 | FR-008 | 设计器候选加载失败 | 任一平台 API 返回失败或空数据 | DESIGNER_OPTION_LOAD_FAILED | 上游能力异常 | “流程设计器候选数据加载失败” | requestId、失败来源 | workflow_designer_option_failed_total{reason=upstream} | 修复上游后重试 | 不吞错、不伪造空集合 |
...
| UI-002 | PG-001, FR-008 | 用户/角色/岗位/组织/字典候选 | workflow/definition/index、WorkflowParticipantSelector | 五个入口调用同一 `workflowApi.designerOptions()` Promise；子组件只发 ensure 事件 | WorkflowDesignerOptions 的五个集合 | API-006 | 仅流程定义查询权限；不传 tenantId | 统一 loading；失败由请求层明确展示并允许重试 | API 测试+源码禁用跨域 URL+Playwright 网络断言 | 保留本地过滤和组织树交互 | rules/frontend/01-vue-code.md, rules/frontend/04-test.md, rules/frontend/12-business-api.md |
...
| TC-009 | SAC-009 | DEC-004, MOD-001, MOD-003, MOD-004, API-006, SEC-005, ERR-005, ERR-006, UI-002 | 五类映射、Provider 替换、缺失/失败显式错误、单 Workflow 请求和无跨域 URL | P0 | JUnit/Vitest/Playwright | AUTO | 五类候选、缺 Bean、失败 R、仅 workflow 权限 | 当前租户且客户端不传 tenantId | 候选可见、无 403、只请求 designer-options；错误不为空成功 | 定向 Maven/Vitest 和浏览器网络记录 | 测试报告、trace/screenshot | 任一跨域请求、403 或吞错阻断 | rules/backend/08-test.md, rules/frontend/04-test.md |
...
| IMP-004 | DEC-004, API-006, UI-002 | 流程定义管理员和承载应用 | 设计器直接请求五个模块并要求跨域权限 | 只请求 Workflow 候选接口；默认或自定义 Provider 提供数据 | HTTP 接口纯新增；旧跨域请求从定义设计器移除 | 自定义目录场景注册一个 `WorkflowDesignerOptionProvider` Bean | 更新后端/前端 README、能力地图和接入指南 | TC-009 | Workflow owner |
...
| SC-003, SA-003, FR-005, UC-004, PG-001, BT-001, DR-002, NFR-002, NFR-004, SAC-004, SAC-005, SAC-006 | DEC-003, DEC-006, DEC-007, MOD-002, MOD-004, DM-002, DM-003, FLOW-004, API-005, DB-002, SEC-003, ERR-002, ERR-003, UI-001, IMP-002 | TC-003, TC-004, TC-007, TC-008 | 覆盖节点配置、运行时候选目录、严格轮询和空候选回滚 |
...
| SC-005, FR-008, UC-005, IR-003, NFR-005, SAC-009 | DEC-004, MOD-001, MOD-003, MOD-004, API-006, SEC-005, ERR-005, ERR-006, UI-002, IMP-004 | TC-009 | 覆盖设计器候选项 Provider、Workflow 权限、可信租户、显式失败和前端去耦 |
