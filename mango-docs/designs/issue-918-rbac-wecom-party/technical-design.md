...
| DEC-004 | 如何修复历史错误数据                                 | 运维手工 SQL；跨模块脚本；各 owner 模块 Flyway migration                 | Identity V5 与 Authorization V2 分别修复自有表                                | 满足表所有权与标准升级入口；已人工归一化环境幂等 no-op                                                          | FR-003、DR-002                                            | 否       | 两模块 migration 与升级验证          | Authorization 多个部门绑定归一化会触发唯一键冲突                               | 先按目标唯一键删除较大 ID 重复行，再更新保留行                      |
...
| MOD-003    | `mango-identity-core`      | 归一化历史用户主体                   | 新增 V5 数据 migration、README | 只写 `identity_user` owner 表              | 升级行为变化            | FR-003     | M09、M11、M08      | migration 静态检查、真实 MySQL、README 审计  |
...
| DM-002 | mismatched           | 同步或 V5 | consistent               | 数值 tenantId      | Identity update 或 SQL update | 非数值同步失败；migration 只处理数值 tenantId | SAC-002、SAC-003 |
...
| FLOW-003   | FR-003     | Flyway module migration         | MOD-003、MOD-004 | Identity V5 更新数值租户错误主体；Authorization V2 先 delete duplicate 后 update mismatched   | 每模块 Flyway migration 事务能力                 | DM-002、DM-003 -> canonical | 表唯一键                       | 数据库 migration lock    | SQL 失败阻断升级；恢复备份后重试同版本 | 升级完成且错误数据清零 |
...
| ---------- | -------------- | ---------------------------- | ----------------------------- | ------------------------------------------------------------------------------ | -------------- | ------------------------------------------ | ----------------------------- | ---------- | ---------------------------------------------------- | ---------------------------------------------- | -------------- | ----------------------------------------------- |
| DB-001     | DM-002、DR-002 | `identity_user`              | 无 DDL；修复 `party_id`       | 仅 `party_type='INTERNAL_ORG'` 且 `tenant_id` 为正整数字符串                   | 不变           | 使用同记录 tenant_id；update time 由表维护 | Identity owner migration      | 当前表数据 | V5 将 party_id 更新为无符号 tenant_id                | 执行前数据库备份；失败由 Flyway 回滚或恢复备份 | M11            | MySQL 错误/正确/重复执行数据断言                |
