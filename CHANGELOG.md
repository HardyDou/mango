...
- [PR #832](https://github.com/HardyDou/mango/pull/832) Fixed the Release-only required check registry route. Packages: Mango release tooling source only; no standalone coordinate. Business Adaptation: none; business runtime behavior is unchanged.
...
- Keep the protected Release-only frozen lockfile check runnable on GitHub-hosted runners without weakening its blocking semantics.
