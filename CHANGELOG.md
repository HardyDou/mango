...
- Keep architecture-tool build preparation separate from the quality gate: a clean runner explicitly installs the single `mango-common` test dependency before testing the two gate modules, without expanding either build or quality scope through `-am` or `-amd`.
