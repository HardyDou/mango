...
- 单测默认不得新增 mock；除非有明确测试内容和明确断言目标，才允许使用 mock。
- 新增 mock 前必须先明确测试目标：要验证哪个业务规则、状态变化、异常分支、调用边界或回归点。
- 没有明确测试目标、没有对应断言、只是为了让测试容易通过的 mock 禁止新增。
- mock 只能替代当前被测对象的外部协作者，例如 Mapper、远程网关、消息发送器、时钟或不可控第三方客户端。
- mock 不能替代当前被测实现本身，也不能替代本次声称已经完成的核心能力。
- 使用 mock 的测试必须至少断言一项真实行为：状态变化、入参内容、返回结果、异常、持久化调用、外部边界调用或未调用的副作用。
- 如果测试目标是验证某个适配器、算法、状态机、路由器、转换器或支付通道实现本身，必须使用真实实现或专用测试实现，不得用 mock 冒充该实现。
- 交付报告中提到 mock 时，必须说明 mock 替代了哪个外部协作者，以及测试实际证明了什么。
...
- 不写测试直接提交业务改动
- 用 mock 覆盖真实集成问题
- 无明确测试目标、无断言目标地新增 mock
- 用 mock 冒充本次交付的核心能力已经实现
- 发现回归后不补测试
