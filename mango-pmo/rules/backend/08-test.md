...
- 本文件规定按交付模式和真实观察面启用的 M09-M16 如何实施；模式选择和例外以 `rules/11-delivery-assurance.md` 为准。
- 有代码改动时，AI 必须按任务事实推荐适用的 M09-M16；用户确认的措施才进入执行范围。
- 新增业务逻辑或修复缺陷时，AI 应说明可由哪些测试层证明目标及停用风险，不能把固定测试套餐当成默认要求。
- 只写 happy path 不算完成。
...
- 不要求每次都写入口级流程测试或 UI/E2E；只执行有真实观察目标的 M09-M16。后端无浏览器入口时不得仅因 L3 伪造 M13。
...
同一后端能力采用 `api/core/starter/starter-remote` 分层时，还必须按实际职责放置测试：

- `api`：只保存协议、接口、DTO、注解、枚举和其它契约；不得为 accessor、构造器、注解元数据、枚举 `values/valueOf` 等机械契约代码新增测试。发现需要单测的执行逻辑时，优先把逻辑及测试迁移到 `core` 或 `support`，不得用继续在 `api` 增加测试固化错误边界。
- `core`：集中承载业务规则、状态转换、校验、持久化协作及对应单元测试和集成测试，是业务行为测试的主要归属。
- `starter-remote`：只承载远程适配和装配，不放业务规则单测。HTTP、Feign、签名、序列化和远程协议兼容必须在能启动真实远程边界的集成测试或入口级流程测试模块验证，不得用简单委托或反射快照代替。
- `starter`：只保留能证明 Spring 条件装配、HTTP/安全边界、资源初始化或运行时集成结果的少量测试；Bean 数量、接口实现关系、反射字段或 Controller 透传等机械断言不构成有效测试。
- 存量测试按测试目标逐项 `KEEP/MIGRATE/REWRITE/DELETE`，不得仅按目录整批判定有价值或无价值；新增和修改测试必须立即遵守本归属规则。
...
- 说明新增或修改测试观察的规则、状态、失败语义、副作用或集成边界，并说明其模块归属理由；无法指出观察目标的测试不得计入完成证据。

## 15. 禁止事项

- 已确认启用对应测试措施却不执行，仍直接提交业务改动
- 用 mock 覆盖真实集成问题
