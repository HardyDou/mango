...
以下事实出现时，AI 必须推荐 `M08 能力说明`，说明更新价值、成本和停用后的消费者信息漂移风险，并通过原生 Ask User 由用户确认。出现事实不等于自动启用；用户确认 `DISABLE` 时不得由本规则或 CI 恢复为强制更新。

以下变更必须触发 M08 建议与人工确认；只有用户确认 `ENABLE` 后才同步更新能力说明：
...
5. 如果当前能力文档缺少新特性、升级步骤、初始化边界或验证入口，必须推荐 M08 并指出信息漂移风险；用户确认 `ENABLE` 时先补能力说明，确认 `DISABLE` 时记录缺口和剩余风险后按人工决定继续，不得继续套用旧路径。
...
用户确认 M08=`ENABLE` 时，正式交付和 PR 必须说明：
...
## 5.1 M08 启用后的提交前能力文档门禁

M08=`ENABLE` 且正式提交 PR 前必须完成以下检查；M08 未触发或确认为 `DISABLE` 时，CI 只校验人工确认和剩余风险，不运行能力文档覆盖门禁：
