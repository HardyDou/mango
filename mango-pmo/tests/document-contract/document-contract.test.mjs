  ...
  assert.ok(
    result.findings.some(
      (finding) =>
        ...
        finding.message.includes("pmoVersion 必须为 1.4.3"),
    ),
  );
...
test("业务文档集合拒绝当前版本文档使用历史章节变体", (t) => {
  const root = writeHistoricalSectionDocumentSet(t, { pmoVersion: "1.4.3" });
  const pinned = pinHistoricalPmoVersionDocuments(root);
