...
function hydrateLifecycle(levels = {}, pmoVersion = "1.3.15") {
  const brd = readFixture("valid/business-requirements.md")
    .replace("riskLevel: L2", `riskLevel: ${levels.brd ?? "L2"}`)
    .replace("pmoVersion: 1.3.15", `pmoVersion: ${pmoVersion}`);
  ...
  assert.ok(
    result.findings.some(
      (finding) =>
        ...
        finding.message.includes("pmoVersion 必须为 1.3.15"),
    ),
  );
...
test("业务文档集合拒绝当前版本文档使用历史章节变体", (t) => {
  const root = writeHistoricalSectionDocumentSet(t, { pmoVersion: "1.3.15" });
  const pinned = pinHistoricalPmoVersionDocuments(root);
