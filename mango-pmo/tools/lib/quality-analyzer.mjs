...
function analyzeJavaTest(file, rawText, contract, issues) {
  ...
  const apiClaim = /(?:Api|API|Integration|E2E)Test\.java$/.test(file) || /@Tag\s*\(\s*"(?:api|e2e)"/.test(text);
  if (apiClaim) {
...
function analyzePom(file, text, issues) {
  ...
  const artifacts = [...text.matchAll(/<dependency>[\s\S]*?<artifactId>\s*([^<]+)\s*<\/artifactId>[\s\S]*?<\/dependency>/g)]
