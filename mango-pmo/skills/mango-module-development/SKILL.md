...
Run PMO preflight with role `dev`, phase `develop`, and all affected module paths. Generate new code from the selected code baseline, use project code only for business and compatibility facts, and consult references only for unresolved boundaries. Follow automatic M01 CREATE/REUSE policy and the resolved delivery mode.
