...
2. Run `mango release registry doctor`, then use `mango release status/verify/publish/repair --version <version>` as the only batch state owner. Configure stable adapters to invoke existing Maven/npm/docs scripts; do not duplicate their publish logic in the Skill.
