...
1. For each acceptance outcome verify that the selected `STATIC/UNIT/API/UI` type is the lowest-cost type that can observe it, then execute that mapped set and priorities. Do not add all four by default; do not accept a cheaper type that cannot prove the outcome.
