...
2. Read the resolved `L0-L5` baseline, requirement impact, solution risk and final maximum. `L0/L1` use direct acceptance facts, `L2-L4` use the matching lean single document, and `L5` uses its four lifecycle documents.
...
7. Run acceptance-evidence, `check-lean-document.mjs` for the applicable artifact, and targeted validators required by the resolved facts. Record exceptions and residual risk; never fabricate heavyweight evidence.
