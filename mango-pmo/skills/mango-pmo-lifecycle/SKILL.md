...
1. Record requirement impact and solution risk separately from repository facts, then calculate the final L0-L3 level as their maximum. At requirement stages the solution value may remain pending; it must be resolved before editing. Inventory the applicable artifacts, approval state, checker results, unresolved decisions, and trace links.
...
3. Final L0/L1 work follows its approved lightweight task and evidence path without fabricating BRD, SRS, TDD or Plan. Final L2/L3 work must follow the complete BRD -> SRS -> TDD -> Plan chain. If solution analysis raises a lightweight task to L2/L3, route back before implementation.
...
7. Do not declare lifecycle completion until the selected `STATIC/UNIT/API/UI` evidence proves every acceptance outcome, every skipped type has a reason, dedicated checks pass, and all required trace links are closed.
