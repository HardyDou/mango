...
import { getCaptchaTypes, verifyCaptcha, CaptchaType, type CaptchaResponse } from '@/api/admin/captcha';
 ...
 * @returns 验证码配置或null
 */
export function getCaptchaConfig(path: string): CaptchaConfig | null {
  const menuItems = menuLoader.getMenuConfig();
...
export async function captchaRequest<T>(
  ...
  // 1. 检查菜单配置是否需要验证码
  const captchaConfig = getCaptchaConfig(url);
