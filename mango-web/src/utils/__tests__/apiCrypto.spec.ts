...
describe('API Encryption Utils', () => {
  ...
  describe('isBffMode', () => {
    it('应该正确检测 BFF 模式', async () => {
      ...
      const { isBffMode } = await import('@/utils/apiCrypto');
  ...
  describe('isEncryptionEnabled', () => {
    it('应该正确检测加密是否启用', async () => {
      const { isEncryptionEnabled } = await import('@/utils/apiCrypto');
  ...
  describe('wrapRequest', () => {
    it('加密未启用时应直接返回数据', async () => {
      const { wrapRequest } = await import('@/utils/apiCrypto');
  ...
  describe('sm2Encrypt', () => {
    it('加密未启用时应返回原始数据', async () => {
      const { sm2Encrypt } = await import('@/utils/apiCrypto');
      // 默认加密未启用，直接返回原始数据
  ...
  describe('sm2Decrypt', () => {
    it('加密未启用时应返回原始数据', async () => {
      const { sm2Decrypt } = await import('@/utils/apiCrypto');
      // 默认加密未启用，直接返回原始数据
