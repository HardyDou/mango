...
11. 按 `rules/09-test-case-automation-flow.md` 分别评估需求影响和解决方案风险，取较高值形成 `L0-L5`；M09-M16 按真实观察面启用。M13 只覆盖用户可见布局/交互/浏览器结果；无 UI 的高等级任务不伪造 M13。
12. 测试结果基线必须记录测试命令、环境和版本、数据库名或测试数据集标识、账号/租户标识、通过/失败/阻塞/例外列表、证据路径和相对上一基线的行为变化；禁止记录密码、token、密钥。
13. 任务结束必须通过 worktree 交付完整性检查，确认当前任务无未提交、未跟踪、未推送或远端未同步内容；其它脏 worktree 必须逐项说明任务归属和保留原因，不能只检查当前目录。
14. 新增代码必须记录采用的 code baseline ID 和版本；没有匹配模板时记录例外，不得以旧项目代码作为默认生成模板。
15. 触及旧文件时必须清理本次符号范围内可保持行为不变、可由当前验证证明的违规；会改变公共契约、数据、安全、事务、租户或跨模块边界的治理不得顺手扩展。
