...
11. 按 `rules/09-test-case-automation-flow.md` 分别评估需求影响和解决方案风险；AI 只推荐事实触发的 M09-M16 并等待用户确认。M13 只覆盖用户可见布局/交互/浏览器结果；无 UI 的 L3 不伪造 M13。只打开页面、接口 200 或无明显报错不能作为完整断言。
...
- 专业不确定性、高影响决定或治理系统修改自身时，AI 推荐 M14 并说明适合的专家视角；是否启用及采用几个视角由用户确认，风险等级不得自动增加专家数量。
- M14=`ENABLE` 时，专家评审必须输出阻断问题、非阻断建议和结论。
- 存在阻断问题时不得合并；必须把问题登记到 PR 评论或评审结论中。
- 无阻断问题时，合并前必须记录评审视角、验证命令、未验证项和风险。
- PR 只检查用户确认启用的能力说明、验证、复核、回读和人工验收措施证据；未启用措施保留确认值和剩余风险，不要求伪造交付台账字段。
- PR 保留需求影响、解决方案风险及二者最大值，并登记人工确认的 M01-M16 精确值、选择理由和已启用措施证据；CI 不得补加固定验证套餐。
...
- **正向要求**：普通后端 PR 使用 Git 变更映射直接 Maven 模块；质量门禁只扫描这些模块，不使用 `-am` 或 `-amd` 扩大 Reactor。依赖构建、消费者编译和 API 兼容性验证属于独立验证步骤，根据当前依赖和兼容事实推荐并由用户确认，不由风险等级直接选择；`changed`/`no-new-violations` 只阻断本次可归因的新违规，partial-reactor 报告只作为本次门禁证据。`.github/workflows/architecture-debt-inventory.yml` 定时或手工执行完整 Reactor，报告使用 schema v2、`inventoryScope=full-reactor`、`issueInventory=all-detected-issues`，全部问题唯一归属 `moduleKey`。schema v4 预算保存全局和逐模块规则计数与稳定 identity；历史问题修复后只能用同一份完整报告执行全局或 `--module ... --write` 下调，后续不得回升。
