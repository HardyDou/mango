...
- 本文件只规定用户已确认启用的 M09-M16 如何实施，不负责自动启用任何测试措施；措施选择、Ask User 确认和停用后的剩余风险以 `rules/11-delivery-assurance.md` 为准。
...
- 不要求每次都写入口级流程测试或 UI/E2E；只执行用户确认启用的 M09-M16。后端无浏览器入口时不得仅因 L3 伪造 M13；本文件定义启用后的实现方式，不替用户启用措施。
