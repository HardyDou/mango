...
正式测试类型、风险义务、有效测试、证明路径、Mock 边界、基线和例外统一遵循 [`../10-executable-quality-contract.md`](../10-executable-quality-contract.md)。本文件只补充 Java/Spring/MySQL 测试实现细节。
...
- `UNIT`：验证纯业务规则、复杂分支、状态机、校验器、转换规则和异常路径。
- `API`：从真实 HTTP、事件、Consumer、Job 或 CLI 入口验证模块协作、数据库、Mapper、事务、Spring 装配、接口契约、资源注入和权限过滤。
- 后端浏览器前的入口链路归入 `API`；只有从真实浏览器操作开始的完整流程归入 `UI/E2E`。
- 组件或集成只描述执行方式，必须按证明入口归入 `UNIT` 或 `API`，不能作为独立正式测试类型。
...
- 不要求每次都写 `UI/E2E`；是否需要由可执行质量契约根据影响面决定。
