...
当 PMO 合同在同一 schema revision 内仅升级 `pmoVersion` 时，重新执行 `mango pmo upgrade` 会为升级前已有且合同明确支持的版本自动登记路径、SHA-256、`pmoVersion` 和迁移原因；不会改写文档正文、审批证据或上游摘要。新建文档仍必须使用当前合同版本。
