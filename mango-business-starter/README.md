...
`@mango/pmo@1.3.10` 发布 canonical `business-module` code baseline，精确依赖它的 `@mango/cli@1.0.99` 负责项目初始化、PMO 升级和 `mango module add`。模板 manifest 同时定义模块/包结构、`moduleKebab` 等输入与派生变量、Mango Checkstyle 和架构规则源，以及 `XxxCode`、`Require`、typed CRUD、tenant、Mapper、资源、migration、前端导出和测试等可执行规范证据。升级只同步受管 baseline/template/Skill，不会批量重写已有业务模块。
