...
- v2026.07.19 本地候选将业务 API 改为 `createXxxApi(HttpClient)`，host 使用 `@mango/http-client@1.0.0` 注入，页面传递取消信号且不再引用 Axios/全局 request；候选锁为 `@mango/pmo@1.3.2`、`@mango/cli@1.0.83` 和同批前端包。当前未发布，业务项目只能在本地 tarball/candidate 验证后试用。
