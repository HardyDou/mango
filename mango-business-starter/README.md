...
该脚本会先执行 `mvn -f backend/pom.xml -DskipTests install`，再启动 `backend/app`。新增本地业务模块后，不能直接从 `backend/app/pom.xml` 首次执行 `spring-boot:run`，否则 Maven 会到远端仓库解析尚未安装的本地 `{{moduleKebab}}-starter`。
