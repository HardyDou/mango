...
- 文件格式：`V{version}__{description}.sql`
...
| 约束 | 说明 |
|------|------|
| 跨域禁止 | 每个模块 V1 建表，不能依赖另一模块的表 |
| 跨域查询 | Mapper 禁止跨域 SQL，通过 API 调用 |
| 版本递增 | 每次 DDL 变更新增版本号 |
...
mango:
  flyway:
    ...
    modules:
      {module}:
        enabled: true           # 模块级开关
        baseline-on-migrate: true  # 已有数据库引入 Flyway
```
...
mvn flyway:info       # 查看状态
mvn flyway:clean      # 清理（慎用）
```
