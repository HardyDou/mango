...
@DisplayName("防重放签名过滤器测试")
class AntiReplayFilterTest {

    @Test
    @DisplayName("配置了 appKey 密钥时应校验签名并放行")
    void shouldPassWhenConfiguredSecretMatchesSignature() throws Exception {
        String appKey = "demo-app";
        String secret = "demo-secret";
        String timestamp = String.valueOf(System.currentTimeMillis());
        String body = "{\"name\":\"mango\"}";
        SignatureValidator validator = new SignatureValidator();
        String sign = validator.computeSignature("MD5",
                validator.buildSignatureData(appKey, secret, timestamp, body));
        MockHttpServletRequest request = signedRequest(appKey, timestamp, body, sign);
        MockHttpServletResponse response = new MockHttpServletResponse();

        newFilter(Map.of(appKey, secret), null, false).doFilter(request, response, new MockFilterChain());
    ...
    void shouldRejectWhenAppSecretMissing() throws Exception {
        String timestamp = String.valueOf(System.currentTimeMillis());
        MockHttpServletRequest request = signedRequest("missing-app", timestamp, "", "bad-sign");
        MockHttpServletResponse response = new MockHttpServletResponse();

        newFilter(Map.of(), null, false).doFilter(request, response, new MockFilterChain());
    ...
    void shouldUseFallbackSecretOnlyWhenEnabled() throws Exception {
        String appKey = "unknown-app";
        String secret = "fallback-secret";
        String timestamp = String.valueOf(System.currentTimeMillis());
        SignatureValidator validator = new SignatureValidator();
        String sign = validator.computeSignature("MD5",
                validator.buildSignatureData(appKey, secret, timestamp, ""));
        MockHttpServletRequest request = signedRequest(appKey, timestamp, "", sign);
        MockHttpServletResponse response = new MockHttpServletResponse();

        newFilter(Map.of(), secret, true).doFilter(request, response, new MockFilterChain());

        assertEquals(200, response.getStatus());
    }

    private AntiReplayFilter newFilter(Map<String, String> appSecrets, String defaultSecret, boolean allowFallback) {
