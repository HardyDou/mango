...
class MangoJobAlarmNotificationServiceTest {
    ...
    void notifyInstanceFailedShouldPreserveStructuredNoticePayload() {
        ...
        ObjectProvider<NoticeApi> provider = mock(ObjectProvider.class);
        when(provider.getIfAvailable()).thenReturn(noticeApi);
        ...
        MangoJobAlarmNotificationService service = new MangoJobAlarmNotificationService(
                mapper, provider, new ObjectMapper());

        assertThat(service.notifyInstanceFailed(definition, instance, "boom"))
