...
Mango API 资源按访问模式分为三类：

| 类型 | 访问模式 | 是否需要角色/用户授权 | 典型入口 |
|------|----------|------------------------|----------|
| 公开入口 | `PUBLIC` | 不需要登录，也不需要授权 | 登录配置、验证码、健康检查、短期 token 入口 |
| 登录用户基础能力 | `LOGIN` | 不需要给每个角色或用户配置权限；所有已登录用户默认可进入 | 当前用户信息、用户菜单、字典选项、文件详情/预览/下载入口、Realtime 建连、日历计算、个人小组件布局、个人通知 |
| 业务权限接口 | `PERMISSION` | 需要通过菜单、按钮、套餐、角色授权获得权限码 | 业务列表、新增、修改、删除、审批、配置维护 |

判断规则：

- 平台运行体验的一部分，且几乎所有登录用户都需要的接口，优先声明为 `LOGIN`。
- 代表业务动作、管理动作、批量数据读取或配置维护的接口，必须声明为 `PERMISSION` 并配置权限码。
- `LOGIN` 只表示允许进入基础能力入口，不表示允许访问所有数据。
- 文件、消息、租户数据、WebSocket topic、业务对象等仍必须在 service、listener 或数据查询层继续校验租户、归属、数据权限和对象可见性。
- 个人小组件布局只保存当前登录人的布局 JSON，不保存业务小组件数据；业务小组件的数据接口继续按各自业务权限控制。
...
2. `LOGIN` 接口不需要加入菜单资源，也不需要给每个角色、每个用户单独授权。
3. 用户登录后自动具备 `LOGIN` 基础接口访问能力。
4. 普通用户访问 `PERMISSION` 接口仍按角色权限码校验，无权限返回 403。
5. 如果基础接口内部要访问业务对象，接口实现必须继续按当前用户、当前租户和对象归属判断是否可见。
