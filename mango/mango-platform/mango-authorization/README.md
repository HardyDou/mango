...
- 本地应用显式依赖 `mango-auth-starter`、`mango-identity-starter`、`mango-authorization-starter`；远程业务 App 直接依赖 `mango-authorization-resource-sync-starter`，通过 `mango-authorization-starter-remote` 的 Feign 注册到平台 authorization 服务。
