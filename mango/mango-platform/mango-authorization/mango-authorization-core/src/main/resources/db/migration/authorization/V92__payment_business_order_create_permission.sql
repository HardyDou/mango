...
VALUES
  (280602,1,'internal-admin','mango-payment',2806,3,'业务订单新增','payment:business-order:create',NULL,'Plus',NULL,2,1,0,0,0,NULL,'payment:business-order:create',NULL,NULL,NOW(),NOW(),'后台创建待支付业务订单权限',0,NULL,NOW(),NULL,NOW())
...
SET @payment_business_order_create_role_menu_base := (
...
SELECT @payment_business_order_create_role_menu_base := @payment_business_order_create_role_menu_base + 1,
       `role`.`tenant_id`,
       `role`.`id`,
       `menu`.`id`,
       NOW(),
