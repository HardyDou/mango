...
CREATE TABLE IF NOT EXISTS `authorization_public_path` (
    `id` BIGINT NOT NULL COMMENT '主键' PRIMARY KEY,
    `path` VARCHAR(255) NOT NULL COMMENT '路径模式',
    `path_type` TINYINT NOT NULL COMMENT '路径类型: 1-匿名, 2-登录, 3-权限, 4-内部',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='公共路径配置表';
