...
VALUES
  (280802,1,'internal-admin','mango-payment',2808,3,'退款订单主动查退款','payment:refund-order:query-channel',NULL,'Refresh',NULL,2,1,0,0,0,NULL,'payment:refund-order:query-channel',NULL,NULL,NOW(),NOW(),'退款订单主动查询通道退款结果权限',0,NULL,NOW(),NULL,NOW())
...
SET @refund_order_query_channel_role_menu_base := (
...
SELECT @refund_order_query_channel_role_menu_base := @refund_order_query_channel_role_menu_base + 1,
       `role`.`tenant_id`,
       `role`.`id`,
       `menu`.`id`,
       NOW(),
