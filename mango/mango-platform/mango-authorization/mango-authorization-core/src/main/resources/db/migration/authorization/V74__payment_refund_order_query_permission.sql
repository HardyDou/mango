...
VALUES
  (280801,1,'internal-admin','mango-payment',2808,3,'退款订单查询','payment:refund-order:query',NULL,'Search',NULL,1,1,0,0,0,NULL,'payment:refund-order:query',NULL,NULL,NOW(),NOW(),'退款订单详情查询权限',0,NULL,NOW(),NULL,NOW())
...
SET @refund_order_role_menu_base := (
...
SELECT @refund_order_role_menu_base := @refund_order_role_menu_base + 1,
       `role`.`tenant_id`,
       `role`.`id`,
       `menu`.`id`,
       NOW(),
