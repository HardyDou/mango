...
VALUES
  (281704,1,'internal-admin','mango-payment',2817,3,'敏感字段重加密','payment:security:reencrypt-sensitive',NULL,'Lock',NULL,4,1,0,0,0,NULL,'payment:security:reencrypt-sensitive',NULL,NULL,NOW(),NOW(),'支付历史明文敏感字段受控重加密权限',0,NULL,NOW(),NULL,NOW())
...
SET @payment_sensitive_role_menu_base := (
...
SELECT @payment_sensitive_role_menu_base := @payment_sensitive_role_menu_base + 1,
       `role`.`tenant_id`,
       `role`.`id`,
       `menu`.`id`,
       NOW(),
