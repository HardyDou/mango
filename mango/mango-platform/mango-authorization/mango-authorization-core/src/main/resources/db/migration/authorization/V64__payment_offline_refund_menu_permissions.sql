...
VALUES
  (2826,1,'internal-admin','mango-payment',2824,2,'线下退款订单','payment:offline-refund','/payment/offline/refunds','ReceiptText','@/views/payment/offline-refunds/index.vue',2,1,1,0,0,NULL,'payment:offline-refund:list',NULL,NULL,NOW(),NOW(),'线下收款通道独立退款订单、退款账户和退款凭证查询入口',0,NULL,NOW(),NULL,NOW()),
  (282601,1,'internal-admin','mango-payment',2826,3,'线下退款查询','payment:offline-refund:query',NULL,'Search',NULL,1,1,0,0,0,NULL,'payment:offline-refund:query',NULL,NULL,NOW(),NOW(),'线下退款详情查询权限',0,NULL,NOW(),NULL,NOW())
...
SET @payment_offline_refund_role_menu_base := (
...
SELECT @payment_offline_refund_role_menu_base := @payment_offline_refund_role_menu_base + 1,
       `role`.`tenant_id`,
       `role`.`id`,
       `menu`.`id`,
       NOW(),
