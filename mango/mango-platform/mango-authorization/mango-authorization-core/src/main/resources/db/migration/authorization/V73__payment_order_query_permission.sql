...
VALUES
  (280701,1,'internal-admin','mango-payment',2807,3,'支付订单查询','payment:payment-order:query',NULL,'Search',NULL,1,1,0,0,0,NULL,'payment:payment-order:query',NULL,NULL,NOW(),NOW(),'支付订单详情查询权限',0,NULL,NOW(),NULL,NOW())
...
SET @payment_order_role_menu_base := (
...
SELECT @payment_order_role_menu_base := @payment_order_role_menu_base + 1,
       `role`.`tenant_id`,
       `role`.`id`,
       `menu`.`id`,
       NOW(),
