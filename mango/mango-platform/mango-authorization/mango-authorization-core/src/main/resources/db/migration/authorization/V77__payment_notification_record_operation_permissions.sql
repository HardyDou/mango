...
VALUES
  (281101,1,'internal-admin','mango-payment',2811,3,'通知记录查询','payment:notification-record:query',NULL,'Search',NULL,1,1,0,0,0,NULL,'payment:notification-record:query',NULL,NULL,NOW(),NOW(),'通知记录详情查询权限',0,NULL,NOW(),NULL,NOW()),
  (281102,1,'internal-admin','mango-payment',2811,3,'通知记录重推','payment:notification-record:retry',NULL,'Refresh',NULL,2,1,0,0,0,NULL,'payment:notification-record:retry',NULL,NULL,NOW(),NOW(),'通知记录人工重推权限',0,NULL,NOW(),NULL,NOW())
...
SET @payment_notification_record_role_menu_base := (
...
SELECT @payment_notification_record_role_menu_base := @payment_notification_record_role_menu_base + 1,
       `role`.`tenant_id`,
       `role`.`id`,
       `menu`.`id`,
       NOW(),
...
WHERE `menu`.`id` IN (281101, 281102);
