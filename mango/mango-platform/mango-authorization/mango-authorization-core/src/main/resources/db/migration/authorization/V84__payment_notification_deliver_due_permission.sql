...
VALUES
  (281103,1,'internal-admin','mango-payment',2811,3,'通知记录投递到期任务','payment:notification-record:deliver-due',NULL,'RefreshRight',NULL,3,1,0,0,0,NULL,'payment:notification-record:deliver-due',NULL,NULL,NOW(),NOW(),'人工触发投递已到期通知记录权限',0,NULL,NOW(),NULL,NOW())
...
SET @payment_notification_deliver_due_role_menu_base := (
...
SELECT @payment_notification_deliver_due_role_menu_base := @payment_notification_deliver_due_role_menu_base + 1,
       `role`.`tenant_id`,
       `role`.`id`,
       `menu`.`id`,
       NOW(),
