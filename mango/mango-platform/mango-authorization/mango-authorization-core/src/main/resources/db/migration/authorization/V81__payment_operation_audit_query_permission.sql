...
SET `permissions` = 'payment:operation-audit:list',
    `update_time` = NOW(),
    `updated_at` = NOW()
WHERE `id` = 2815;

SET @payment_operation_audit_role_menu_base := (
...
SELECT @payment_operation_audit_role_menu_base := @payment_operation_audit_role_menu_base + 1,
       `role`.`tenant_id`,
       `role`.`id`,
       `menu`.`id`,
       NOW(),
