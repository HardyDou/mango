...
VALUES
  (281104,1,'internal-admin','mango-payment',2811,3,'关闭已过期待支付订单任务','payment:task:expire-open-orders',NULL,'CircleClose',NULL,4,1,0,0,0,NULL,'payment:task:expire-open-orders',NULL,NULL,NOW(),NOW(),'人工或平台调度触发关闭已过期且未成功支付订单权限',0,NULL,NOW(),NULL,NOW()),
  (281105,1,'internal-admin','mango-payment',2811,3,'批量主动查询支付中订单任务','payment:task:query-processing-orders',NULL,'Search',NULL,5,1,0,0,0,NULL,'payment:task:query-processing-orders',NULL,NULL,NOW(),NOW(),'人工或平台调度触发支付中订单主动查单权限',0,NULL,NOW(),NULL,NOW())
...
SET @payment_task_role_menu_base := (
...
SELECT @payment_task_role_menu_base := @payment_task_role_menu_base + 1,
       `role`.`tenant_id`,
       `role`.`id`,
       `menu`.`id`,
       NOW(),
...
WHERE `menu`.`id` IN (281104, 281105);
