...
VALUES
  (280702,1,'internal-admin','mango-payment',2807,3,'支付订单同步状态','payment:payment-order:sync-status',NULL,'Refresh',NULL,2,1,0,0,0,NULL,'payment:payment-order:sync-status',NULL,NULL,NOW(),NOW(),'支付订单主动查单并同步状态权限',0,NULL,NOW(),NULL,NOW())
...
SET @payment_order_sync_role_menu_base := (
...
SELECT @payment_order_sync_role_menu_base := @payment_order_sync_role_menu_base + 1,
       `role`.`tenant_id`,
       `role`.`id`,
       `menu`.`id`,
       NOW(),
