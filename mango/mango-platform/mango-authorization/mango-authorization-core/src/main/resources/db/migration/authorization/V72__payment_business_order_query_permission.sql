...
VALUES
  (280601,1,'internal-admin','mango-payment',2806,3,'业务订单查询','payment:business-order:query',NULL,'Search',NULL,1,1,0,0,0,NULL,'payment:business-order:query',NULL,NULL,NOW(),NOW(),'业务订单详情查询权限',0,NULL,NOW(),NULL,NOW())
...
SET @payment_business_order_role_menu_base := (
...
SELECT @payment_business_order_role_menu_base := @payment_business_order_role_menu_base + 1,
       `role`.`tenant_id`,
       `role`.`id`,
       `menu`.`id`,
       NOW(),
