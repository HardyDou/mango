...
public class NoticeChannelResourceHandler implements ResourceHandler {
    ...
    private static final Set<String> SENSITIVE_KEYS = Set.of("secret", "password", "token", "appsecret",
    ...
    private void apply(NoticeChannelConfigEntity entity, ChannelPayload payload) {
        ...
        entity.setManagedFieldsJson(toJson(List.of("channelType", "providerCode", "configName", "configJson",
                "secretRefs", "enabled", "priority", "weight", "rateLimitConfig", "routeTagCodes")));
        Map<String, String> manualSecrets = stringMap(entity.getSecretConfigJson());
        entity.setSecretStatus(resolveSecretStatus(payload.channelType(), payload.providerCode(), payload.configJson(),
                payload.secretRefs(), manualSecrets));
        entity.setEnabled(payload.enabled());
        entity.setPriority(payload.priority());
        entity.setWeight(payload.weight());
        entity.setConfigStatus(resolveConfigStatus(payload.channelType(), payload.providerCode(), payload.configJson(),
                payload.secretRefs(), manualSecrets));
    ...
    private NoticeChannelConfigEntity resolve(ResourceDeclaration resource) {
        ...
        if (StringUtils.hasText(resourceId)) {
            NoticeChannelConfigEntity entity = channelConfigMapper.selectOne(
        ...
        if (channelType != null && StringUtils.hasText(providerCode)) {
            NoticeChannelConfigEntity entity = findLegacy(tenantId, channelType, providerCode.trim());
            if (entity != null) {
                return entity;
            }
        }
    ...
    private NoticeChannelConfigEntity findLegacy(String tenantId, NoticeChannelType channelType, String providerCode) {
        return channelConfigMapper.selectOne(new LambdaQueryWrapper<NoticeChannelConfigEntity>()
                .eq(NoticeChannelConfigEntity::getTenantId, tenantId)
    ...
    private void replaceRouteTags(NoticeChannelConfigEntity config, List<String> tagCodes) {
        ...
        for (String tagCode : tagCodes) {
            NoticeChannelRouteTagEntity tag = routeTagMapper.selectOne(
                    new LambdaQueryWrapper<NoticeChannelRouteTagEntity>()
                            .eq(NoticeChannelRouteTagEntity::getChannelType, config.getChannelType())
    ...
    private static String validatedConfigJson(Object value) {
        ...
        Map<String, Object> config = objectMap(value, "MESSAGE_CHANNEL configJson must be a JSON object");
        if (containsPlaintextSecret(config)) {
            throw new IllegalStateException("MESSAGE_CHANNEL configJson must not contain plaintext Secret values");
        }
    ...
    private static boolean containsPlaintextSecret(Map<String, Object> config) {
        for (Map.Entry<String, Object> entry : config.entrySet()) {
            ...
            if (SENSITIVE_KEYS.contains(key) || key.endsWith("password") || key.endsWith("token")) {
                return entry.getValue() != null && StringUtils.hasText(String.valueOf(entry.getValue()));
            }
            if (entry.getValue() instanceof Map<?, ?> nested && containsPlaintextSecret(castMap(nested))) {
                return true;
            }
        }
        return false;
    ...
    private static Map<String, String> secretRefs(Object value) {
        Map<String, Object> raw = value == null ? Collections.emptyMap()
                : objectMap(value, "MESSAGE_CHANNEL secretRefs must be a JSON object");
        Map<String, String> refs = new LinkedHashMap<>();
        raw.forEach((key, reference) -> {
            ...
            if (!StringUtils.hasText(key) || !StringUtils.hasText(text)
                ...
                throw new IllegalStateException("MESSAGE_CHANNEL Secret reference must use env: or property:");
            }
            refs.put(key, text);
        });
    ...
    private static NoticeChannelSecretStatus resolveSecretStatus(NoticeChannelType channelType, String providerCode,
                                                                 String configJson, Map<String, String> refs,
                                                                 Map<String, String> manualSecrets) {
        if (channelType == NoticeChannelType.SITE) {
            return NoticeChannelSecretStatus.NOT_REQUIRED;
        }
        Map<String, Object> effective = new LinkedHashMap<>(objectMap(configJson, "Invalid channel config"));
        refs.keySet().forEach(key -> effective.put(key, "referenced"));
        manualSecrets.forEach(effective::putIfAbsent);
        boolean complete = switch (channelType) {
            case EMAIL -> "ALIYUN_DM".equalsIgnoreCase(providerCode)
                    ? hasAny(effective, "accessKeySecret", "accessSecret")
                    : hasAny(effective, "password", "smtpPassword");
            case SMS -> "TENCENT_SMS".equalsIgnoreCase(providerCode)
                    ? hasAny(effective, "secretKey")
                    : hasAny(effective, "accessKeySecret", "accessSecret");
            case WECHAT_OFFICIAL -> hasAny(effective, "appSecret", "secret");
            case WECOM -> hasAny(effective, "secret", "corpSecret", "webhookUrl");
            case DINGTALK -> hasAny(effective, "appSecret", "secret", "webhookUrl");
            case SITE -> true;
        };
        return complete ? NoticeChannelSecretStatus.COMPLETE : NoticeChannelSecretStatus.INCOMPLETE;
    }

    private static NoticeChannelConfigStatus resolveConfigStatus(NoticeChannelType channelType, String providerCode,
                                                                 String configJson, Map<String, String> refs,
                                                                 Map<String, String> manualSecrets) {
        if (channelType == NoticeChannelType.SITE) {
            return NoticeChannelConfigStatus.COMPLETE;
        }
        Map<String, Object> config = new LinkedHashMap<>(objectMap(configJson, "Invalid channel config"));
        ...
        boolean complete = switch (channelType) {
            case EMAIL -> "ALIYUN_DM".equalsIgnoreCase(providerCode)
                    ? hasAny(config, "accessKeyId", "accessKey")
                    && hasAny(config, "accessKeySecret", "accessSecret")
                    && hasAny(config, "accountName", "from", "fromAddress")
                    : hasAny(config, "host", "smtpHost") && hasAny(config, "username", "account")
                    && hasAny(config, "password", "smtpPassword") && hasAny(config, "from", "fromAddress");
            case SMS -> hasAny(config, "accessKeyId", "accessKey", "secretId")
            ...
            case WECOM -> hasAny(config, "corpId") && hasAny(config, "agentId", "webhookUrl")
    ...
    private record ChannelPayload(Long channelConfigId, String resourceId, Integer resourceVersion,
                                  String resourceModuleCode, String tenantId, String configCode,
                                  NoticeChannelType channelType,
                                  String providerCode, String configName, String configJson, Boolean enabled,
                                  Integer priority, Integer weight, NoticeChannelSendHealthStatus lastSendStatus,
                                  String rateLimitConfig, Map<String, String> secretRefs, List<String> routeTagCodes) {
        ...
        private static ChannelPayload from(ResourceDeclaration resource) {
            return new ChannelPayload(
                    ...
                    requiredText(fieldValue(resource, "configCode", true),
                            "MESSAGE_CHANNEL configCode is required").trim(),
                    ...
                    NoticeChannelResourceHandler.secretRefs(fieldValue(resource, "secretRefs", false)),
