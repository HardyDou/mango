...
|------|------|
| `tenant_id` | 当前登录租户 |
| `org_id` | 当前组织上下文（上下文未提供组织时为空） |
| `user_id` | 当前登录用户 |
| `page_code` | 页面编码 |
| `layout_json` | 前端布局 JSON 字符串 |
| `created_by / created_at` | 创建审计信息 |
| `updated_by / updated_at` | 更新审计信息 |

模块不提供默认布局初始化数据。恢复默认布局时物理删除个人配置，由前端业务页面重新使用默认布局。

## 7. 管理入口

本模块不新增后台菜单页面，不新增按钮权限码，也不新增默认套餐或角色授权数据。当前
`mango-admin` 没有直接调用 `gridLayoutPersonalApi` 的内置页面；业务页面需要按“快速开始”显式接入。

| 菜单 / 页面 | component key | 权限码 | 入库来源 | 默认套餐 / 角色 | 后端校验入口 |
|-------------|---------------|--------|----------|-----------------|--------------|
| 无独立管理页面 | 无 | 无新增 | 无 | 无新增 | `GridLayoutPersonalController` / `GridLayoutPersonalService` |

接口使用登录访问模式，不需要给每个角色或用户单独配置权限。租户和用户来自 `MangoContextHolder`，前端不传 `tenantId` 和 `userId`。业务侧的小组件权限、菜单权限和数据权限在调用布局组件前完成过滤，本模块只保存当前登录人的布局结果。

这里的“个人布局”只包括组件位置、尺寸、页面编码和布局 JSON，不保存业务小组件数据。例如“我的待办”“我的流程”“统计卡片”的业务数据仍由各自业务接口提供，并继续走对应业务权限和数据权限。保存布局本身是基础体验，业务小组件是否可见、是否可查询数据，不由 `mango-grid-layout` 决定。
