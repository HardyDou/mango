...
class FileControllerDownloadResponseTest {
    ...
    void downloadResponse_中文文件名_响应头只做一次协议编码() throws Exception {
        ...
        when(fileService.download(any(FileDownloadOptions.class))).thenReturn(new FileDownloadVO(
                new ByteArrayInputStream("ok".getBytes(StandardCharsets.UTF_8)),
                fileName,
                "application/zip",
                2L));
        MockMvc mockMvc = MockMvcBuilders.standaloneSetup(new FileBinaryController(fileService)).build();
