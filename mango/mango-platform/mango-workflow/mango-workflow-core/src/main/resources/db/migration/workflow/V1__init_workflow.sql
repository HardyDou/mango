...
CREATE TABLE IF NOT EXISTS `workflow_definition` (
  ...
  `start_entry_visible` tinyint NOT NULL DEFAULT '1' COMMENT '启动入口是否可见: 0-隐藏 1-可见',
...
CREATE TABLE IF NOT EXISTS `workflow_template` (
  ...
  `latest_flag` tinyint NOT NULL DEFAULT '1' COMMENT '是否当前版本',
...
CREATE TABLE IF NOT EXISTS `workflow_definition_version` (
  ...
  `start_entry_visible` tinyint NOT NULL DEFAULT '1' COMMENT '发布时启动入口是否可见快照: 0-隐藏 1-可见',
...
CREATE TABLE IF NOT EXISTS `workflow_copied_task` (
  ...
  `read_flag` tinyint NOT NULL DEFAULT 0 COMMENT '是否已阅',
...
CREATE TABLE IF NOT EXISTS `workflow_business_apply` (
  ...
  `latest_flag` tinyint NOT NULL DEFAULT 1 COMMENT '是否最新申请',
  `created_by` bigint DEFAULT NULL COMMENT '创建人',
  `created_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_by` bigint DEFAULT NULL COMMENT '更新人',
...
CREATE TABLE IF NOT EXISTS `workflow_business_apply_current_task` (
  ...
  `claim_status` varchar(32) NOT NULL DEFAULT 'ASSIGNED' COMMENT '认领状态: ASSIGNED-已分配 UNCLAIMED-待领取',
  `candidate_users` varchar(1000) DEFAULT NULL COMMENT '候选用户，逗号分隔',
