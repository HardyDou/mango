-- Numgen fresh-install baseline. Flyway owns DDL only; required resources are
-- registered under META-INF/mango/resources and demo data is kept separate.

CREATE TABLE IF NOT EXISTS `numgen_generator` (
  `id` bigint NOT NULL COMMENT '主键',
  `gen_key` varchar(128) NOT NULL COMMENT '业务 Key',
...
CREATE TABLE IF NOT EXISTS `numgen_rule` (
  ...
  `version_state` varchar(16) NOT NULL DEFAULT 'DRAFT' COMMENT '版本状态：DRAFT、ACTIVE、HISTORY',
  `tenant_id` bigint NOT NULL COMMENT '租户 ID',
  `org_id` bigint DEFAULT NULL COMMENT '组织 ID',
  `created_by` bigint DEFAULT NULL COMMENT '创建人 ID',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  ...
  KEY `idx_numgen_rule_tenant_key_state` (`tenant_id`, `gen_key`, `version_state`)
...
CREATE TABLE IF NOT EXISTS `numgen_rule_segment` (
  ...
  KEY `idx_numgen_rule_segment_tenant_rule` (`tenant_id`, `rule_id`)
...
CREATE TABLE IF NOT EXISTS `numgen_sequence` (
  `id` bigint NOT NULL COMMENT '主键',
  `gen_key` varchar(128) NOT NULL COMMENT '业务 Key',
  `rule_version` int NOT NULL DEFAULT '1' COMMENT '最近分配时的规则版本',
...
CREATE TABLE IF NOT EXISTS `numgen_history` (
  ...
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_numgen_history_tenant_key_time` (`tenant_id`, `gen_key`, `created_at`),
