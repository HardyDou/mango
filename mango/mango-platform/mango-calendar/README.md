...
工作日计算和农历查询属于登录用户基础能力：

| 场景 | 访问模式 | 是否需要为角色/用户单独配置 |
|------|----------|------------------------------|
| `/calendar/workdays/**`、`/calendar/lunar/**` | `LOGIN` | 不需要。业务表单、SLA、账期、审批截止日等页面登录后可直接调用。 |
| `/calendar/admin/**` 日历定义、年度初始化、日期维护 | `PERMISSION` | 需要。只给日历管理员或平台管理员。 |

`LOGIN` 只表示允许读取和计算已维护的日历数据，不表示可以维护日历定义或修改节假日。业务页面需要计算工作日时，不需要给每个角色、每个用户配置 `calendar:*` 权限；管理端初始化年度、导入日期、修改工作日属性仍按菜单和按钮权限授权。
...
构建期 cold baseline 使用 `calendarCode` 生成日历定义的稳定主键，并使用 `calendarCode + date` 生成日期明细的稳定主键。相同正式 Resource 在不同空库回放时会得到相同 ID；已有日历仍按业务键幂等更新，`INIT_ONLY` 保护语义不变。业务项目不需要预先分配数据库自增 ID，也不得通过调整模块执行顺序保证日历数据一致。
