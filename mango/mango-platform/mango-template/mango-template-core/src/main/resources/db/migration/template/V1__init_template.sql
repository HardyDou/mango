-- Template module canonical schema for new databases. DDL only.
...
CREATE TABLE IF NOT EXISTS `template` (
  ...
  `business_group` varchar(64) DEFAULT NULL COMMENT '兼容业务组编码',
  `business_type` varchar(64) DEFAULT NULL COMMENT '兼容业务类型',
  `business_key` varchar(128) DEFAULT NULL COMMENT '兼容业务Key',
  `source_format` varchar(32) DEFAULT NULL COMMENT '当前发布内容稿源格式',
  `status` tinyint NOT NULL DEFAULT '1' COMMENT '状态: 0-停用 1-启用',
  `current_version_no` int NOT NULL DEFAULT '0' COMMENT '当前发布版本号',
  `draft_source_format` varchar(32) DEFAULT NULL COMMENT '未发布草稿源格式',
  `draft_content` longtext DEFAULT NULL COMMENT '未发布草稿文本或HTML内容',
  `draft_source_file_id` bigint DEFAULT NULL COMMENT '未发布草稿文档源文件ID',
  `draft_variable_schema` json DEFAULT NULL COMMENT '未发布草稿变量定义',
  `has_unpublished_changes` tinyint NOT NULL DEFAULT '0' COMMENT '是否存在未发布变更',
  ...
  KEY `idx_template_domain` (`tenant_id`,`domain_code`),
  KEY `idx_template_business_scope` (`tenant_id`,`business_group`,`business_type`,`business_key`),
...
CREATE TABLE IF NOT EXISTS `template_version` (
  ...
  `source_format` varchar(32) NOT NULL COMMENT '内容稿源格式',
