...
public class PaymentChannelSynchronizer {
    ...
    public PaymentSyncResult syncPaymentStatus(String payOrderNo) {
        ...
        Require.isTrue(PaymentOrderStatusEnum.PAYING.getCode().equals(currentStatus),
                PaymentCode.PAYMENT_ORDER_STATE_INVALID, "只有支付中的订单允许主动查单推进");
    ...
    private PaymentSyncResult applyPaymentChannelResult(
        ...
        if (updated == -1) {
            PaymentDuplicatePaymentGuard.DuplicatePaymentResult duplicateResult = duplicatePaymentService.handleDuplicateSuccess(
                    ...
                    order,
                    queryTime,
                    PaymentOrderStatusFlowRecorder.SOURCE_CHANNEL_QUERY,
                    order.getPayOrderNo(),
        ...
        Require.isTrue(updated == 1, PaymentCode.PAYMENT_ORDER_STATE_INVALID, "支付订单状态已变化，请刷新后重试");
    ...
    public RefundSyncResult syncRefundStatus(String refundOrderNo) {
        Require.notBlank(refundOrderNo, PaymentCode.PAYMENT_REFUND_ORDER_INVALID, "退款订单号不能为空");
        String tenantId = PaymentContextSupport.currentTenantId();
    ...
    public RefundSyncResult syncRefundStatusByChannelRefundNo(String channelRefundNo) {
        Require.notBlank(channelRefundNo, PaymentCode.PAYMENT_REFUND_ORDER_INVALID, "通道退款单号不能为空");
        String tenantId = PaymentContextSupport.currentTenantId();
        PaymentRefundOrderVO refundOrder = toApi(refundOrderMapper.selectByTenantAndChannelRefundNo(
                tenantId, channelRefundNo), PaymentRefundOrderVO.class);
        Require.notNull(refundOrder, PaymentCode.PAYMENT_REFUND_ORDER_NOT_FOUND);
        return syncRefundStatus(tenantId, refundOrder);
    }

    private RefundSyncResult syncRefundStatus(String tenantId, PaymentRefundOrderVO refundOrder) {
        long startedAt = System.nanoTime();
        ...
        Require.isTrue(PaymentRefundOrderStatusEnum.REFUNDING.getCode().equals(currentStatus),
                PaymentCode.PAYMENT_REFUND_ORDER_STATE_INVALID, "只有退款中的订单允许主动查询推进");
    ...
    private IPaymentChannelAdapter.PaymentQueryResult queryTargetStatus(String tenantId, PaymentOrderEntity order) {
        return channelAdapterRegistry.requireAdapter(order.getChannelCode())
                .queryPayment(new IPaymentChannelAdapter.PaymentQueryInput(tenantId, order));
    }

    private IPaymentChannelAdapter.RefundQueryResult queryTargetStatus(String tenantId, PaymentRefundOrderVO refundOrder) {
        return channelAdapterRegistry.requireAdapter(refundOrder.getChannelCode())
                .queryRefund(new IPaymentChannelAdapter.RefundQueryInput(tenantId, refundOrder));
    }

    private void notifyRefundTerminal(String tenantId, PaymentRefundOrderVO refundOrder) {
        PaymentApplicationEntity application = applicationMapper.selectOne(new LambdaQueryWrapper<PaymentApplicationEntity>()
                .eq(PaymentApplicationEntity::getTenantId, tenantId)
                .eq(PaymentApplicationEntity::getAppId, refundOrder.getAppId()));
        Require.notNull(application, PaymentCode.PAYMENT_APPLICATION_NOT_FOUND);
        PaymentBusinessOrderEntity businessOrder = businessOrderMapper.selectCashierBusinessOrder(tenantId, refundOrder.getBusinessOrderId());
        Require.notNull(businessOrder, PaymentCode.PAYMENT_BUSINESS_ORDER_NOT_FOUND);
    ...
    private int updatePayingQueryResult(String tenantId, PaymentOrderEntity order, String targetStatus, LocalDateTime payTime) {
        try {
    ...
    private String selectPaymentFlowNo(String tenantId, Long paymentOrderId) {
    ...
    private QueryRecordSummary recordQuery(
        ...
        record.setQueryNo(numberService.next(PaymentNumberGenerator.PAY_QUERY_NO));
    ...
    private QueryRecordSummary recordRefundQuery(
        ...
        record.setQueryNo(numberService.next(PaymentNumberGenerator.PAY_REFUND_QUERY_NO));
    ...
    private String writeSummary(Map<String, String> values) {
        try {
            ...
            return Require.fail(PaymentCode.PAYMENT_ORDER_STATE_INVALID, "查单记录摘要生成失败", ex);
        }
    }
