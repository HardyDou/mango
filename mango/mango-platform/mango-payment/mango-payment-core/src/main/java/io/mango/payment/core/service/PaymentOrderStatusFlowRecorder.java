...
public class PaymentOrderStatusFlowRecorder {
    ...
    public void record(
            ...
            String remark) {
        Require.notNull(tenantId, PaymentCode.PAYMENT_READONLY_RESOURCE_INVALID, "租户 ID 不能为空");
        Require.notBlank(orderType, PaymentCode.PAYMENT_READONLY_RESOURCE_INVALID, "订单类型不能为空");
        Require.notNull(orderId, PaymentCode.PAYMENT_READONLY_RESOURCE_INVALID, "订单 ID 不能为空");
        Require.notBlank(orderNo, PaymentCode.PAYMENT_READONLY_RESOURCE_INVALID, "订单号不能为空");
        Require.notBlank(toStatus, PaymentCode.PAYMENT_READONLY_RESOURCE_INVALID, "目标状态不能为空");
        Require.notBlank(triggerSource, PaymentCode.PAYMENT_READONLY_RESOURCE_INVALID, "触发来源不能为空");
    ...
    public List<PaymentOrderStatusFlowVO> list(String tenantId, String orderType, Long orderId) {
        Require.notNull(tenantId, PaymentCode.PAYMENT_READONLY_RESOURCE_INVALID, "租户 ID 不能为空");
        Require.notBlank(orderType, PaymentCode.PAYMENT_READONLY_RESOURCE_INVALID, "订单类型不能为空");
        Require.notNull(orderId, PaymentCode.PAYMENT_READONLY_RESOURCE_INVALID, "订单 ID 不能为空");
        return toApiList(statusFlowMapper.selectStatusFlows(
                tenantId, orderType, orderId), PaymentOrderStatusFlowVO.class);
    }
}
