...
public class PaymentNumberGenerator {
    ...
    public String next(String genKey) {
        Require.notBlank(genKey, PaymentCode.PAYMENT_READONLY_RESOURCE_INVALID, "支付编号规则键不能为空");
        NumgenNextCommand command = new NumgenNextCommand();
        command.setGenKey(genKey);
        command.setParams(Map.of("bizKey", genKey));
        return PaymentRemoteResultSupport.requireData(
                numgenApi.nextValue(command),
                PaymentCode.PAYMENT_READONLY_RESOURCE_INVALID,
                "支付编号生成失败");
    }
}
