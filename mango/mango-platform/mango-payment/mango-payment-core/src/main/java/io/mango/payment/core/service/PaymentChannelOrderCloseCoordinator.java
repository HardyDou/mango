...
public class PaymentChannelOrderCloseCoordinator {
    ...
    private CloseResult closePayment(String payOrderNo, boolean expiredClose) {
        ...
        Require.isTrue(isClosablePayment(currentStatus),
                PaymentCode.PAYMENT_ORDER_STATE_INVALID, "只有未支付或支付中的订单允许关单");
        Require.isTrue(!Integer.valueOf(1).equals(paymentOrder.getSuccessFlag()),
                PaymentCode.PAYMENT_ORDER_STATE_INVALID, "支付成功订单不允许关单");
        ...
        Require.isTrue(isClosableBusiness(businessOrder.getStatus()),
                PaymentCode.PAYMENT_BUSINESS_ORDER_STATE_CHANGED, "业务订单当前状态不允许关单");
        Require.isTrue((businessOrder.getPaidAmount() == null ? 0L : businessOrder.getPaidAmount()) == 0L,
                PaymentCode.PAYMENT_BUSINESS_ORDER_STATE_CHANGED, "业务订单已有支付金额，不允许关单");
        ...
        Require.isTrue(paymentUpdated == 1, PaymentCode.PAYMENT_ORDER_STATE_INVALID, "支付订单状态已变化，请刷新后重试");
        int businessUpdated = businessOrderMapper.closeOpenBusinessOrder(tenantId, businessOrder.getId());
        Require.isTrue(businessUpdated == 1, PaymentCode.PAYMENT_BUSINESS_ORDER_STATE_CHANGED, "业务订单状态已变化，请刷新后重试");
        statusFlowService.record(
                ...
                PaymentOrderStatusFlowRecorder.SOURCE_CHANNEL_CLOSE,
                payOrderNo,
                null,
        ...
        if (expiredClose) {
            exceptionOrderRecordService.createIfAbsent(
                    tenantId,
                    payOrderNo,
                    PaymentExceptionOrderRecorder.TYPE_PAY_TIMEOUT,
                    PaymentExceptionOrderRecorder.SEVERITY_MEDIUM,
    ...
    private void notifyPaymentClosed(String tenantId, Long paymentOrderId, PaymentBusinessOrderEntity businessOrder) {
        ...
        PaymentOrderVO closedOrder = toApi(
