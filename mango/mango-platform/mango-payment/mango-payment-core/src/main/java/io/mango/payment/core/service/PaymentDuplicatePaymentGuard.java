...
public class PaymentDuplicatePaymentGuard {
    ...
    public DuplicatePaymentResult handleDuplicateSuccess(
        ...
        Require.notNull(tenantId, PaymentCode.PAYMENT_ORDER_STATE_INVALID, "租户 ID 不能为空");
        Require.notNull(order, PaymentCode.PAYMENT_ORDER_NOT_FOUND);
        Require.notNull(order.getId(), PaymentCode.PAYMENT_ORDER_NOT_FOUND, "支付订单 ID 不能为空");
        Require.notBlank(order.getPayOrderNo(), PaymentCode.PAYMENT_ORDER_NOT_FOUND, "支付订单号不能为空");
        Require.notNull(order.getBusinessOrderId(), PaymentCode.PAYMENT_BUSINESS_ORDER_NOT_FOUND, "业务订单 ID 不能为空");
        Require.notNull(eventTime, PaymentCode.PAYMENT_ORDER_STATE_INVALID, "支付事件时间不能为空");
        Require.notBlank(triggerSource, PaymentCode.PAYMENT_ORDER_STATE_INVALID, "重复支付触发来源不能为空");
        ...
        Require.isTrue(successUpdated == 1, PaymentCode.PAYMENT_ORDER_STATE_INVALID, "重复支付订单状态已变化，请刷新后重试");
        statusFlowService.record(
                ...
                eventTime,
                "重复成功支付先记录为非有效成功支付");
        insertFlow(numberService.next(PaymentNumberGenerator.PAY_FLOW_NO), order, null, "PAY_SUCCESS", eventTime, tenantId);
        ...
        if (!MANGO_PAY_CHANNEL_CODE.equalsIgnoreCase(PaymentContextSupport.trimToNull(order.getChannelCode()))) {
            String exceptionNo = exceptionOrderRecordService.createIfAbsent(
                    tenantId,
                    order.getPayOrderNo(),
                    PaymentExceptionOrderRecorder.TYPE_DUPLICATE_PAYMENT,
        ...
        Require.isTrue(refundingUpdated == 1, PaymentCode.PAYMENT_ORDER_STATE_INVALID, "重复支付订单退款状态已变化，请刷新后重试");
        statusFlowService.record(
                tenantId,
                PaymentOrderStatusFlowRecorder.ORDER_TYPE_PAYMENT,
                order.getId(),
    ...
    private PaymentRefundOrderEntity createDuplicateRefundOrder(
        ...
        refundOrder.setRefundOrderNo(numberService.next(PaymentNumberGenerator.PAY_REFUND_ORDER_NO));
