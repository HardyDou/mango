...
public class PaymentFuiouPayConfigParser {
    ...
    public PaymentFuiouPayConfig parse(String configValuesJson) {
        Require.notBlank(configValuesJson, PaymentCode.PAYMENT_CHANNEL_CONTRACT_INVALID, "富友签约配置不能为空");
    ...
    private Map<String, Object> parseJson(String configValuesJson) {
        try {
            ...
            return Require.fail(PaymentCode.PAYMENT_CHANNEL_CONTRACT_INVALID, "富友签约配置不是有效 JSON", ex);
        }
    }

    public void validateForScanpay(PaymentFuiouPayConfig config) {
        Require.notBlank(config.insCd(), PaymentCode.PAYMENT_CHANNEL_CONTRACT_INVALID, "富友签约配置缺少机构号");
        Require.notBlank(config.merchantNo(), PaymentCode.PAYMENT_CHANNEL_CONTRACT_INVALID, "富友签约配置缺少商户号");
        Require.notBlank(config.scanpayGatewayBaseUrl(), PaymentCode.PAYMENT_CHANNEL_CONTRACT_INVALID, "富友签约配置缺少扫码网关地址");
        Require.notBlank(config.notifyUrl(), PaymentCode.PAYMENT_CHANNEL_CONTRACT_INVALID, "富友签约配置缺少通知地址");
        Require.notBlank(config.privateKey(), PaymentCode.PAYMENT_CHANNEL_CONTRACT_INVALID, "富友签约配置缺少商户私钥");
        Require.notBlank(config.fuiouPublicKey(), PaymentCode.PAYMENT_CHANNEL_CONTRACT_INVALID, "富友签约配置缺少富友平台公钥");
    }

    public void validateForPcGateway(PaymentFuiouPayConfig config) {
        Require.notBlank(config.gatewayMerchantNo(), PaymentCode.PAYMENT_CHANNEL_CONTRACT_INVALID, "富友签约配置缺少网关商户号");
        Require.notBlank(config.gatewayMerchantKey(), PaymentCode.PAYMENT_CHANNEL_CONTRACT_INVALID, "富友签约配置缺少网关商户密钥");
        Require.notBlank(config.gatewayPayUrl(), PaymentCode.PAYMENT_CHANNEL_CONTRACT_INVALID, "富友签约配置缺少网关支付地址");
        Require.notBlank(config.gatewayQueryUrl(), PaymentCode.PAYMENT_CHANNEL_CONTRACT_INVALID, "富友签约配置缺少网关查单地址");
        Require.notBlank(config.gatewayPageNotifyUrl(), PaymentCode.PAYMENT_CHANNEL_CONTRACT_INVALID, "富友签约配置缺少页面跳转地址");
        Require.notBlank(config.gatewayBackNotifyUrl(), PaymentCode.PAYMENT_CHANNEL_CONTRACT_INVALID, "富友签约配置缺少后台通知地址");
    }
    ...
    private void validate(PaymentFuiouPayConfig config) {
        ...
        Require.isTrue(hasScanpayConfig(config) || hasPcGatewayConfig(config),
                PaymentCode.PAYMENT_CHANNEL_CONTRACT_INVALID, "富友签约配置缺少已开通能力的商户资料");
    }
