...
public class PaymentExceptionOrderRecorder {
    ...
    public PaymentExceptionOrderEntity createIfAbsent(
        ...
        Require.notNull(tenantId, PaymentCode.PAYMENT_EXCEPTION_ORDER_INVALID, "租户 ID 不能为空");
        Require.notBlank(relatedOrderNo, PaymentCode.PAYMENT_EXCEPTION_ORDER_INVALID, "关联订单号不能为空");
        Require.notBlank(exceptionType, PaymentCode.PAYMENT_EXCEPTION_ORDER_INVALID, "异常类型不能为空");
        Require.notBlank(severity, PaymentCode.PAYMENT_EXCEPTION_ORDER_INVALID, "异常级别不能为空");
        ...
        entity.setExceptionNo(numberService.next(PaymentNumberGenerator.PAY_EXCEPTION_NO));
        ...
        try {
            ...
            Require.notNull(existingAfterDuplicate,
                    PaymentCode.PAYMENT_EXCEPTION_ORDER_INVALID,
                    "异常订单幂等创建冲突，请重试");
