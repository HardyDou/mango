...
import io.mango.payment.core.service.IPaymentSensitiveFieldReencryptService;
...
public class PaymentSensitiveFieldReencryptService implements IPaymentSensitiveFieldReencryptService {
    ...
    private int resolveLimit(Integer limit) {
        ...
        Require.isTrue(resolved > 0 && resolved <= MAX_LIMIT,
                PaymentCode.PAYMENT_SENSITIVE_VALUE_INVALID, "重加密批量大小必须在 1-1000 之间");
        return resolved;
    }

    private int reencryptApplicationSecrets(String tenantId, int limit, Long operatorId, LocalDateTime now) {
        List<PaymentApplicationEntity> rows = applicationMapper.selectList(
                new LambdaQueryWrapper<PaymentApplicationEntity>()
                        .eq(PaymentApplicationEntity::getTenantId, tenantId)
                        .isNotNull(PaymentApplicationEntity::getAppSecret)
                        .notLikeRight(PaymentApplicationEntity::getAppSecret, "enc:")
                        .last("LIMIT " + limit));
        int updated = 0;
        for (PaymentApplicationEntity row : rows) {
    ...
    private EnterpriseSubjectReencryptCount reencryptEnterpriseSubjects(
            ...
            Long operatorId,
            LocalDateTime now) {
        List<PaymentEnterpriseSubjectEntity> rows = subjectMapper.selectList(
                new LambdaQueryWrapper<PaymentEnterpriseSubjectEntity>()
                        .eq(PaymentEnterpriseSubjectEntity::getTenantId, tenantId)
                        .and(wrapper -> wrapper
                                .isNotNull(PaymentEnterpriseSubjectEntity::getCreditCode)
                                .notLikeRight(PaymentEnterpriseSubjectEntity::getCreditCode, "enc:")
                                .or()
                                .isNotNull(PaymentEnterpriseSubjectEntity::getBankAccountNo)
                                .notLikeRight(PaymentEnterpriseSubjectEntity::getBankAccountNo, "enc:"))
                        .last("LIMIT " + limit));
        int creditCodeCount = 0;
        int bankAccountCount = 0;
        for (PaymentEnterpriseSubjectEntity row : rows) {
    ...
    private int reencryptSubjectBankAccounts(String tenantId, int limit, Long operatorId, LocalDateTime now) {
