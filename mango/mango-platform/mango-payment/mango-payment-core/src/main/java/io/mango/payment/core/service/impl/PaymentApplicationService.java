...
public class PaymentApplicationService implements IPaymentApplicationService {
    ...
    @Override
    public PageResult<PaymentApplicationVO> pageApplications(PaymentConfigPageQuery query) {
        PaymentConfigPageQuery resolved = query == null ? new PaymentConfigPageQuery() : query;
        IPage<PaymentApplicationEntity> page = applicationMapper.selectPage(new Page<>(resolved.getPage(), resolved.getSize()), wrapper(resolved));
        List<PaymentApplicationVO> records = page.getRecords().stream().map(this::toVO).collect(Collectors.toList());
        return PageResult.of(records, page.getTotal(), page.getCurrent(), page.getSize());
    }

    @Override
    public PaymentApplicationVO detailApplication(Long id) {
        return toVO(selectRequired(id));
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public PaymentApplicationSaveResultVO createApplication(CreatePaymentApplicationCommand command) {
        ...
        PaymentApplicationEntity entity = new PaymentApplicationEntity();
        ...
        return saveResult(entity, generatedSecret);
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public PaymentApplicationSaveResultVO updateApplication(UpdatePaymentApplicationCommand command) {
        Require.notNull(command, PaymentCode.PAYMENT_APPLICATION_INVALID);
        Require.notNull(command.getId(), PaymentCode.PAYMENT_APPLICATION_INVALID, "应用 ID 不能为空");
        validate(command, true);
        PaymentApplicationEntity entity = selectRequired(command.getId());
        copy(command, entity);
        String generatedSecret = refreshSecretState(command, entity);
    ...
    public Boolean deleteApplication(Long id) {
        PaymentApplicationEntity entity = selectRequired(id);
    ...
    private LambdaQueryWrapper<PaymentApplicationEntity> wrapper(PaymentConfigPageQuery query) {
        String keyword = PaymentContextSupport.trimToNull(query.getKeyword());
        return new LambdaQueryWrapper<PaymentApplicationEntity>()
                .and(StringUtils.hasText(keyword), nested -> nested
                        .like(PaymentApplicationEntity::getAppId, keyword)
                        .or()
                        .like(PaymentApplicationEntity::getAppName, keyword))
                .eq(query.getStatus() != null, PaymentApplicationEntity::getStatus, query.getStatus())
                .eq(PaymentApplicationEntity::getTenantId, PaymentContextSupport.currentTenantId())
                .orderByDesc(PaymentApplicationEntity::getUpdatedAt);
    }

    private PaymentApplicationEntity selectRequired(Long id) {
        Require.notNull(id, PaymentCode.PAYMENT_APPLICATION_INVALID, "应用 ID 不能为空");
        PaymentApplicationEntity entity = applicationMapper.selectById(id);
    ...
    private long countDeleteRelations(PaymentApplicationEntity entity) {
    ...
    private void validate(SavePaymentApplicationCommand command, boolean update) {
        ...
        Require.notBlank(command.getAppName(), PaymentCode.PAYMENT_APPLICATION_INVALID, "应用名称不能为空");
        Require.notNull(command.getIpWhitelistEnabled(), PaymentCode.PAYMENT_APPLICATION_INVALID, "IP 白名单开关不能为空");
        if (Integer.valueOf(1).equals(command.getIpWhitelistEnabled())) {
            Require.notBlank(command.getIpWhitelist(), PaymentCode.PAYMENT_APPLICATION_INVALID, "IP 白名单开启时必须配置允许来源");
        }
        Require.notNull(command.getPayloadEncryptEnabled(), PaymentCode.PAYMENT_APPLICATION_INVALID, "报文加密开关不能为空");
        if (Integer.valueOf(1).equals(command.getPayloadEncryptEnabled())) {
            Require.notBlank(command.getSignAlgorithm(), PaymentCode.PAYMENT_APPLICATION_INVALID, "报文加密开启时必须配置签名算法");
        }
        Require.notNull(command.getDemoApp(), PaymentCode.PAYMENT_APPLICATION_INVALID, "示例应用标记不能为空");
        Require.notNull(command.getStatus(), PaymentCode.PAYMENT_APPLICATION_INVALID, "状态不能为空");
    }

    private void copy(SavePaymentApplicationCommand command, PaymentApplicationEntity entity) {
    ...
    private String refreshSecretState(SavePaymentApplicationCommand command, PaymentApplicationEntity entity) {
        if (!Integer.valueOf(1).equals(command.getPayloadEncryptEnabled())) {
    ...
    private PaymentApplicationVO toVO(PaymentApplicationEntity entity) {
    ...
    private PaymentApplicationSaveResultVO saveResult(PaymentApplicationEntity entity, String generatedSecret) {
    ...
    private String generateUniqueAppId(String tenantId) {
        for (int i = 0; i < 8; i++) {
            String appId = "app_" + randomUrlSafe(DEFAULT_APP_ID_BYTES);
            Long count = applicationMapper.selectCount(new LambdaQueryWrapper<PaymentApplicationEntity>()
