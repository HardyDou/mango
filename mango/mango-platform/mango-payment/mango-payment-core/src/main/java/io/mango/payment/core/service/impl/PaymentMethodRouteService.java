...
public class PaymentMethodRouteService implements IPaymentMethodRouteService {
    ...
    @Override
    public PageResult<PaymentMethodRouteRuleVO> pageRouteRules(PaymentMethodRoutePageQuery query) {
        PaymentMethodRoutePageQuery resolved = query == null ? new PaymentMethodRoutePageQuery() : query;
        IPage<PaymentMethodRouteRuleEntity> page = routeRuleMapper.selectPage(new Page<>(resolved.getPage(), resolved.getSize()), wrapper(resolved));
    ...
    @Override
    public PaymentMethodRouteRuleVO detailRouteRule(Long id) {
        return toVO(selectRequired(id), true);
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public Long createRouteRule(SavePaymentMethodRouteRuleCommand command) {
        Require.notNull(command, PaymentCode.PAYMENT_METHOD_ROUTE_INVALID);
        validate(command, false);
        PaymentMethodRouteRuleEntity entity = new PaymentMethodRouteRuleEntity();
        copy(command, entity);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Boolean updateRouteRule(SavePaymentMethodRouteRuleCommand command) {
        Require.notNull(command, PaymentCode.PAYMENT_METHOD_ROUTE_INVALID);
        validate(command, true);
        PaymentMethodRouteRuleEntity entity = selectRequired(command.getId());
        copy(command, entity);
    ...
    public Boolean deleteRouteRule(Long id) {
        PaymentMethodRouteRuleEntity entity = selectRequired(id);
    ...
    @Override
    public PaymentMethodRouteTrialVO trialRoute(PaymentMethodRouteTrialCommand command) {
        Require.notNull(command, PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "支付路由试算命令不能为空");
    ...
    private LambdaQueryWrapper<PaymentMethodRouteRuleEntity> wrapper(PaymentMethodRoutePageQuery query) {
        String keyword = PaymentContextSupport.trimToNull(query.getKeyword());
        return new LambdaQueryWrapper<PaymentMethodRouteRuleEntity>()
                .and(StringUtils.hasText(keyword), nested -> nested
                        .like(PaymentMethodRouteRuleEntity::getRuleCode, keyword)
                        .or()
                        .like(PaymentMethodRouteRuleEntity::getRuleName, keyword)
                        .or()
                        .like(PaymentMethodRouteRuleEntity::getMethodCode, keyword))
                .eq(query.getApplicationId() != null, PaymentMethodRouteRuleEntity::getAppId, query.getApplicationId())
                .eq(query.getSubjectId() != null, PaymentMethodRouteRuleEntity::getSubjectId, query.getSubjectId())
                .eq(StringUtils.hasText(query.getMethodCode()), PaymentMethodRouteRuleEntity::getMethodCode, PaymentContextSupport.trimToNull(query.getMethodCode()))
                .eq(StringUtils.hasText(query.getTerminalType()), PaymentMethodRouteRuleEntity::getTerminalType, PaymentContextSupport.trimToNull(query.getTerminalType()))
                .eq(StringUtils.hasText(query.getEnvironment()), PaymentMethodRouteRuleEntity::getEnvironment, PaymentContextSupport.trimToNull(query.getEnvironment()))
                .eq(query.getStatus() != null, PaymentMethodRouteRuleEntity::getStatus, query.getStatus())
                .eq(PaymentMethodRouteRuleEntity::getTenantId, PaymentContextSupport.currentTenantId())
                .orderByDesc(PaymentMethodRouteRuleEntity::getUpdatedAt)
                .orderByDesc(PaymentMethodRouteRuleEntity::getId);
    }

    private PaymentMethodRouteRuleEntity selectRequired(Long id) {
        Require.notNull(id, PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "路由规则 ID 不能为空");
        PaymentMethodRouteRuleEntity entity = routeRuleMapper.selectById(id);
    ...
    private void validate(SavePaymentMethodRouteRuleCommand command, boolean update) {
        if (update) {
            Require.notNull(command.getId(), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "路由规则 ID 不能为空");
        }
        Require.notBlank(command.getRuleCode(), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "路由规则编码不能为空");
        Require.notBlank(command.getRuleName(), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "路由规则名称不能为空");
        Require.notBlank(command.getMethodCode(), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "标准支付方式不能为空");
        Require.notBlank(command.getTerminalType(), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "终端类型不能为空");
        resolveRouteEnvironment(command);
        Require.notBlank(command.getRouteMode(), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "路由模式不能为空");
        Require.notNull(command.getFallbackEnabled(), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "失败降级开关不能为空");
        Require.notNull(command.getStatus(), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "状态不能为空");
        Require.isTrue(TERMINAL_TYPES.contains(command.getTerminalType().trim()), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "终端类型仅支持 WEB、H5");
        Require.isTrue(ROUTE_MODES.contains(command.getRouteMode().trim()), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "路由模式不正确");
        Require.isTrue(STATUS_VALUES.contains(command.getFallbackEnabled()), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "失败降级开关不正确");
        Require.isTrue(STATUS_VALUES.contains(command.getStatus()), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "状态不正确");
        Require.isTrue(command.getItems() != null && !command.getItems().isEmpty(), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "路由明细不能为空");
    ...
    private void validateTrial(PaymentMethodRouteTrialCommand command) {
        Require.notNull(command, PaymentCode.PAYMENT_METHOD_ROUTE_INVALID);
        Require.notNull(command.getApplicationId(), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "应用不能为空");
        Require.notNull(command.getSubjectId(), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "企业主体不能为空");
        Require.notBlank(command.getMethodCode(), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "标准支付方式不能为空");
        Require.notBlank(command.getTerminalType(), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "终端类型不能为空");
    ...
    private void validateApplication(Long appId) {
        ...
        PaymentApplicationEntity application = applicationMapper.selectById(appId);
    ...
    private void validateSubject(Long subjectId) {
        ...
        PaymentEnterpriseSubjectEntity subject = subjectMapper.selectById(subjectId);
    ...
    private void validateMethod(String methodCode) {
        PaymentMethodEntity method = methodMapper.selectOne(new LambdaQueryWrapper<PaymentMethodEntity>()
                ...
                .eq(PaymentMethodEntity::getMethodCode, methodCode.trim())
                .last("limit 1"));
    ...
    private void validateUniqueRuleCode(SavePaymentMethodRouteRuleCommand command, boolean update) {
        LambdaQueryWrapper<PaymentMethodRouteRuleEntity> wrapper = new LambdaQueryWrapper<PaymentMethodRouteRuleEntity>()
                .eq(PaymentMethodRouteRuleEntity::getTenantId, PaymentContextSupport.currentTenantId())
                .eq(PaymentMethodRouteRuleEntity::getRuleCode, command.getRuleCode().trim());
        if (update) {
            wrapper.ne(PaymentMethodRouteRuleEntity::getId, command.getId());
        }
        Long count = routeRuleMapper.selectCount(wrapper);
        Require.isTrue(count == null || count == 0L, PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "路由规则编码不能重复");
    }
    ...
    private void validateItems(SavePaymentMethodRouteRuleCommand command) {
        ...
        for (SavePaymentMethodRouteRuleItemCommand item : command.getItems()) {
            Require.notNull(item, PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "路由明细不能为空");
            Require.notNull(item.getContractCapabilityId(), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "签约能力不能为空");
            Require.notNull(item.getStatus(), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "路由明细状态不能为空");
            Require.isTrue(STATUS_VALUES.contains(item.getStatus()), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "路由明细状态不正确");
            Require.isTrue(capabilityIds.add(item.getContractCapabilityId()), PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "同一路由规则不能重复选择签约能力");
            Money.requireRange(item.getMinAmount(), item.getMaxAmount(), "路由明细");
            PaymentChannelContractCapabilityEntity capability = contractCapabilityMapper.selectRouteCapability(
    ...
    private void resolveRouteEnvironment(SavePaymentMethodRouteRuleCommand command) {
        ...
        Require.isTrue(command.getItems() != null && !command.getItems().isEmpty(),
                PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "路由明细不能为空");
        Long capabilityId = command.getItems().get(0).getContractCapabilityId();
        Require.notNull(capabilityId, PaymentCode.PAYMENT_METHOD_ROUTE_INVALID, "签约能力不能为空");
    ...
    private void copy(SavePaymentMethodRouteRuleCommand command, PaymentMethodRouteRuleEntity entity) {
    ...
    private void saveItems(PaymentMethodRouteRuleEntity rule, List<SavePaymentMethodRouteRuleItemCommand> items) {
        for (SavePaymentMethodRouteRuleItemCommand command : items) {
            PaymentMethodRouteRuleItemEntity item = new PaymentMethodRouteRuleItemEntity();
    ...
    private PaymentMethodRouteRuleVO toVO(PaymentMethodRouteRuleEntity entity, boolean withItems) {
        ...
        if (entity.getAppId() != null) {
            PaymentApplicationEntity application = applicationMapper.selectById(entity.getAppId());
        ...
        PaymentMethodEntity method = methodMapper.selectOne(new LambdaQueryWrapper<PaymentMethodEntity>()
                .eq(PaymentMethodEntity::getTenantId, entity.getTenantId())
                .eq(PaymentMethodEntity::getMethodCode, entity.getMethodCode())
                .last("limit 1"));
        vo.setMethodName(method == null ? null : method.getMethodName());
        if (withItems) {
            vo.setItems(toApiList(routeRuleItemMapper.selectItemsByRuleId(
                    entity.getId(), entity.getTenantId()), PaymentMethodRouteRuleItemVO.class));
        }
        return vo;
    }
