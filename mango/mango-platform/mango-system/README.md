...
> Issue #934：运行时 API Resource 扫描现在保留注解声明版本；系统公共配置接口与静态资源声明保持版本 2。接口路径、访问模式和业务接入方式不变。
...
| 网站配置 | 维护 Admin 登录页、后台框架展开 Logo、折叠 Logo、浏览器标题、favicon 和页脚展示信息 | `/system/admin-branding`、`AdminBrandingView`、`adminBrandingApi` |
...
| 机构基线对账 | Resource Registry 初次同步因内置角色等前置数据缺失时，先重放一次不标记完成的幂等前置基线并触发 Resource 立即重试；资源成功后再执行最终对账，对账自身失败时定时重试 | `TenantProvisioningReconciliationRunner` |
...
| `SysLoginLogApi` | 登录日志分页、详情、记录、清理、统计；`pageCurrentUser` 只返回当前租户、当前账号记录。 |
...
8. 应用启动时会在 Resource Registry 同步成功后对启用机构执行一次幂等基线对账；初次同步失败时不会提前执行最终对账，后续同步成功事件会自动触发，对账失败则按配置间隔重试。Resource 与机构对账全部完成前 readiness 保持拒绝流量，完成后转为接收流量；扩展点实现必须可安全重复执行。
9. 个人中心“登录日志”使用 `GET /system/log/login/my/list`，只要求登录态，服务端固定按当前租户和当前账号过滤，不接受前端传入用户 ID。

## 6. 配置说明

`mango-system` 当前没有专属 `@ConfigurationProperties`。运行时可维护的配置来自数据库表 `sys_config`；机构启动对账另支持以下应用配置：

| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `mango.system.tenant-provisioning.retry-interval` | `10s` | 资源同步完成后，机构幂等基线对账发生瞬时失败时的重试间隔。 |
...
系统默认租户、行政区划、字典、配置和国际化文案通过 `mango-resource` 注入，不在 Flyway 中写业务数据。正式必需数据放在 `META-INF/mango/resources/`；默认产品态只初始化一个租户。
...
`SYSTEM_TENANT` 落库到 `sys_tenant`。正式资源只登记平台默认租户“芒果集团”，demo 不再追加第二租户；租户管理能力与内部 `tenant_id` 隔离边界仍保留。
...
| 网站配置 | `GET /system/admin-branding` | `system:admin-branding:query` |
| 网站公共配置 | `GET /system/admin-branding/public` | `PUBLIC`（匿名访问，由 `@PublicAccess` / `API_RESOURCE` 注册） |
| 保存网站配置 | `PUT /system/admin-branding` | `system:admin-branding:edit` |
...
| `AdminBrandingVO` | 无 | `enabled`、`title`、`shortTitle`、`subtitle`、`loginTitle`、`loginSubtitle`、`logoFile`、`logoIconFile`、`faviconFile`、`loginImageFile`、`footerCopyright`、`icp`、`contact` |
...
网站配置复用 `sys_config`，配置 key 统一使用 `admin.branding.*`。`admin.branding.enabled` 控制是否启用自定义后台外观；`logoFile` 用于展开态 Logo，`logoIconFile` 用于折叠态 Logo。图片类字段只保存文件中心 ID，例如 `{id}`，不能保存预览 URL、下载 URL 或对象存储直连地址。运行时由 Admin Shell 根据文件 ID 调用文件中心派生临时展示地址。
...
应用启动且 Resource Registry 完成正式资源同步后，`TenantProvisioningReconciliationRunner` 会遍历启用机构，幂等重放 `TenantProvisioner` 和 `TenantPackageBindingHandler`，用于补齐资源创建机构或存量机构缺少的模块基线。如果首次资源同步因 `AUTH_ROLE` 等机构前置数据尚未创建而延后，Runner 会先执行一次不设置最终完成态的幂等前置基线，再通知 Resource 立即重试；资源成功事件触发第二次最终对账。扩展实现必须幂等，不得依赖仅执行一次。
...
| 网站配置 | `system:admin-branding:query`、`system:admin-branding:edit` |
