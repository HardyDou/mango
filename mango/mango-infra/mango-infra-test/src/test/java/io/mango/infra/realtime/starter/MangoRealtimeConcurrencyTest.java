 ...
 */
@SpringBootTest(
        classes = MangoRealtimeConcurrencyTest.TestApplication.class,
...
class MangoRealtimeConcurrencyTest {
    ...
    void concurrent_publishToUser_noExceptions() throws Exception {
        ...
        WebSocketSession session = connectWebSocketClient(
                receivedMessages, "ws://localhost:" + port + "/realtime/ws?token=test&tenantId=concurrent&userId=9999"
        );
                ...
                executor.submit(() -> {
                    try {
                        for (int m = 0; m < messagesPerSender; m++) {
                            messagePublisher.publishToUser(9999L, "concurrent-msg", "sender-" + senderId + "-msg-" + m);
    ...
    void concurrent_subscribeAndUnsubscribe_noDuplicateDelivery() throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            WebSocketSession s = connectWebSocketClient(
                    queue,
                    "ws://localhost:" + port + "/realtime/ws?token=test&tenantId=rapid&userId=" + userId
            );
            sessions.add(s);
            queues.add(queue);
        ...
        for (int i = 0; i < 5; i++) {
            messagePublisher.publishToUser(8000L + i, "rapid-msg", "msg-for-" + (8000 + i));
        }
    ...
    void mixedProtocol_sameUserId_bothConnectionsReceiveMessage() throws Exception {
        ...
        WebSocketSession wsSession = connectWebSocketClient(
                wsReceived,
                "ws://localhost:" + port + "/realtime/ws?token=test&tenantId=mixed&userId=7001"
        );

        var sseSubscription = WebClient.create()
                .get()
                .uri("http://localhost:" + port + "/realtime/subscribe?userId=7001")
        ...
        try {
            ...
            messagePublisher.publishToUser(7001L, "mixed-msg", "dual-delivery");
    ...
    void mixedProtocol_userIdOnlyWSOnline_sseOffline_messageRoutedCorrectly() throws Exception {
        ...
        WebSocketSession wsSession = connectWebSocketClient(
                wsReceived,
                "ws://localhost:" + port + "/realtime/ws?token=test&tenantId=mixed2&userId=7002"
        );
        ...
        try {
            ...
            messagePublisher.publishToUser(7002L, "ws-only-msg", "ws-should-receive");
    ...
    void lifecycle_connectDisconnectReconnect_resumesDelivery() throws Exception {
        ...
        WebSocketSession session1 = connectWebSocketClient(
                received,
                "ws://localhost:" + port + "/realtime/ws?token=test&tenantId=life&userId=6001"
        );
        ...
        messagePublisher.publishToUser(6001L, "after-reconnect", "message-after-reconnect");
    ...
    void broadcast_allConnectedSessionsReceive() throws Exception {
        ...
        WebSocketSession session1 = connectWebSocketClient(
                received1,
                "ws://localhost:" + port + "/realtime/ws?token=test&tenantId=bcast&userId=5001"
        );
        WebSocketSession session2 = connectWebSocketClient(
                received2,
                "ws://localhost:" + port + "/realtime/ws?token=test&tenantId=bcast&userId=5002"
        );
        ...
        try {
            ...
            // Broadcast to all
            messagePublisher.broadcast("bcast", "hello everyone");
    ...
    void tenant_isolation_tenantAMessagesNotReceivedByTenantB() throws Exception {
        ...
        WebSocketSession sessionA = connectWebSocketClient(
                receivedA,
                "ws://localhost:" + port + "/realtime/ws?token=test&tenantId=tenant-A&userId=4001"
        );
        WebSocketSession sessionB = connectWebSocketClient(
                receivedB,
                "ws://localhost:" + port + "/realtime/ws?token=test&tenantId=tenant-B&userId=4002"
        );
        ...
        try {
            ...
            messagePublisher.publishToTenant("tenant-A", "tenant-msg", "secret-for-A");
