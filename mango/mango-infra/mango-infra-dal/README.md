...
├── mango-infra-dal-core/              ← MemoryKvStore + 9个 Memory 实现
...
### DbKvStore — 使用 DAL 专属配置（ Druid 连接池）
...
### DbKvStore — 兼容 dynamic-datasource
...
# 强制 DbKvStore（需 DataSource + RedissonClient）
mango:
  dal:
    type: db
...
| 条件 | 效果 |
|------|------|
| `mango.dal.type=redis` | 强制使用 RedisKvStore（无 RedissonClient 则启动失败） |
| `mango.dal.type=db` | 强制使用 DbKvStore（需同时有 DataSource 和 RedissonClient） |
| `mango.dal.type=memory` | 强制使用 MemoryKvStore |
| `mango.dal.type=auto`（默认） | RedissonClient 存在 → RedisKvStore；否则 → MemoryKvStore |

> **自动检测不会级联到 DbKvStore**。如果需要使用 DbKvStore，必须显式设置 `type=db`。
